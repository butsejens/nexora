...
app.use((req, res, next) => {
  const isSlowRoute =
    ...
    req.path.includes("/topscorers/") ||
    req.path.includes("/topassists/");
...
// Transfermarkt blocks direct access without a Referer → proxied.
// Sofascore api.sofascore.app blocks CORS/direct requests from apps and browsers → proxied.
function proxyPhotoUrl(url) {
  if (!url || !url.startsWith("http")) return url || null;
  if (
    /transfermarkt\.technology|img\.a\.transfermarkt|img\.[a-z]\.transfermarkt/i.test(
      url,
    )
  ) {
    return `${PUBLIC_URL}/api/img?url=${encodeURIComponent(url)}`;
  }
  if (/api\.sofascore\.app\/api\/v1\/(player|team)\/[^/]+\/image/i.test(url)) {
    return `${PUBLIC_URL}/api/img?url=${encodeURIComponent(url)}`;
  }
  return url;
}
...
function countSportsItems(payload) {
  ...
  if (
    Array.isArray(p.live) ||
    Array.isArray(p.upcoming) ||
    Array.isArray(p.finished)
  ) {
    return (
      (Array.isArray(p.live) ? p.live.length : 0) +
...
function rememberLastGoodSportsPayload(
  cacheKey,
  payload,
  ttlMs = 6 * 60 * 60 * 1000,
) {
...
function getLastGoodSportsPayload(cacheKey) {
  return (
    cacheGet(`${cacheKey}__last_good`) ||
    cacheGetStale(`${cacheKey}__last_good`) ||
    null
  );
}
...
const APIFY_TRANSFERMARKT_ACTOR =
  process.env.APIFY_TRANSFERMARKT_ACTOR ||
  "data_xplorer/transfermarkt-api-scraper";
const APIFY_SOFASCORE_ACTOR =
  process.env.APIFY_SOFASCORE_ACTOR || "azzouzana/sofascore-scraper-pro";
const GITHUB_FIFA18_PERSONAL_CSV =
  "https://raw.githubusercontent.com/4m4n5/fifa18-all-player-statistics/master/Complete/PlayerPersonalData.csv";
const GITHUB_FIFA18_OVERVIEW_CSV =
  "https://raw.githubusercontent.com/kevinheavey/fifa18-even-more-player-data/master/data/final/current/overview.csv";
const GITHUB_PLAYER_DATA_TTL_MS = Number(
  process.env.GITHUB_PLAYER_DATA_TTL_MS || 12 * 60 * 60 * 1000,
);

const ESPN_SCOREBOARD_BASE =
  "https://site.web.api.espn.com/apis/v2/sports/soccer/scoreboard";
...
const ESPN_API_KEY = String(
  process.env.ESPN_API_KEY || process.env.ESPN_KEY || "",
).trim();
const ESPN_PUBLIC_CACHE_TTL_MS = Number(
  process.env.ESPN_PUBLIC_CACHE_TTL_MS || 30000,
);
...
const ESPN_LEAGUE_SCOREBOARDS = {
  ...
  "La Liga 2":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/esp.2/scoreboard",
  "Copa del Rey":
  ...
  "Jupiler Pro League":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/scoreboard",
  "Challenger Pro League":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.2/scoreboard",
  "Belgian Cup":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.cup/scoreboard",
  "Ligue 1":
  ...
  "Eerste Divisie":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/ned.2/scoreboard",
  "KNVB Beker":
    "https://site.api.espn.com/apis/site/v2/sports/soccer/ned.knvb_beker/scoreboard",
  "FIFA World Cup":
...
function mapEspnMultiSportEvent(ev, sport, leagueSlug) {
  ...
  const leagueName =
    ev?.league?.name || ev?.league?.shortName || ev?._leagueHint || "";
...
async function espnScoreboard(dateYmd) {
  ...
  const results = await Promise.allSettled(
    leagueUrls.map(async ([leagueName, baseUrl]) => {
      const url = withEspnApiKey(
        `${baseUrl}?dates=${encodeURIComponent(dates)}&limit=20`,
      );
      const controller = new AbortController();
      const timer = setTimeout(
        () => controller.abort(),
      ...
      try {
        ...
        const leagueSlug = String(
          baseUrl.match(/\/soccer\/([^/]+)\/scoreboard/)?.[1] || "",
        );
  ...
  console.log(
    `[espn] scoreboard ${dates}: ${allEvents.length} events from ${leagueUrls.length} leagues`,
  );

  // If per-league failed or returned nothing, try generic endpoint
  if (allEvents.length === 0) {
    const url = withEspnApiKey(
      `${ESPN_SCOREBOARD_BASE}?dates=${encodeURIComponent(dates)}`,
    );
    const resp = await fetch(url, {
      headers: espnRequestHeaders(),
    });
...
function normalizeStatusFromEspn(comp) {
  ...
  if (state === "post")
    return { status: "finished", minute, statusDetail: detail };
...
const TEAM_LOGO_ALIASES = {
  ...
  "royal charleroi sc": "Royal Charleroi SC",
  ...
  patro: "Patro Eisden Maasmechelen",
  "francs borains": "Francs Borains",
  "rfc francs borains": "Francs Borains",
  virton: "Royal Excelsior Virton",
  "excelsior virton": "Royal Excelsior Virton",
  "re virton": "Royal Excelsior Virton",
  "rfc liege": "RFC Liège",
  "rfc liège": "RFC Liège",
  "fc liege": "RFC Liège",
  liege: "RFC Liège",
  ...
  "sporting lokeren-temse": "Lokeren-Temse",
  lokeren: "Lokeren-Temse",
  "fcv dender": "FCV Dender EH",
  dender: "FCV Dender EH",
  "dender eh": "FCV Dender EH",
  "rsca futures": "RSC Anderlecht Futures",
  "anderlecht futures": "RSC Anderlecht Futures",
  ...
  // Israel
  "maccabi tel aviv": "Maccabi Tel Aviv FC",
  "maccabi tel-aviv": "Maccabi Tel Aviv FC",
  ...
  "aris limassol fc": "Aris Limassol FC",
  ...
  "ferencvaros tc": "Ferencvárosi TC",
  "puskas akademia": "Puskás Akadémia FC",
  "puskas akademia fc": "Puskás Akadémia FC",
  "mol fehervar": "MOL Fehérvár FC",
  "mol fehervar fc": "MOL Fehérvár FC",
  videoton: "MOL Fehérvár FC",
  // Slovakia
  ...
  "fk astana": "FK Astana",
  // Armenia
  "pyunik yerevan": "FC Pyunik",
  pyunik: "FC Pyunik",
  // Azerbaijan
  qarabag: "Qarabağ FK",
  "qarabag fk": "Qarabağ FK",
  "neftchi baku": "Neftçi PFK",
  "neftci baku": "Neftçi PFK",
  // Georgia
  "dinamo tbilisi": "FC Dinamo Tbilisi",
  "fc dinamo tbilisi": "FC Dinamo Tbilisi",
  // Finland
  ...
  // Albania
  "partizani tirana": "FK Partizani",
  "fk partizani": "FK Partizani",
  // Slovakia (expanded)
  "spartak trnava": "Spartak Trnava",
  // Republic of Ireland
  "shamrock rovers": "Shamrock Rovers FC",
  "shamrock rovers fc": "Shamrock Rovers FC",
  // Luxembourg
  "f91 dudelange": "F91 Dudelange",
  dudelange: "F91 Dudelange",
  // North Macedonia
  shkendija: "KF Shkëndija",
  "shkendija tetovo": "KF Shkëndija",
  // Moldova
  "sheriff tiraspol": "FC Sheriff Tiraspol",
  sheriff: "FC Sheriff Tiraspol",
  // Belarus
  "bate borisov": "FC BATE Borisov",
  ...
  "fc flora": "FC Flora",
  // Bosnia
  "zrinjski mostar": "HŠK Zrinjski",
  zrinjski: "HŠK Zrinjski",
  // Montenegro
  "buducnost podgorica": "FK Budućnost Podgorica",
  buducnost: "FK Budućnost Podgorica",
  // Slovenia
...
const TEAM_FILENAME_TO_FOLDER = (() => {
  ...
  for (const [, folder] of Object.entries(LEAGUE_TO_LOGO_FOLDER)) {
    ...
    if (!folderByCountryKeyword[country])
      folderByCountryKeyword[country] = folder;
  }
  // Hard-map known teams to their domestic folder by filename patterns
  const teamCountryMap = {
    // Belgium
    "Club Brugge KV": "Belgium - Jupiler Pro League",
    "KRC Genk": "Belgium - Jupiler Pro League",
    "Royal Antwerp FC": "Belgium - Jupiler Pro League",
    "KAA Gent": "Belgium - Jupiler Pro League",
    "RSC Anderlecht": "Belgium - Jupiler Pro League",
    "Standard Liège": "Belgium - Jupiler Pro League",
    "Union Saint-Gilloise": "Belgium - Jupiler Pro League",
    "Cercle Brugge": "Belgium - Jupiler Pro League",
    "Oud-Heverlee Leuven": "Belgium - Jupiler Pro League",
    "KV Mechelen": "Belgium - Jupiler Pro League",
    "Sint-Truidense VV": "Belgium - Jupiler Pro League",
    "KVC Westerlo": "Belgium - Jupiler Pro League",
    "FCV Dender EH": "Belgium - Jupiler Pro League",
    "Zulte Waregem": "Belgium - Jupiler Pro League",
    "RAAL La Louvière": "Belgium - Jupiler Pro League",
    "Royal Charleroi SC": "Belgium - Jupiler Pro League",
    "KV Kortrijk": "Belgium - Jupiler Pro League",
    RWDM: "Belgium - Jupiler Pro League",
    "Beerschot VA": "Belgium - Jupiler Pro League",
    // England
    "Arsenal FC": "England - Premier League",
    "Aston Villa": "England - Premier League",
    "AFC Bournemouth": "England - Premier League",
    "Brentford FC": "England - Premier League",
    "Brighton & Hove Albion": "England - Premier League",
    "Burnley FC": "England - Premier League",
    "Chelsea FC": "England - Premier League",
    "Crystal Palace": "England - Premier League",
    "Everton FC": "England - Premier League",
    "Fulham FC": "England - Premier League",
    ...
    "Sunderland AFC": "England - Premier League",
    ...
    "FC Barcelona": "Spain - LaLiga",
    "Atlético de Madrid": "Spain - LaLiga",
    "Real Sociedad": "Spain - LaLiga",
    "Real Betis Balompié": "Spain - LaLiga",
    "Villarreal CF": "Spain - LaLiga",
    "Athletic Bilbao": "Spain - LaLiga",
    "Sevilla FC": "Spain - LaLiga",
    "Valencia CF": "Spain - LaLiga",
    "Girona FC": "Spain - LaLiga",
    "Celta de Vigo": "Spain - LaLiga",
    "Getafe CF": "Spain - LaLiga",
    "Rayo Vallecano": "Spain - LaLiga",
    "CA Osasuna": "Spain - LaLiga",
    "RCD Mallorca": "Spain - LaLiga",
    "Deportivo Alavés": "Spain - LaLiga",
    "RCD Espanyol Barcelona": "Spain - LaLiga",
    "Elche CF": "Spain - LaLiga",
    "Real Oviedo": "Spain - LaLiga",
    "Levante UD": "Spain - LaLiga",
    // Germany
    ...
    "AJ Auxerre": "France - Ligue 1",
    // Netherlands
    "Ajax Amsterdam": "Netherlands - Eredivisie",
    "PSV Eindhoven": "Netherlands - Eredivisie",
    "Feyenoord Rotterdam": "Netherlands - Eredivisie",
    "AZ Alkmaar": "Netherlands - Eredivisie",
    "Twente Enschede FC": "Netherlands - Eredivisie",
    ...
    "Excelsior Rotterdam": "Netherlands - Eredivisie",
    "FC Volendam": "Netherlands - Eredivisie",
    // Portugal
    "SL Benfica": "Portugal - Liga Portugal",
    "FC Porto": "Portugal - Liga Portugal",
    "Sporting CP": "Portugal - Liga Portugal",
    "SC Braga": "Portugal - Liga Portugal",
    "Vitória SC": "Portugal - Liga Portugal",
    "Boavista FC": "Portugal - Liga Portugal",
    "Gil Vicente FC": "Portugal - Liga Portugal",
    "Casa Pia AC": "Portugal - Liga Portugal",
    "FC Famalicão": "Portugal - Liga Portugal",
    "Rio Ave FC": "Portugal - Liga Portugal",
    "CF Estrela da Amadora": "Portugal - Liga Portugal",
    "FC Arouca": "Portugal - Liga Portugal",
    "Moreirense FC": "Portugal - Liga Portugal",
    // Scotland
    "Celtic FC": "Scotland - Scottish Premiership",
    "Rangers FC": "Scotland - Scottish Premiership",
    "Heart of Midlothian": "Scotland - Scottish Premiership",
    "Aberdeen FC": "Scotland - Scottish Premiership",
    "Hibernian FC": "Scotland - Scottish Premiership",
    "Dundee United": "Scotland - Scottish Premiership",
    "St Mirren": "Scotland - Scottish Premiership",
    "Kilmarnock FC": "Scotland - Scottish Premiership",
    "Motherwell FC": "Scotland - Scottish Premiership",
    "Ross County FC": "Scotland - Scottish Premiership",
    "Livingston FC": "Scotland - Scottish Premiership",
    "St Johnstone": "Scotland - Scottish Premiership",
    // Turkey
    Galatasaray: "Türkiye - Süper Lig",
    Fenerbahce: "Türkiye - Süper Lig",
    "Besiktas JK": "Türkiye - Süper Lig",
    Trabzonspor: "Türkiye - Süper Lig",
    Kasimpasa: "Türkiye - Süper Lig",
    "Basaksehir FK": "Türkiye - Süper Lig",
    // Denmark
    "FC København": "Denmark - Superliga",
    "FC Midtjylland": "Denmark - Superliga",
    "Brøndby IF": "Denmark - Superliga",
    "FC Nordsjælland": "Denmark - Superliga",
    "Aarhus GF": "Denmark - Superliga",
    // Switzerland
    "BSC Young Boys": "Switzerland - Super League",
    "FC Basel 1893": "Switzerland - Super League",
    "FC Zürich": "Switzerland - Super League",
    "Servette FC": "Switzerland - Super League",
    "FC Lugano": "Switzerland - Super League",
    // Norway
    "FK Bodø/Glimt": "Norway - Eliteserien",
    "Rosenborg BK": "Norway - Eliteserien",
    "Molde FK": "Norway - Eliteserien",
    "Viking FK": "Norway - Eliteserien",
    "SK Brann": "Norway - Eliteserien",
    // Sweden
    "Malmö FF": "Sweden - Allsvenskan",
    "Djurgårdens IF": "Sweden - Allsvenskan",
    "Hammarby IF": "Sweden - Allsvenskan",
    AIK: "Sweden - Allsvenskan",
    // Poland
    "Legia Warszawa": "Poland - PKO BP Ekstraklasa",
    "Lech Poznan": "Poland - PKO BP Ekstraklasa",
    "Raków Częstochowa": "Poland - PKO BP Ekstraklasa",
    // Croatia
    "GNK Dinamo Zagreb": "Croatia - SuperSport HNL",
    "HNK Hajduk Split": "Croatia - SuperSport HNL",
    // Greece
    "Olympiacos FC": "Greece - Super League 1",
    "Panathinaikos FC": "Greece - Super League 1",
    "AEK Athens": "Greece - Super League 1",
    "PAOK FC": "Greece - Super League 1",
    // Czech Republic
    "AC Sparta Prague": "Czech Republic - Chance Liga",
    "SK Slavia Prague": "Czech Republic - Chance Liga",
    "SK Sigma Olomouc": "Czech Republic - Chance Liga",
    // Serbia
    "FK Crvena zvezda": "Serbia - Super liga Srbije",
    "FK Partizan": "Serbia - Super liga Srbije",
    // Romania
    FCSB: "Romania - SuperLiga",
    "CFR Cluj": "Romania - SuperLiga",
    // Ukraine
    "Shakhtar Donetsk": "Ukraine - Premier Liga",
    "FC Dynamo Kyiv": "Ukraine - Premier Liga",
    // Israel
    "Maccabi Tel Aviv FC": "Israel - Ligat ha'Al",
    "Maccabi Haifa FC": "Israel - Ligat ha'Al",
    "Hapoel Beer Sheva FC": "Israel - Ligat ha'Al",
    // Bulgaria
    "PFC Ludogorets Razgrad": "Bulgaria - efbet Liga",
    "PFC CSKA Sofia": "Bulgaria - efbet Liga",
    "PFC Levski Sofia": "Bulgaria - efbet Liga",
    // Austria
    "SK Rapid Wien": "Austria - Bundesliga",
    "FK Austria Wien": "Austria - Bundesliga",
    LASK: "Austria - Bundesliga",
    "Wolfsberger AC": "Austria - Bundesliga",
    "FC Red Bull Salzburg": "Austria - Bundesliga",
    "SK Sturm Graz": "Austria - Bundesliga",
...
function resolveFootballLogosUrl(teamName, leagueName) {
  ...
  if (cached && Date.now() - cached.ts < FOOTBALL_LOGOS_CACHE_TTL)
    return cached.url;
  ...
  if (!fileName) {
    const stripped = normalized
      .replace(
        /\b(fc|afc|sc|ac|cf|fk|sv|vfl|vfb|tsg|bsc|rsc|krc|kvc|kaa|kv|us|ss|acf|ssc|rc|ogc|losc|1\s*fc|bv)\b/g,
        "",
      )
      .replace(/\s+/g, " ")
      .trim();
  ...
  if (!fileName && normalized.length >= 5) {
    ...
    if (inputTokens.size >= 1) {
      ...
      for (const k of aliasKeys) {
        ...
        const tokenScore =
          overlap / Math.max(inputTokens.size, kTokens.size, 1);
  ...
  const rawFolder =
    LEAGUE_TO_LOGO_FOLDER[leagueName] ??
    LEAGUE_TO_LOGO_FOLDER[normalizeLeagueName(leagueName)] ??
    undefined;
...
function normalizePlayerPhoto(_playerId, ...candidates) {
  for (const c of candidates) {
    ...
    // Never accept Wikipedia/Wikimedia/generated-avatar URLs as player photos.
    // These cause historical figures (e.g. Edgar Allan Poe) to appear for players
    // whose name incidentally matches a Wikipedia article title.
    if (/wikipedia\.org|wikimedia\.org|wikidata\.org|ui-avatars\.com/i.test(v))
      continue;
    return v;
  }
  return null;
...
function isEspnHeadshotUrl(url) {
  return /a\.espncdn\.com\/i\/headshots\/soccer\/players\/full\//i.test(
    String(url || ""),
  );
}
...
function keepIncomingPlayerPhoto(url, { allowEspn = false } = {}) {
  const normalized = normalizeRemoteMediaUrl(url);
  if (!normalized) return null;
  // ESPN headshot URLs from the API are real (the API only includes them when a photo exists).
  // Only strip ESPN URLs when they were speculatively constructed (allowEspn=false default for legacy callers).
  if (!allowEspn && isEspnHeadshotUrl(normalized)) return null;
  if (isGeneratedAvatarUrl(normalized)) return null;
  return normalized;
}

function generatedAvatarUrlForPlayer(_name, _playerId = "") {
  // No longer used — removed ui-avatars fallback.
  // Returns null so callers gracefully show initials instead.
  return null;
}

function ensureValidPlayerPhoto(photo, _name, playerId = "") {
  const url = normalizePlayerPhoto(playerId, photo) || null;
  // ESPN headshot URLs that survived the enrichment pipeline were validated in Step 6.
  // Do NOT strip them here — that undoes the validation work.
  return url;
}
...
function areTeamNamesLikelySame(a, b) {
  const aCanonical = resolveCanonicalTeamName(a);
  const bCanonical = resolveCanonicalTeamName(b);
  const aStrict = normalizeTeamKey(aCanonical || a);
  const bStrict = normalizeTeamKey(bCanonical || b);
  const aLoose = normalizeTeamLookupKey(aCanonical || a);
  const bLoose = normalizeTeamLookupKey(bCanonical || b);

  if (!aStrict || !bStrict) return false;
  if (aStrict === bStrict || (aLoose && bLoose && aLoose === bLoose))
    return true;
  if (aStrict.includes(bStrict) || bStrict.includes(aStrict)) return true;
  if (aLoose && bLoose && (aLoose.includes(bLoose) || bLoose.includes(aLoose)))
    return true;

  const strictScore = similarityScore(aStrict, bStrict);
  const looseScore = aLoose && bLoose ? similarityScore(aLoose, bLoose) : 0;
  return strictScore >= 0.84 || looseScore >= 0.9;
}
...
async function collectTeamLineupPhotoIndex(teamName, scheduleEvents) {
  ...
  const canonicalTeam = resolveCanonicalTeamName(teamName);
  const normalizedTeam = normalizeTeamKey(canonicalTeam || teamName);
  ...
  const withSofa = await Promise.race([
    enrichMatchesWithSofaData(
      candidates,
      String(candidates[0]?.startDate || "").slice(0, 10),
    ),
    new Promise((resolve) => setTimeout(() => resolve(candidates), 7000)),
  ]).catch(() => candidates);

  // Extract sofaTeamId directly from the event data (homeTeam.id / awayTeam.id)
  // This is more reliable than waiting for lineups to extract the ID from a logo URL.
  let sofaTeamId = null;
  for (const event of Array.isArray(withSofa) ? withSofa : []) {
    if (!event?.sofaData) continue;
    const homeId = String(event.sofaData.homeTeam?.id || "").trim();
    const homeName = String(event.sofaData.homeTeam?.name || "").trim();
    const awayId = String(event.sofaData.awayTeam?.id || "").trim();
    const awayName = String(event.sofaData.awayTeam?.name || "").trim();
    if (
      homeId &&
      normalizedTeam &&
      areTeamNamesLikelySame(normalizedTeam, homeName)
    ) {
      sofaTeamId = homeId;
      break;
    }
    if (
      awayId &&
      normalizedTeam &&
      areTeamNamesLikelySame(normalizedTeam, awayName)
    ) {
      sofaTeamId = awayId;
      break;
    }
  ...
  // Fetch the full Sofascore squad now — covers every registered player,
  // not just those who appeared in recent lineups.
  if (sofaTeamId) {
    const squadPlayers = await fetchSofaTeamPlayerPhotos(sofaTeamId).catch(
      () => null,
    );
    if (Array.isArray(squadPlayers)) {
      for (const p of squadPlayers) {
        const photo = normalizePlayerPhoto(p.id, p.photo);
        if (!photo) continue;
        const key = normalizePersonName(p.name || "");
        if (p.id && !byId.has(`sofa:${p.id}`)) byId.set(`sofa:${p.id}`, photo);
        if (key && !byName.has(key)) byName.set(key, photo);
      }
    }
  }

  // Also fetch recent match lineups — adds photos for players known only by
  // their Sofascore lineup entry (fallback for any squad photo gaps).
  const sofaIds = [];
  for (const event of Array.isArray(withSofa) ? withSofa : []) {
  ...
  if (sofaIds.length) {
    ...
    for (const entry of lineupsByMatch) {
      ...
      for (const side of entry.value) {
        const sideTeamRaw = String(side?.team || "").trim();
        const sameTeam =
          !normalizedTeam || !sideTeamRaw
            ? true
            : areTeamNamesLikelySame(normalizedTeam, sideTeamRaw);
        if (!sameTeam) continue;

        // Fallback: extract sofaTeamId from lineup logo URL if not yet found
        if (!sofaTeamId && side?.teamLogo) {
          const m = String(side.teamLogo).match(/\/team\/(\d+)\/image/);
          if (m) sofaTeamId = m[1];
        }

        const rows = [
          ...(Array.isArray(side?.players) ? side.players : []),
          ...(Array.isArray(side?.bench) ? side.bench : []),
        ];
...
function mergePlayersWithLineupPhotos(players, lineupIndex) {
  ...
  const byName =
    lineupIndex?.byName instanceof Map ? lineupIndex.byName : new Map();
  ...
  return list.map((player) => {
    ...
    const lineupPhoto =
      (id && byId.get(id)) || (nameKey && byName.get(nameKey)) || null;
    if (!lineupPhoto) return player;
    const proxiedLineupPhoto = proxyPhotoUrl(lineupPhoto);
    if (String(player?.photo || "") === String(proxiedLineupPhoto || ""))
      return player;
    return {
      ...player,
      photo: proxiedLineupPhoto,
      photoSource: "lineup",
    };
  });
}
...
async function validateEspnHeadshot(url) {
  ...
  try {
    ...
    const totalFromLength = parseInt(
      getResp.headers.get("content-length") || "0",
      10,
    );
...
async function zillizInit() {
  ...
  try {
    ...
    const exists =
      Array.isArray(list?.data) && list.data.includes(ZILLIZ_COLLECTION);
    if (!exists) {
        ...
        schema: {
          fields: [
            ...
            },
            {
              ...
              elementTypeParams: { dim: String(EMBEDDING_DIM) },
            },
      ...
      if (res?.code !== 0) {
        console.warn("Zilliz collection create fout:", res?.message);
        return false;
      }
    }
    _zillizReady = true;
...
const PLAYER_ANALYSIS_STORE_PATH = join(
  __dirname,
  "data",
...
function savePlayerAnalysisStore() {
  try {
    ...
    writeFileSync(
      PLAYER_ANALYSIS_STORE_PATH,
      JSON.stringify(playerAnalysisStore, null, 2),
      "utf8",
    );
  } catch {
    // best effort
...
function getStoredPlayerAnalysis(key) {
  ...
  if (!row) return null;
  if (Date.now() - Number(row.updatedAt || 0) > PLAYER_ANALYSIS_MAX_AGE_MS)
    return null;
  return row;
}
...
function similarityScore(a, b) {
  ...
  // Concatenated-adjacent-token matching: "de bruyne" → check if "debruyne" is in other set
  let concatMatches = 0;
  for (const [tokens, otherSet] of [
    [tokensA, setB],
    [tokensB, setA],
  ]) {
    for (let i = 0; i < tokens.length - 1; i++) {
      const merged = tokens[i] + tokens[i + 1];
      if (merged.length >= 4 && otherSet.has(merged)) {
        concatMatches += 1;
        break;
      }
    }
  ...
  const totalMatched =
    overlap + initialMatches * 0.7 + partialMatches + concatMatches * 0.8;
...
async function loadGithubPlayerFallbackEntries() {
  const now = Date.now();
  if (
    _githubPlayerFallbackCache.entries.length &&
    now - _githubPlayerFallbackCache.ts < GITHUB_PLAYER_DATA_TTL_MS
  ) {
  ...
  _githubPlayerFallbackInflight = (async () => {
    try {
      ...
      const personalLines = String(personalCsv || "")
        .replace(/^\uFEFF/, "")
        .split(/\r?\n/)
        .filter(Boolean);
      if (personalLines.length > 1) {
        const header = splitCsvLine(personalLines[0]).map((h) =>
          String(h || "")
            .trim()
            .toLowerCase(),
        );
      ...
      const overviewLines = String(overviewCsv || "")
        .replace(/^\uFEFF/, "")
        .split(/\r?\n/)
        .filter(Boolean);
      if (overviewLines.length > 1) {
        const header = splitCsvLine(overviewLines[0]).map((h) =>
          String(h || "")
            .trim()
            .toLowerCase(),
        );
...
async function fetchGithubPlayerFallback(playerName, teamName) {
  ...
  for (const row of entries) {
    ...
    if (
      !isSafeFuzzyPlayerMatch(
        playerName,
        row?.name || "",
        targetTeam ? 0.72 : 0.82,
      )
    )
      continue;
    ...
    if (targetTeam) {
      ...
      if (candTeam && candTeam === targetTeam) score += 0.32;
        ...
        candTeam &&
        (candTeam.includes(targetTeam) || targetTeam.includes(candTeam))
      )
        score += 0.18;
  ...
  if (!best) return null;
  const minScore = targetTeam ? 0.7 : 0.78;
  ...
  return {
    ...
    marketValue: best.marketValueEur
      ? formatEURShort(best.marketValueEur)
      : null,
...
function pickBestProfileMatch(profiles, player, teamName) {
  ...
  for (const row of profiles) {
    ...
    const profName = normalizePersonName(
      prof?.name || prof?.firstname || prof?.lastname || "",
    );
    const profTeam = normalizePersonName(
      row?.statistics?.[0]?.team?.name || "",
    );

    let score = similarityScore(targetName, profName);
    if (targetTeam && profTeam) {
      if (profTeam === targetTeam) score += 0.35;
      else if (profTeam.includes(targetTeam) || targetTeam.includes(profTeam))
        score += 0.2;
    }
...
function parseMarketValueEUR(value) {
  ...
  if (value && typeof value === "object" && value.value != null)
    return parseMarketValueEUR(value.value);
  const text = String(value ?? "").trim();
...
function normalizeMarketValueInput(value) {
  ...
  if (num && /k|m|b|bn|mln|million|billion|eur|euro/i.test(raw)) {
    if (/b|bn|billion/i.test(raw))
      return formatEURShort(Math.round(num * 1_000_000_000));
    if (/m|mln|million/i.test(raw))
      return formatEURShort(Math.round(num * 1_000_000));
...
function normalizeTransferAction(role, fee, note) {
  ...
  if (
    /end of loan|loan ended|returned|return from loan|einde huur|einde van huur/.test(
      text,
    )
  )
    return "loan_end";
...
function normalizeFormerClubEvents(rows, options = {}) {
  ...
  const mapped = source
    .map((item) => {
      const name = stripHtmlArtifacts(
        item?.name || item?.team || item?.club || item?.to || item?.from,
      );
      if (!name) return null;
      const role =
        String(item?.role || "").toLowerCase() === "to" ? "to" : "from";
      const date = stripHtmlArtifacts(
        item?.date || item?.season || item?.year || "",
      );
      const fee = stripHtmlArtifacts(
        item?.fee || item?.transferFee || item?.value || "",
      );
      const note = stripHtmlArtifacts(
        item?.note ||
          item?.description ||
          item?.details ||
          item?.transferType ||
      ...
      const dedupeKey =
        `${event.name}|${event.role}|${event.date}|${event.fee || ""}|${event.note || ""}`.toLowerCase();
    ...
    .sort((a, b) => {
      if (a.moment !== b.moment)
        return descending ? b.moment - a.moment : a.moment - b.moment;
      const order = {
        joined: 0,
        loan: 1,
        loan_end: 2,
...
function isLikelyFemale(value) {
  ...
  return /female|woman|women|feminine|ladies|vrouw|dames|feminino|femminile|weibl/i.test(
    text,
  );
}
...
function isSoccerContext(value) {
  const text = String(value || "").toLowerCase();
  if (!text) return true;
  return /soccer|football|voetbal|futbol|calcio|bundesliga|liga|premier|uefa|cup|serie a/i.test(
    text,
  );
}
...
function normalizeApifyPlayerItem(raw, requestedName, requestedTeam) {
  ...
  const read = (...keys) => {
    for (const scope of scopes) {
      for (const key of keys) {
        if (scope?.[key] != null && String(scope[key]).trim() !== "")
          return scope[key];
      }
    }
    return undefined;
  ...
  const name = pickString(
    read("name", "playerName", "fullName", "displayName"),
    requestedName,
  );
  const team = pickString(
    read("team", "club", "currentClub", "clubName", "teamName"),
    requestedTeam,
  );
  const marketValue = normalizeMarketValueInput(
    read("marketValue", "market_value", "value", "estimatedValue", "valueEur"),
  );
  const teamLogo = read(
    "teamLogo",
    "clubLogo",
  ...
  const position = pickString(
    read("position", "positionName", "role", "mainPosition"),
  );
  const nationality = pickString(read("nationality", "country", "nation"));
  const gender = pickString(
    read("gender", "sex", "category", "teamGender", "playerGender"),
  );
  ...
  const height = toMetersStringFromAny(
    read("height", "heightCm", "height_cm", "height_m"),
  );
  const weight = toKgStringFromAny(read("weight", "weightKg", "weight_kg"));
  const birthDate = pickString(read("birthDate", "dateOfBirth", "dob"));
  const jerseyNumber = pickString(
    read("jerseyNumber", "shirtNumber", "number", "jersey"),
  );
  const contractUntil = pickString(
    read("contractUntil", "contractEnd", "contractEndDate"),
  );
  const formerClubs = mapFormerClubsFromApify(
    read("formerClubs", "transfers", "transferHistory", "history"),
  );
...
function pickBestApifyCandidate(items, requestedName, requestedTeam) {
  ...
  for (const item of items || []) {
    const candidate = normalizeApifyPlayerItem(
      item,
      requestedName,
      requestedTeam,
    );
    ...
    if (targetName && candName) {
      ...
      const tokenOverlap = nameTokens.filter((token) =>
    ...
    if (targetTeam && candTeam) {
      ...
      else if (candTeam.includes(targetTeam) || targetTeam.includes(candTeam))
        score += 0.2;
      else score -= 0.4;
    }
...
function buildTheSportsDBTeamSearchCandidates(teamName) {
  ...
  while (strippedLeading.length > 1 && leadingTokens.has(strippedLeading[0]))
    strippedLeading.shift();
  let strippedTrailing = [...strippedLeading];
  while (
    strippedTrailing.length > 1 &&
    trailingTokens.has(strippedTrailing[strippedTrailing.length - 1])
  )
    strippedTrailing.pop();

  add(strippedLeading.join(" "));
  add(strippedTrailing.join(" "));
  if (strippedTrailing.length >= 2) add(strippedTrailing.slice(0, 2).join(" "));
  if (strippedTrailing.length >= 1)
    add(strippedTrailing[strippedTrailing.length - 1]);
...
function scoreTheSportsDBTeamMatch(team, requestedName) {
  ...
  // Reject non-soccer teams entirely to avoid cross-sport mismatches
  const sport = String(team?.strSport || "").toLowerCase();
  if (sport && sport !== "soccer") return 0;
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.includes(requestedNorm) || requestedNorm.includes(candidate))
      score += 0.12;
  ...
  if (
    country === "belgium" ||
    league.includes("belgian") ||
    league2.includes("belgian")
  )
    best += 0.03;
  return best;
}
...
async function resolveTheSportsDBTeam(teamName) {
  ...
  const normalizedInput = normalizeTeamKey(canonicalName || teamName);
  ...
  const candidates = buildTheSportsDBTeamSearchCandidates(
  ...
  try {
    for (const candidate of candidates) {
      ...
      const exact = Array.from(teamMap.values()).find(
        (team) =>
          scoreTheSportsDBTeamMatch(team, canonicalName || teamName) >= 1.05,
      );
      if (exact) {
    ...
    for (const team of teamMap.values()) {
      const score = scoreTheSportsDBTeamMatch(team, canonicalName || teamName);
      if (score > bestScore) {
        bestScore = score;
        best = team;
      }
...
async function fetchTheSportsDBTeamPlayers(teamName) {
  if (!teamName) return [];
  const canonicalTeam = resolveCanonicalTeamName(teamName);
  const normTeam = normalizePersonName(canonicalTeam || teamName);
  const cacheKey = `thesportsdb_players_${normTeam}`;
  const cacheItem = __cache.get(cacheKey);
  if (cacheItem && Date.now() <= cacheItem.expiresAt) return cacheItem.value;

  try {
    const team = await resolveTheSportsDBTeam(canonicalTeam || teamName);
    const teamId = String(team?.idTeam || "").trim();
    if (!teamId) {
...
async function resolvePlayerPhotosViaAI(players, teamName, leagueName = "") {
  ...
  if (!hasAnyProvider || !Array.isArray(players) || players.length === 0)
    return new Map();
  ...
  const normalizedLookup = new Map(
    (players || [])
      .map((p) => [normalizePersonName(p?.name || ""), String(p?.name || "")])
      .filter(([k, v]) => k && v),
  );

  const isBelgianLeague =
    /bel\.1|bel\.2|jupiler|challenger|pro league|eerste klasse/i.test(
      String(leagueName || ""),
    );
...
Return ONLY valid JSON. No markdown, no explanation.`;
      ...
      const sys = {
        role: "system",
        content:
          "You are a football/soccer identity resolver. Always resolve ambiguous names to the player from the given team/league context. If uncertain, return null. Output valid JSON only.",
      };
      const user = { role: "user", content: prompt };

      const providers = [];
      if (process.env.OLLAMA_MODEL)
        providers.push(() => ollamaChat([sys, user], { temperature: 0 }));
      if (process.env.DEEPSEEK_API_KEY)
        providers.push(() => deepseekChat([sys, user], { temperature: 0 }));
      if (process.env.OPENROUTER_API_KEY)
        providers.push(() => openrouterChat([sys, user], { temperature: 0 }));
      if (process.env.GROQ_API_KEY)
        providers.push(() => groqChat([sys, user], { temperature: 0 }));
      if (process.env.OPENAI_API_KEY)
        providers.push(() =>
          openaiChat([sys, user], { temperature: 0, model: "gpt-4o-mini" }),
        );
      if (process.env.GEMINI_API_KEY)
        providers.push(() => geminiChat([sys, user], { temperature: 0 }));
  ...
  if (photoMap.size > 0) {
    console.log(
      `[photos][ai] LLM resolved ${photoMap.size} Wikipedia photos for ${teamName || leagueName || "unknown"}`,
    );
  }
  return photoMap;
}
...
async function fetchWikipediaPlayerPhoto(playerName, hintContext = "") {
  ...
  try {
    ...
    if (direct) {
      cacheSet(cacheKey, direct, 86_400_000);
      return direct;
    }
    ...
    for (const variant of footballerVariants) {
      ...
      if (photo) {
        cacheSet(cacheKey, photo, 86_400_000);
        return photo;
      }
    }
...
async function enrichMatchLogos(matches) {
  ...
  for (let i = 0; i < needsTsdb.length; i += 5) {
    ...
    const results = await Promise.all(
      batch.map(async (name) => [name, await fetchTheSportsDBTeamLogo(name)]),
    );
  ...
  const needsWiki = teamNames.filter((name) => {
    const hasExistingLogo = matches.some(
      (m) =>
        (m.homeTeam === name && m.homeTeamLogo) ||
        (m.awayTeam === name && m.awayTeamLogo),
    );
  ...
  if (needsWiki.length > 0) {
    const results = await Promise.all(
      needsWiki.map(async (name) => [name, await fetchWikipediaTeamLogo(name)]),
    );
...
async function enrichScorersPhotos(scorers, leagueName) {
  ...
  console.log(
    `[topscorers][photos] ${leagueName}: ${needPhoto.length}/${preparedScorers.length} scorers need photo enrichment`,
  );
  const photoMap = new Map();

  // --- STEP -1: Team cache — look up Sofascore photos already cached from team-detail routes ---
  // When a user has browsed a team recently, its player list (with Sofascore photos) is in the
  // in-memory getOrFetch cache. Use that to instantly map ESPN player ID → photo without any
  // additional network requests.
  {
    const espnSlug = ESPN_LEAGUE_SLUGS[leagueName] || "";
    const season = seasonForDate(new Date());
    const uniqueTeamIds = [
      ...new Set(needPhoto.map((s) => String(s.teamId || "")).filter(Boolean)),
    ];
    for (const teamId of uniqueTeamIds) {
      const cacheKey = `team_${teamId}_${season}_${espnSlug}`;
      const cached = cacheGet(cacheKey);
      if (!cached || !Array.isArray(cached.players)) continue;
      for (const p of cached.players) {
        const photo = String(p?.photo || "");
        if (!photo || !photo.startsWith("http")) continue;
        // Index by ESPN player ID (most reliable)
        const pid = String(p?.id || "");
        if (pid) photoMap.set(`id:${pid}`, photo);
        // Also index by normalized name as fallback
        const nameKey = normalizePersonName(p?.name || "");
        if (nameKey && !photoMap.has(nameKey)) photoMap.set(nameKey, photo);
      }
    }
    // Apply team-cache photos to scorers that now have a match
    const stillNeedPhoto = [];
    for (const scorer of preparedScorers) {
      if (scorer?.photo) continue; // already has photo
      const pid = String(scorer?.id || "");
      const nameKey = normalizePersonName(scorer?.name || "");
      const found =
        (pid && photoMap.get(`id:${pid}`)) ||
        (nameKey && photoMap.get(nameKey));
      if (found) {
        photoMap.set(scorer.name, found);
      } else {
        stillNeedPhoto.push(scorer);
      }
    }
    if (stillNeedPhoto.length < needPhoto.length) {
      console.log(
        `[topscorers][photos] ${leagueName}: team-cache covered ${needPhoto.length - stillNeedPhoto.length} photos`,
      );
    }
    // Rebuild needPhoto to only contain scorers still without photos
    needPhoto.length = 0;
    needPhoto.push(...stillNeedPhoto);
  }
  ...
  if (needPhoto.length === 0) {
    return preparedScorers.map((s) => {
      ...
      const found =
        photoMap.get(s.name) || photoMap.get(`id:${String(s.id || "")}`);
      return found ? { ...s, photo: found } : s;
    });
  }

  // --- STEP 0: Bulk preprocessing (Transfermarkt lookup tables only) ---
  // TheSportsDB bulk team-roster lookups are intentionally skipped here because
  // they exhaust the TheSportsDB rate limit (429) before the per-player searches
  // in STEP 3 can run. The per-player TheSportsDB search in STEP 3 has much
  // better coverage for recent transfers anyway.
  const teamNames = [...new Set(needPhoto.map((s) => s.team).filter(Boolean))];
  const tmPhotoMap = new Map();
  const dbPhotoMap = new Map(); // kept for API compat with STEP 1 lookup

  // Run TM preprocessing in parallel with a short timeout to avoid blocking.
  // Sequential with 8 teams × 8s = 64s, parallel caps at 4s total.
  await Promise.allSettled(
    teamNames.map(async (teamName) => {
      try {
        const tmPlayers = await Promise.race([
          fetchTransfermarktClubPlayers(teamName),
          new Promise((_, r) =>
            setTimeout(() => r(new Error("TM prep timeout")), 4000),
          ),
        ]);
        ...
        /* TM unavailable for this team — skip silently */
      }
    }),
  );

  console.log(
    `[topscorers][photos] ${leagueName}: Preprocessing found ${tmPhotoMap.size} TM photos`,
  );

  // --- PASS 1: TM preprocessing map + TM direct (all players in parallel, 3s cap) ---
  // This completes in ~3 seconds for all players.
  const tmPassResults = await Promise.all(
    needPhoto.map(async (scorer) => {
      const name = String(scorer.name || "").trim();
      const team = String(scorer.team || "").trim();
      const normName = normalizePersonName(name);

      // STEP 1: preprocessing maps (instant)
      if (normName) {
        const tmPhoto = tmPhotoMap.get(normName);
        if (tmPhoto) return [name, proxyPhotoUrl(tmPhoto)];
        const dbPhoto = dbPhotoMap.get(normName);
        if (dbPhoto) return [name, dbPhoto];
      }

      // STEP 2: TM direct (3s cap — avoids blocking TheSportsDB pass)
      try {
        const tm = await Promise.race([
          fetchTransfermarktPlayerDirect(name, team),
          new Promise((_, r) =>
            setTimeout(() => r(new Error("TM direct timeout")), 3000),
          ),
        ]);
        if (tm?.photo) return [name, proxyPhotoUrl(tm.photo)];
      } catch (err) {
        console.debug(
          `[topscorers][photos] TM direct failed for ${name}:`,
          err.message,
        );
      }
  ...
  for (const [name, photo] of tmPassResults) {
    if (name && photo) photoMap.set(name, photo);
  }

  // Players still needing a photo after TM pass
  const stillNeed = needPhoto.filter(
    (s) => !photoMap.has(String(s.name || "").trim()),
  );

  // --- PASS 2: TheSportsDB + ESPN CDN — sequential with 800ms between requests ---
  // Sequential + 800ms gap avoids TheSportsDB rate limits (free tier = ~15 req/min).
  // Worst case: 30 players × (1s avg response + 0.8s delay) = ~54s — but:
  //   - Many players are found early (loop breaks on success, delay still runs)
  //   - The getOrFetch cache means this only runs once per 15 minutes
  //   - Average response time is <500ms, so 30 players ≈ 39s in practice
  for (const scorer of stillNeed) {
    const name = String(scorer.name || "").trim();
    const team = String(scorer.team || "").trim();
    const normName = normalizePersonName(name);
    ...
    try {
      ...
      if (resp.ok) {
        ...
        for (const r of players) {
          // Only accept soccer players — reject unknown sport too
          const rSport = String(r?.strSport || "").toLowerCase();
          if (rSport !== "soccer") continue;
          ...
          const teamMatch = normTeam && similarityScore(normTeam, rTeam) >= 0.5;
          if (nameScore >= 0.42 || (nameScore >= 0.35 && teamMatch)) {
            console.debug(
              `[topscorers][photos] TheSportsDB found ${name} → ${photo.slice(0, 60)}`,
            );
        ...
        if (!found) {
          console.debug(
            `[topscorers][photos] TheSportsDB no match for ${name}: ${players.length} results`,
          );
        }
      } else {
        console.debug(
          `[topscorers][photos] TheSportsDB ${resp.status} for ${name}`,
        );
      }
    } catch (err) {
      console.debug(
        `[topscorers][photos] TheSportsDB search failed for ${name}:`,
        err.message,
      );
    }

    // Skip ESPN CDN and delay if we already found a photo
    if (photoMap.has(name)) {
      await new Promise((r) => setTimeout(r, 800));
      continue;
    }

    // -- STEP 4: ESPN CDN validation --
    ...
    if (espnId && /^\d+$/.test(espnId)) {
      try {
        ...
        if (validated) photoMap.set(name, validated);
      } catch (err) {
        console.debug(
          `[topscorers][photos] ESPN validation failed for ${name}:`,
          err.message,
        );
      }
    }

    // 800ms between TheSportsDB requests to stay under rate limit (~75 req/min)
    await new Promise((r) => setTimeout(r, 800));
  }
  ...
  console.log(
    `[topscorers][photos] ${leagueName}: Resolved ${filledCount} + ${preFilledCount} prefilled = ${filledCount + preFilledCount}/${preparedScorers.length} scorer photos (${Math.round((100 * (filledCount + preFilledCount)) / (preparedScorers.length || 1))}%)`,
  );
  return preparedScorers.map((s) => {
    if (!s || s.photo) return s;
...
function inferFormationFromPlayers(players) {
  ...
  const outfield = starters.filter(
    (p) =>
      !/gk|goalkeeper/i.test(
        String(p?.position || "") + " " + String(p?.positionName || ""),
      ),
  );
  let def = 0;
  let mid = 0;
  ...
  for (const p of outfield) {
    ...
    if (
      /GK|DEF|CB|LB|RB|SWB|LWB|RWB|BACK/.test(pos) ||
      /defend|back/.test(posName)
    ) {
      def += 1;
      continue;
    }
    ...
    if (
      /FW|ST|CF|LW|RW|ATT/.test(pos) ||
      /forward|striker|wing|attack/.test(posName)
    ) {
      fwd += 1;
      continue;
    }
...
function mapEspnEventToMatch(ev) {
  ...
  const leagueNameRaw =
    ...
    "";
  const leagueName =
    normalizeLeagueName(leagueNameRaw) || ev?._leagueHint || "";
  ...
  return {
    ...
    homeTeamLogo: normalizeTeamLogo(
      ...
      home?.team?.logo || null,
      home?.team?.id
        ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(home.team.id))}.png`
        : null,
    ),
    awayTeamLogo: normalizeTeamLogo(
      ...
      away?.team?.logo || null,
      away?.team?.id
        ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(away.team.id))}.png`
        : null,
    ),
    homeScore,
    awayScore,
...
function buildSofaDataFromEvent(rawEvent) {
  ...
  return {
    ...
    venue: {
      ...
      capacity:
        Number(
          event?.venue?.capacity || event?.venue?.stadium?.capacity || 0,
        ) || null,
    ...
    const mapped = incidents
      .map((inc) => {
        ...
        if (type === "period") {
          ...
          type === "card" &&
          (detail.includes("secondyellow") || detail.includes("second yellow"))
        )
          ...
          if (detail.includes("miss") || detail.includes("saved"))
...
// Fetch all squad players for a Sofascore team — returns array of { id, name, photo }
// where photo is the canonical Sofascore player image URL.
async function fetchSofaTeamPlayerPhotos(sofaTeamId) {
  if (!sofaTeamId) return null;
  const cacheKey = `sofascore_team_players_${sofaTeamId}`;
  const cached = cacheGet(cacheKey);
  if (cached !== null) return cached;
  try {
    const url = `${SOFASCORE_API_BASE}/team/${encodeURIComponent(sofaTeamId)}/players`;
    const resp = await fetch(url, {
      headers: {
        "user-agent": "Mozilla/5.0 (Nexora/1.0)",
        accept: "application/json",
    ...
    const result = rows
      .flatMap(({ player: p }) => {
        if (!p?.id) return [];
        const photo = `https://api.sofascore.app/api/v1/player/${encodeURIComponent(p.id)}/image`;
        const entries = [{ id: String(p.id), name: p?.name || "", photo }];
        // Also index by short name so abbreviated names match ESPN roster names
        if (p?.shortName && p.shortName !== p?.name) {
          entries.push({ id: String(p.id), name: p.shortName, photo });
        }
        return entries;
      })
      .filter((r) => r.id && r.name && r.photo);
    const value = result.length ? result : null;
    cacheSet(cacheKey, value, 30 * 60_000); // 30 min — squad changes rarely
    return value;
  } catch {
    cacheSet(cacheKey, null, 120_000);
    return null;
...
/**
 * Resolve a Sofascore team ID from a display name.
 * Uses Sofascore's search API → cached for 12h.
 * Falls back to scanning cached Sofascore events from recent dates.
 */
async function findSofaTeamIdByName(teamName) {
  if (!teamName) return null;
  const canonical = resolveCanonicalTeamName(teamName) || teamName;
  const normalized = normalizeTeamKey(canonical);
  if (!normalized) return null;

  const cacheKey = `sofascore_team_id_${normalized}`;
  const cached = cacheGet(cacheKey);
  if (cached !== undefined && cached !== null) return cached || null;

  // Strategy 1: Sofascore search API
  try {
    const q = encodeURIComponent(String(canonical).trim().slice(0, 80));
    const url = `${SOFASCORE_API_BASE}/search/teams/${q}/page/1`;
    const resp = await fetch(url, {
      headers: {
        "user-agent":
    ...
    if (resp.ok) {
      ...
      for (const t of teams) {
        const tName = normalizeTeamKey(
          resolveCanonicalTeamName(t?.name || "") || t?.name || "",
        );
        const tShort = normalizeTeamKey(t?.shortName || "");
        const score = Math.max(
          similarityScore(normalized, tName),
          tShort ? similarityScore(normalized, tShort) : 0,
        );
        if (score > bestScore && score >= 0.6 && t?.id) {
          bestScore = score;
          bestId = String(t.id);
        }
      }
      if (bestId) {
        cacheSet(cacheKey, bestId, 12 * 60 * 60_000); // 12h
        console.log(
          `[photos][sofa] Team search: "${teamName}" → sofa id ${bestId} (score=${bestScore.toFixed(2)})`,
        );
        return bestId;
      }
    }
  } catch {
    /* search failed — try event scan */
  }

  // Strategy 2: Scan cached Sofascore events from recent dates
  ...
  for (const date of dates) {
    const events = cacheGet(`sofascore_events_${date}`);
    ...
    for (const raw of events) {
      ...
      for (const t of [home, away]) {
        if (!t?.id) continue;
        const tKey = normalizeTeamKey(
          resolveCanonicalTeamName(t?.name || "") || t?.name || "",
        );
        if (
          tKey &&
          (tKey === normalized || similarityScore(tKey, normalized) >= 0.8)
        ) {
          const id = String(t.id);
          cacheSet(cacheKey, id, 12 * 60 * 60_000);
          return id;
        }
      }
    }
  ...
  // Not found — cache negative result (2h so we don't spam)
  cacheSet(cacheKey, "", 2 * 60 * 60_000);
  return null;
}
...
async function enrichMatchesWithSofaData(matches, dateHint = "") {
  ...
  for (const d of candidateDates) {
    ...
    for (const value of values) {
      if (!expandedDates.includes(value)) expandedDates.push(value);
    }
  }

  const dates = expandedDates.slice(0, 6);
  ...
  const batches = await Promise.allSettled(
    dates.map((d) => fetchSofaEventsByDate(d)),
  );
  for (const batch of batches) {
    ...
    for (const raw of batch.value || []) {
      ...
      const hk = normalizeTeamKey(
        resolveCanonicalTeamName(sofa.homeTeam?.name || ""),
      );
      const ak = normalizeTeamKey(
        resolveCanonicalTeamName(sofa.awayTeam?.name || ""),
      );
      if (!hk || !ak) continue;
  ...
  return list.map((match) => {
    ...
    const hk = normalizeTeamKey(resolveCanonicalTeamName(homeName));
    const ak = normalizeTeamKey(resolveCanonicalTeamName(awayName));
    ...
    if (!sofa && hk && ak) {
      for (const candidate of allSofaEvents) {
        const shName = String(candidate.homeTeam?.name || "").trim();
        const saName = String(candidate.awayTeam?.name || "").trim();
        const sh = normalizeTeamKey(resolveCanonicalTeamName(shName));
        const sa = normalizeTeamKey(resolveCanonicalTeamName(saName));
        if (!sh || !sa) continue;
        const homeMatch = areTeamNamesLikelySame(homeName, shName);
        const awayMatch = areTeamNamesLikelySame(awayName, saName);
        const homeMatchReversed = areTeamNamesLikelySame(awayName, shName);
        const awayMatchReversed = areTeamNamesLikelySame(homeName, saName);
        if (
          (homeMatch && awayMatch) ||
...
function leagueValueMultiplier(leagueName) {
  ...
  if (n.includes("jupiler") || n.includes("bel.1") || n.includes("pro league"))
    return 0.45;
  if (
...
function estimateMarketValueEUR(player, leagueName) {
  ...
  const pos = String(
    player?.position?.abbreviation || player?.position || "",
  ).toUpperCase();
  const baseByPos = pos.includes("GK")
    ? 5_000_000
...
function tryParseJSON(text) {
  ...
  const fenced =
    String(text || "").match(/```json\s*([\s\S]*?)```/i) ||
...
async function aiEstimateRosterValues(players, teamName, leagueName) {
  ...
  const isBelgian = /jupiler|bel\.1|pro.?league|eerste.*klasse/i.test(
    leagueName || "",
  );
  ...
  try {
    const sys = {
      role: "system",
      content:
        "Je bent een expert voetbaltransfer analist met diepgaande kennis van Transfermarkt.com waarden. Je geeft altijd realistische waarden gebaseerd op je trainingsdata. Output uitsluitend geldige JSON.",
    };
    const user = { role: "user", content: prompt };
    const providers = [];
    if (process.env.OLLAMA_MODEL)
      providers.push(() => ollamaChat([sys, user], { temperature: 0.2 }));
    if (process.env.DEEPSEEK_API_KEY)
      providers.push(() => deepseekChat([sys, user], { temperature: 0.2 }));
    if (process.env.OPENROUTER_API_KEY)
      providers.push(() => openrouterChat([sys, user], { temperature: 0.2 }));
    if (process.env.GROQ_API_KEY)
      providers.push(() => groqChat([sys, user], { temperature: 0.2 }));
    if (process.env.OPENAI_API_KEY)
      providers.push(() =>
        openaiChat([sys, user], { temperature: 0.2, model: "gpt-4o-mini" }),
      );
    if (process.env.GEMINI_API_KEY)
      providers.push(() => geminiChat([sys, user], { temperature: 0.2 }));
...
async function fetchTransfermarktPlayerDirect(playerName, teamName) {
  ...
  try {
    ...
    const searchResp = await fetch(
      `https://transfermarkt-api-sigma.vercel.app/players/search/${q}`,
      {
        headers: {
          "user-agent": "Mozilla/5.0 (Nexora/1.0)",
          accept: "application/json",
    ...
    for (const r of results.slice(0, 12)) {
      ...
      if (normTeam && rClub && similarityScore(normTeam, rClub) >= 0.5)
        score += 0.3;
      if (score > bestScore) {
        bestScore = score;
        best = r;
    ...
    const marketValueEur = parseMarketValueEUR(
      best?.marketValue?.value ?? best?.marketValue,
    );
    const photo = proxyPhotoUrl(
      String(best?.image || best?.photo || "").trim() || null,
    );
    ...
    try {
      const trResp = await fetch(
        `https://transfermarkt-api-sigma.vercel.app/players/${encodeURIComponent(playerId)}/transfers`,
        {
          headers: {
            "user-agent": "Mozilla/5.0 (Nexora/1.0)",
            accept: "application/json",
    ...
    const result = {
      marketValueEur:
        Number.isFinite(marketValueEur) && marketValueEur > 0
          ? marketValueEur
          : null,
      photo: photo || null,
      transfers,
...
async function fetchTransfermarktClubPlayers(teamName) {
  if (!teamName) return null;
  const canonicalTeam = resolveCanonicalTeamName(teamName);
  const normKey = normalizePersonName(canonicalTeam || teamName);
  ...
  try {
    ...
    const sourceName = canonicalTeam || String(teamName).trim();
    const searchName = TM_CLUB_NAME_MAP[sourceName] || sourceName;
    const q = encodeURIComponent(searchName);
    const searchResp = await fetch(
      `https://transfermarkt-api-sigma.vercel.app/clubs/search/${q}`,
      {
        headers: {
          "user-agent": "Mozilla/5.0 (Nexora/1.0)",
          accept: "application/json",
    ...
    if (clubs.length === 0 && TM_CLUB_NAME_MAP[sourceName]) {
      const q2 = encodeURIComponent(sourceName);
      const resp2 = await fetch(
        `https://transfermarkt-api-sigma.vercel.app/clubs/search/${q2}`,
        {
          headers: {
            "user-agent": "Mozilla/5.0 (Nexora/1.0)",
            accept: "application/json",
    ...
    const normInput = normalizePersonName(sourceName);
    let club = clubs[0];
    let bestScore = 0;
    ...
    const playersResp = await fetch(
      `https://transfermarkt-api-sigma.vercel.app/clubs/${encodeURIComponent(club.id)}/players`,
      {
        headers: {
          "user-agent": "Mozilla/5.0 (Nexora/1.0)",
          accept: "application/json",
    ...
    const result = players
      .map((p) => ({
        ...
        marketValueEur: parseMarketValueEUR(
          p?.marketValue?.value ?? p?.marketValue,
        ),
        photo: proxyPhotoUrl(
          String(p?.image || p?.photo || p?.picture || "").trim() || null,
        ),
      }))
      .filter((p) => p.name);
...
async function enrichRosterMarketValues(players, teamName, _leagueName) {
  ...
  console.log(
    `[market] ${teamName}: Transfermarkt found ${tmValueMap.size}/${players.length} values`,
  );
...
function mapTheSportsDBPosition(pos) {
  const p = String(pos || "")
    .trim()
    .toLowerCase();
  if (/goalkeeper|keeper/i.test(p)) return "G";
  if (/centre.?back|center.?back|defender/i.test(p)) return "D";
  if (/left.?back/i.test(p)) return "D";
  if (/right.?back/i.test(p)) return "D";
  if (/midfield|central.?mid/i.test(p)) return "M";
  if (/attacking.?mid|playmaker/i.test(p)) return "M";
  if (/defensive.?mid/i.test(p)) return "M";
  if (/forward|striker|winger/i.test(p)) return "F";
  return "";
}

function parseTheSportsDBHeight(h) {
  const raw = String(h || "").trim();
  const mMatch = raw.match(/([\d.]+)\s*m/i);
  if (mMatch) return `${mMatch[1]} m`;
  const cmMatch = raw.match(/(\d{3})\s*cm/i);
  if (cmMatch) return `${(Number(cmMatch[1]) / 100).toFixed(2)} m`;
  return null;
}

function parseTheSportsDBWeight(w) {
...
// Phase 1 — Fast bulk photo enrichment (TM team roster + TheSportsDB team lookup)
// This completes in ~3-5s because both APIs are fetched in parallel.
async function enrichRosterPhotosBulk(players, teamName, _leagueName = "") {
  if (!Array.isArray(players) || players.length === 0) return players || [];

  // Build Sofascore + Transfermarkt + TheSportsDB photo maps in parallel
  const [sofaPlayers, tmPlayers, dbPlayers] = await Promise.all([
    teamName
      ? findSofaTeamIdByName(teamName).then((id) =>
          id ? fetchSofaTeamPlayerPhotos(id) : null,
        )
  ...
  // ------ Sofascore photo lookup tables (highest coverage ~85-95%) ------
  const sofaPhotoMap = new Map();
  const sofaPhotoEntries = [];
  if (Array.isArray(sofaPlayers)) {
    for (const p of sofaPlayers) {
      if (p.name && p.photo) {
        const normed = normalizePersonName(p.name);
        if (normed) {
          sofaPhotoMap.set(normed, p.photo);
          sofaPhotoEntries.push({ normed, photo: p.photo });
        }
      }
    }
  }
  ...
  const enriched = players.map((player) => {
    ...
    // Keep ESPN API-provided photos (they came from the real API, not speculatively constructed).
    let bestPhoto = keepIncomingPlayerPhoto(player.photo, {
      allowEspn: Boolean(player._espnApiPhoto),
    });
    let sportsDbPhoto = null;

    // --- Sofascore: exact then fuzzy (highest coverage source) ---
    if (!bestPhoto) {
      const sofaExact = sofaPhotoMap.get(normName);
      if (sofaExact) {
        bestPhoto = sofaExact;
      } else {
        let bestSofaScore = 0;
        let bestSofaPhoto = null;
        for (const entry of sofaPhotoEntries) {
          const score = similarityScore(normName, entry.normed);
          const safeMatch = isSafeFuzzyPlayerMatch(
            normName,
            entry.normed,
            0.65,
          );
          if (!safeMatch && score < 0.85) continue;
          if (score > bestSofaScore) {
            bestSofaScore = score;
            bestSofaPhoto = entry.photo;
          }
        }
        if (bestSofaScore >= 0.65 && bestSofaPhoto) bestPhoto = bestSofaPhoto;
      }
    }
    ...
    // ── Fill gaps from TheSportsDB info (nationality, position, height, weight) ──
    const dbInfo =
      dbInfoMap.get(normName) ||
      (() => {
        let best = null,
          bestS = 0;
        for (const [k, v] of dbInfoMap) {
          const s = similarityScore(normName, k);
          if (s > bestS && s >= 0.7) {
            bestS = s;
            best = v;
          }
    ...
    return {
      ...player,
      nationality: player.nationality || dbInfo?.nationality || undefined,
      position:
        player.position ||
        (dbInfo?.position
          ? mapTheSportsDBPosition(dbInfo.position)
          : undefined),
      height:
        player.height ||
        (dbInfo?.height ? parseTheSportsDBHeight(dbInfo.height) : undefined),
      weight:
        player.weight ||
        (dbInfo?.weight ? parseTheSportsDBWeight(dbInfo.weight) : undefined),
      photo: bestPhoto,
  ...
  console.log(
    `[photos][bulk] ${teamName}: ${withPhoto}/${total} players have photos after bulk enrichment (sofa=${sofaPhotoEntries.length} tm=${tmPhotoEntries.length} db=${dbPhotoEntries.length} candidates)`,
  );

  return enriched;
}

// Phase 2 — Slow individual photo searches (TheSportsDB search + TM direct + ESPN validation)
// This can take 30-60s for large squads. Called separately with its own timeout.
async function enrichRosterPhotosIndividual(
  players,
  teamName,
  _leagueName = "",
) {
  ...
  let enriched = [...players];

  // ------ Steps 3+4: TheSportsDB search + TM direct (sequential TheSportsDB to avoid 429) ------
  const stillNeed = enriched.filter(
    (p) => p && !p.photo && p.name && p.name !== "Onbekend",
  );
  if (stillNeed.length > 0 && stillNeed.length <= 200) {
    const combinedMap = new Map();
    for (const player of stillNeed) {
      const normName = normalizePersonName(player.name || "");
      const normTeam = normalizePersonName(teamName || "");

      // Try TheSportsDB search (full name only — skip last/first name variants to reduce calls)
      try {
        ...
        if (q) {
          ...
          if (resp.ok) {
            ...
            for (const r of data?.player || []) {
              ...
              const safeNameMatch = isSafeFuzzyPlayerMatch(
                normName,
                rName,
                0.7,
              );
              ...
              const teamMatch =
                normTeam &&
                (similarityScore(normTeam, rTeam) >= 0.5 ||
                  similarityScore(normTeam, rTeam2) >= 0.5);
            ...
            if (bestResult) {
              combinedMap.set(player.name, bestResult);
      ...
      // Try Transfermarkt direct player search (3s cap)
      try {
        const tmResult = await Promise.race([
          fetchTransfermarktPlayerDirect(player.name, teamName),
          new Promise((resolve) => setTimeout(() => resolve(null), 3000)),
        ]);
        if (tmResult?.photo) {
          combinedMap.set(player.name, tmResult.photo);
          await new Promise((r) => setTimeout(r, 300));
          continue;
        }
      } catch {
      ...
      // 300ms delay between TheSportsDB requests to stay under rate limit
      await new Promise((r) => setTimeout(r, 300));
    }
  ...
  // Step 5 (AI/Wikipedia) intentionally removed — produces false positives for uncommon names.
  // Sofascore squad photos are merged after this function via mergePlayersWithLineupPhotos.

  // ------ Step 6: ESPN CDN headshot fallback (validated — skip black placeholders) ------
  const espnCandidates = enriched.filter(
    (p) => p && !p.photo && /^\d+$/.test(String(p.id || "").trim()),
  );
  if (espnCandidates.length > 0) {
    ...
    console.log(
      `[photos][espn] ${teamName}: ${espnMap.size}/${espnCandidates.length} ESPN headshots validated`,
    );
  }

  // Step 7 (ui-avatars) intentionally removed — initials rendered by PlayerPhoto component.

  const withPhoto = enriched.filter((p) => p && p.photo).length;
  const total = enriched.filter((p) => p).length;
  console.log(
    `[photos][individual] ${teamName}: ${withPhoto}/${total} players have photos after individual enrichment`,
  );

  return enriched;
}

// Combined wrapper that calls both phases. Kept for any callers outside the team endpoint.
async function enrichRosterPhotos(players, teamName, leagueName = "") {
  const bulk = await enrichRosterPhotosBulk(players, teamName, leagueName);
  return enrichRosterPhotosIndividual(bulk, teamName, leagueName);
}
...
async function aiAnalyzePlayerProfile(player, context = {}) {
  ...
  const sys = {
    role: "system",
    content:
      "Je bent een elite voetbalscout met diepgaande kennis van profvoetballers wereldwijd. Je gebruikt je volledige trainingskennis over bekende spelers. Antwoord strikt in geldig JSON.",
  };
  ...
  if (process.env.XAI_API_KEY)
    providers.push(() => xaiChat([sys, user], { temperature: 0.25 }));
  if (process.env.OLLAMA_MODEL)
    providers.push(() => ollamaChat([sys, user], { temperature: 0.25 }));
  if (process.env.DEEPSEEK_API_KEY)
    providers.push(() => deepseekChat([sys, user], { temperature: 0.25 }));
  if (process.env.OPENROUTER_API_KEY)
    providers.push(() => openrouterChat([sys, user], { temperature: 0.25 }));
  if (process.env.GROQ_API_KEY)
    providers.push(() => groqChat([sys, user], { temperature: 0.25 }));
  if (process.env.OPENAI_API_KEY)
    providers.push(() => openaiChat([sys, user], { temperature: 0.25 }));
  if (process.env.GEMINI_API_KEY)
    providers.push(() => geminiChat([sys, user], { temperature: 0.25 }));
  ...
  for (const run of providers) {
    try {
      ...
      const weaknesses = Array.isArray(parsed?.weaknesses)
        ? parsed.weaknesses.map((x) => String(x).trim()).filter(Boolean)
        : [];
...
function mapFormerClubs(transfers) {
  ...
  for (const t of transfers || []) {
    const outClub = stripHtmlArtifacts(
      t?.teams?.out?.name || t?.from || t?.fromClub || "",
    );
    const inClub = stripHtmlArtifacts(
      t?.teams?.in?.name || t?.to || t?.toClub || "",
    );
    const date = stripHtmlArtifacts(t?.date || t?.season || "");
    const fee = stripHtmlArtifacts(t?.fee || t?.transferFee || t?.value || "");
    const note = stripHtmlArtifacts(
      t?.note || t?.description || t?.details || "",
    );
...
function mapEspnRosterPlayer(player) {
  ...
  // Use ESPN API-provided headshot URL directly — the API only returns it when a real photo exists.
  // Do NOT construct ESPN CDN URLs from the player ID (those often 404).
  const apiHeadshot =
    normalizePlayerPhoto(playerId, player?.headshot?.href) || null;
  return {
    id: playerId,
    ...
    isRealValue: false,
    photo: apiHeadshot,
    _espnApiPhoto: Boolean(apiHeadshot && isEspnHeadshotUrl(apiHeadshot)),
  };
}
...
function buildScoreboardDateCandidates(
  date,
  lookaheadDays = ESPN_LOOKAHEAD_DAYS,
) {
...
async function footballApi(pathAndQuery, _sportsrcParams = null) {
  ...
  try {
    ...
    console.warn(
      `[apifootball] fetch failed for ${pathAndQuery}: ${err?.message}`,
    );
    return { response: [] };
  }
}
...
function normalizeLeagueName(name) {
  ...
  if (/champions.?league/i.test(n) || n === "UCL" || n === "UEFA CL")
    return "UEFA Champions League";
  // Europa League variants
  if (/europa.?league/i.test(n) || /uefa.?europa/i.test(n))
    return "UEFA Europa League";
  // Conference League variants
  if (
    /conference.?league/i.test(n) ||
    /uefa.?conference/i.test(n) ||
    /europa.?conference/i.test(n)
  )
  ...
  if (
    /challenger/i.test(n) ||
    /bel\.?2/i.test(n) ||
    /belgian.?second/i.test(n) ||
    /tweede.*klasse/i.test(n)
  )
    return "Challenger Pro League";
  // Belgium first division
  if (/jupiler|bel\.?1|pro.?league|belgian.?first|eerste.*klasse/i.test(n))
    return "Jupiler Pro League";
  // Belgian Cup
  if (/belgian.?cup|beker.*belgi|bel\.?cup/i.test(n)) return "Belgian Cup";
  // Premier League
  if (/premier.?league|epl|english.?premier|^eng\.?1$/i.test(n))
    return "Premier League";
  // La Liga
  if (/la.?liga|laliga|primera.?division|spain.*1|^esp\.?1$/i.test(n))
    return "La Liga";
  // Bundesliga
  if (/bundesliga|german.*1|1.*bundesliga|^ger\.?1$/i.test(n))
    return "Bundesliga";
  // Ligue 1
  if (/ligue.?1|french.?1|ligue1|^fra\.?1$/i.test(n)) return "Ligue 1";
  // Serie A
  if (/serie.?a|italian.?1|serie.*italiana|^ita\.?1$/i.test(n))
    return "Serie A";
  ...
  if (/scottish premiership|premiership|sco\.?1/i.test(n))
  ...
  if (/swiss challenge league|sui\.?2/i.test(n))
    return "Swiss Challenge League";
  ...
  if (/czech first league|chance liga|cze\.?1/i.test(n))
  ...
  if (/romanian liga 1|rou\.?1/i.test(n)) return "Romanian Liga 1";
  if (/superliga/i.test(n) && /roman/i.test(n)) return "Romanian Liga 1";
...
function broadcasterUrlFromName(name) {
  ...
  const foundKey = Object.keys(OFFICIAL_BROADCASTER_URLS).find((key) =>
    normalized.includes(key),
  );
  if (foundKey) return OFFICIAL_BROADCASTER_URLS[foundKey];
  return (
    "https://www.google.com/search?q=" +
    encodeURIComponent(`${String(name)} live stream official`)
  );
}
...
const STREAM_SERVERS = [{ id: "auto", name: "AUTO", quality: "BEST" }];

// ── Streamed.pk integration ──────────────────────────────────────────
const STREAMED_API = "https://streamed.pk/api";
const STREAMED_WATCH = "https://streamed.pk/watch";
let _streamedMatchCache = { ts: 0, entries: [] };
const STREAMED_CACHE_TTL = 3 * 60 * 1000; // 3 min
// Preferred source order (most reliable first)
const STREAMED_SOURCE_PREF = [
  "delta",
  "echo",
  "alpha",
  "bravo",
...
async function _fetchStreamedMatches() {
  if (
    Date.now() - _streamedMatchCache.ts < STREAMED_CACHE_TTL &&
    _streamedMatchCache.entries.length > 0
  ) {
    return _streamedMatchCache.entries;
  }
  try {
    const resp = await fetch(`${STREAMED_API}/matches/football`, {
      headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0" },
      signal: AbortSignal.timeout(8000),
    });
    if (!resp.ok) return _streamedMatchCache.entries;
    const data = await resp.json();
    if (Array.isArray(data) && data.length > 0) {
      _streamedMatchCache = { ts: Date.now(), entries: data };
      serverLog.info("Refreshed streamed.pk match cache", {
        count: data.length,
      });
    }
    return _streamedMatchCache.entries;
...
function _matchStreamedTeam(apiName, queryName) {
  const a = _normalizeTeam(apiName);
  const b = _normalizeTeam(queryName);
  if (!a || !b) return false;
  return a.includes(b) || b.includes(a);
}

async function resolveStreamedPkUrl(homeTeam, awayTeam) {
  const matches = await _fetchStreamedMatches();
  // Find the match by team name fuzzy match
  const match = matches.find((m) => {
    const h = m.teams?.home?.name;
    const a = m.teams?.away?.name;
    if (!h || !a) return false;
    return (
      (_matchStreamedTeam(h, homeTeam) && _matchStreamedTeam(a, awayTeam)) ||
      (_matchStreamedTeam(h, awayTeam) && _matchStreamedTeam(a, homeTeam))
    );
  });
  if (!match || !match.sources || match.sources.length === 0) return null;

  // Build watch page URL (works directly in browser)
  const watchUrl = `${STREAMED_WATCH}/${match.id}`;

  // Try to get embed URL from best available source
  const sortedSources = [...match.sources].sort((a, b) => {
    const ai = STREAMED_SOURCE_PREF.indexOf(a.source);
    const bi = STREAMED_SOURCE_PREF.indexOf(b.source);
  ...
  for (const src of sortedSources) {
    try {
      const resp = await fetch(
        `${STREAMED_API}/stream/${src.source}/${src.id}`,
        {
          headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0" },
          signal: AbortSignal.timeout(6000),
        },
      ...
      // Pick first HD stream, or first available
      const best =
        streams.find((s) => s.hd && s.embedUrl) ||
        streams.find((s) => s.embedUrl);
      if (best?.embedUrl) {
        // Try to extract the ad-free pooembed.eu direct player URL
        const directStreamUrl = await _resolvePooembed(best.embedUrl);
        return {
          url: directStreamUrl || watchUrl,
          embedUrl: best.embedUrl,
          directStreamUrl: directStreamUrl || null,
          source: "streamed.pk",
          matchId: match.id,
          streamSource: src.source,
  ...
  // No embed found but match exists — return the watch page
  return {
    url: watchUrl,
    embedUrl: null,
    source: "streamed.pk",
...
// ── Pooembed.eu: extract ad-free direct stream URL from embed pages ──
async function _resolvePooembed(embedUrl) {
  ...
  try {
    ...
    const m = html.match(/iframe\s+src="(https:\/\/pooembed\.eu\/[^"]+)"/);
    return m ? m[1] : null;
  } catch {
    return null;
  }
...
function buildStreamUrl(matchId, serverId, homeTeam, awayTeam) {
  const home = slugifyTeam(homeTeam);
  const away = slugifyTeam(awayTeam);
  if (!home || !away) return "https://streamed.pk/category/football";
  return `https://streamed.pk/category/football`;
}
...
function mapFixtureToMatch(fix) {
  ...
  return {
    ...
    homeTeamLogo: normalizeTeamLogo(
      fix?.teams?.home?.name,
      fix?.teams?.home?.logo || null,
    ),
    awayTeamLogo: normalizeTeamLogo(
      fix?.teams?.away?.name,
      fix?.teams?.away?.logo || null,
    ),
...
function mapSportSrcToMatch(m) {
  ...
  else if (ln.includes("champions") && ln.includes("uefa"))
    leagueName = "UEFA Champions League";
  else if (ln.includes("la liga") || ln.includes("laliga"))
    leagueName = "La Liga";
  else if (ln.includes("bundesliga")) leagueName = "Bundesliga";
  else if (
    ln.includes("jupiler") ||
    ln.includes("pro league") ||
    ln.includes("belg")
  )
    leagueName = "Jupiler Pro League";
  else if (ln.includes("ligue 1") || ln.includes("ligue1"))
    leagueName = "Ligue 1";
  else if (ln.includes("serie a") || ln.includes("seriea"))
    leagueName = "Serie A";

  const st = String(m?.status || m?.status_name || "").toLowerCase();
  const status =
    st.includes("inprogress") || st.includes("live")
      ? "live"
  ...
  const homeTeamLogo = normalizeTeamLogo(
    homeTeam,
    m?.home_team?.logo || m?.homeTeam?.logo || m?.home_logo || null,
  );
  const awayTeamLogo = normalizeTeamLogo(
    awayTeam,
    m?.away_team?.logo || m?.awayTeam?.logo || m?.away_logo || null,
  );
  ...
  return {
    ...
    servers: STREAM_SERVERS.map((s) => ({
      ...
      name: s.name,
      quality: s.quality,
      url: buildStreamUrl(matchId, s.id, homeTeam, awayTeam),
    })),
    sport: "football",
    heroGradient: HERO_GRADIENTS[leagueName] || ["#101828", "#0b1220"],
  };
...
function normalizeOutcome(value) {
  ...
  if (v === "home" || v === "home win" || v === "home_win" || v === "1")
    return "Home Win";
  if (v === "away" || v === "away win" || v === "away_win" || v === "2")
    return "Away Win";
...
function deterministicPrediction(payload) {
  ...
  const isInternational =
    Boolean(context?.isInternational) ||
    /fifa|nations|international|friendly|world cup|euro/i.test(
      String(payload?.league || context?.competitionContext || ""),
    );
  ...
  const attackingHome =
    homeShots * 0.95 + homeSot * 1.9 + homeCorners * 0.5 + homePoss * 0.07;
  const attackingAway =
    awayShots * 0.95 + awaySot * 1.9 + awayCorners * 0.5 + awayPoss * 0.07;
  const attackingEdge = (attackingHome - attackingAway) * 0.35;
  const rankEdge =
    awayRank > 0 && homeRank > 0 ? (awayRank - homeRank) * 1.1 : 0;
  ...
  const fifaRankEdge =
    awayFifaRank > 0 && homeFifaRank > 0
      ? (awayFifaRank - homeFifaRank) * 0.95
      : 0;
  const marketValueEdge =
    homeMarketValue > 0 && awayMarketValue > 0
      ? clamp(
          ((homeMarketValue - awayMarketValue) /
            Math.max(homeMarketValue, awayMarketValue)) *
            22,
          -14,
          14,
        )
      : 0;
  const lineupCertaintyEdge =
    homeLineupCertainty > 0 || awayLineupCertainty > 0
  ...
  const homeName = String(payload?.homeTeam || "Home").trim() || "Home";
  const awayName = String(payload?.awayTeam || "Away").trim() || "Away";
  let eventEdge = 0;
  for (const event of events.slice(0, 40)) {
    const joined =
      `${event?.type || ""} ${event?.detail || ""} ${event?.text || ""}`.toLowerCase();
    const isCard = /red|second yellow|penalty miss|own goal/.test(joined);
    const isPositive = /goal|penalty scored|big chance|shot on target/.test(
      joined,
    );
  ...
  const xgHome = hasXgInputs
    ? Number(
        Math.max(
          0,
          homeShots * 0.075 + homeSot * 0.24 + homeCorners * 0.018,
        ).toFixed(2),
      )
    : null;
  const xgAway = hasXgInputs
    ? Number(
        Math.max(
          0,
          awayShots * 0.075 + awaySot * 0.24 + awayCorners * 0.018,
        ).toFixed(2),
      )
    : null;
  ...
  const predictedHome =
    xgHome == null
      ? homeScore
      : Math.max(homeScore, xgToGoals(xgHome) + (minute > 70 ? 0 : 0));
  const predictedAway =
    xgAway == null
      ? awayScore
      : Math.max(awayScore, xgToGoals(xgAway) + (minute > 70 ? 0 : 0));

  const keyFactors = [];
  if (homePoss || awayPoss)
    keyFactors.push(`Possession ${homePoss || 0}% - ${awayPoss || 0}%`);
  if (homeShots || awayShots)
    keyFactors.push(`Shots ${homeShots || 0} - ${awayShots || 0}`);
  if (homeSot || awaySot)
    keyFactors.push(`On target ${homeSot || 0} - ${awaySot || 0}`);
  if (homeRed > 0 || awayRed > 0)
    keyFactors.push(
      `Red cards: ${homeName} ${homeRed} - ${awayName} ${awayRed}`,
    );
  else if (homeCorners || awayCorners)
    keyFactors.push(`Corners ${homeCorners || 0} - ${awayCorners || 0}`);
  if (homeRank > 0 && awayRank > 0)
    keyFactors.push(
      `${homeName} #${homeRank} vs ${awayName} #${awayRank} in league table`,
    );
  if (homeFifaRank > 0 && awayFifaRank > 0)
    keyFactors.push(`FIFA ranking: #${homeFifaRank} vs #${awayFifaRank}`);
  if (homeMarketValue > 0 && awayMarketValue > 0)
    keyFactors.push(
      `Squad value: ${(homeMarketValue / 1_000_000).toFixed(0)}M vs ${(awayMarketValue / 1_000_000).toFixed(0)}M`,
    );
  if (homeLineupCertainty > 0 || awayLineupCertainty > 0)
    keyFactors.push(
      `Lineup certainty: ${Math.round((homeLineupCertainty || 0) * 100)}% vs ${Math.round((awayLineupCertainty || 0) * 100)}%`,
    );
  if (minute) keyFactors.push(`Match minute: ${minute}'`);

  const tacticalNotes = [];
  if (homeRed > 0)
    tacticalNotes.push(
      `${homeName} are down to ${Math.max(7, 11 - homeRed)} men after a red card.`,
    );
  if (awayRed > 0)
    tacticalNotes.push(
      `${awayName} are down to ${Math.max(7, 11 - awayRed)} men after a red card.`,
    );
  if (Math.abs(homeSot - awaySot) >= 2) {
    tacticalNotes.push(
      homeSot > awaySot
        ? `${homeName} are creating the better chances (${homeSot} shots on target vs ${awaySot}).`
        : `${awayName} are creating the better chances (${awaySot} shots on target vs ${homeSot}).`,
    );
  }
  if (Math.abs(homePoss - awayPoss) >= 10) {
    tacticalNotes.push(
      homePoss > awayPoss
        ? `${homeName} controlling tempo with ${homePoss}% possession.`
        : `${awayName} controlling tempo with ${awayPoss}% possession.`,
    );
  }
  if (homeRank > 0 && awayRank > 0 && Math.abs(homeRank - awayRank) >= 4) {
    tacticalNotes.push(
      homeRank < awayRank
        ? `${homeName} hold a structural edge based on league position (#${homeRank} vs #${awayRank}).`
        : `${awayName} hold a structural edge based on league position (#${awayRank} vs #${homeRank}).`,
    );
  }
  if (homeGkSaves > 0 || awayGkSaves > 0) {
    if (homeGkSaves >= 3)
      tacticalNotes.push(
        `${homeName}'s keeper is having an excellent game (${homeGkSaves} saves).`,
      );
    if (awayGkSaves >= 3)
      tacticalNotes.push(
        `${awayName}'s keeper is having an excellent game (${awayGkSaves} saves).`,
      );
  }
  if (homeFouls + awayFouls > 0 && Math.abs(homeFouls - awayFouls) >= 3) {
    tacticalNotes.push(
      homeFouls > awayFouls
        ? `${homeName} committing more fouls (${homeFouls}), risk of further cards.`
        : `${awayName} committing more fouls (${awayFouls}), risk of further cards.`,
    );
  }
  if (tacticalNotes.length === 0) {
    tacticalNotes.push(
      isInternational
        ? `International match with higher variance — individual quality and transitions carry more weight.`
        : `Match is tactically balanced based on current numbers.`,
    );
  }
  ...
  const over25Pct =
    hasXgInputs && xgHome != null && xgAway != null
      ? clamp(Math.round(((xgHome + xgAway) / 3.2) * 100), 8, 92)
      : clamp(
          Math.round(
            40 + (homeShots + awayShots) * 1.5 + (homeSot + awaySot) * 2,
          ),
          8,
          90,
        );

  const bothTeamsToScorePct =
    hasXgInputs && xgHome != null && xgAway != null
      ? clamp(
          Math.round(
            (Math.min(xgHome, xgAway) /
              Math.max(0.7, Math.max(xgHome, xgAway))) *
              100 *
              0.85,
          ),
          10,
  ...
  const confidenceReason =
    confidence >= 72
      ? `Strong statistical edge for ${prediction === "Home Win" ? homeName : prediction === "Away Win" ? awayName : "either side"} across shots, chances and momentum.`
      : confidence >= 58
        ? isInternational
          ? `Multiple signals point the same direction, but international variance adds risk.`
          : `Multiple signals lean toward ${prediction === "Home Win" ? homeName : prediction === "Away Win" ? awayName : "a draw"}, but match volatility remains.`
        : `Match picture is volatile — the outcome remains open between ${homeName} and ${awayName}.`;
  ...
  return {
    ...
    nextGoalProbability,
    momentum:
      homePct > awayPct ? "Home" : awayPct > homePct ? "Away" : "Balanced",
    danger:
      homeSot > awaySot
        ? "Home Attack"
        : awaySot > homeSot
    ...
    summary: hasMatchSignal
      ? minute > 0
        ? `${homeName} ${homeScore}-${awayScore} ${awayName} at ${minute}' — analysis based on ${hasXgInputs ? `shot quality (${homeSot}-${awaySot} on target)` : "score and events"}${hasContextInputs ? ", league context" : ""} and momentum.`
        : `${homeName} vs ${awayName} — pre-match analysis based on ${hasContextInputs ? "league position, squad depth" : "available context"}${hasXgInputs ? " and shot data" : ""}.`
      : `${homeName} vs ${awayName} — limited data available, prediction uses contextual baseline.`,
    keyFactors,
    tacticalNotes,
    h2hSummary: null,
    formHome: null,
    ...
    tip:
      prediction === "Draw"
        ? `Draw looks plausible at ${drawPct}% — consider a cautious approach, especially in late phases.`
        : `${prediction === "Home Win" ? homeName : awayName} hold the statistical edge (${prediction === "Home Win" ? homePct : awayPct}%). Model line: ${predictedHome}-${predictedAway}. Watch for cards and transition moments.`,
    // Enhanced analysis fields (computed from stats)
    matchInsight: hasMatchSignal
      ? `${homeName} host ${awayName} in a ${confidence >= 65 ? "clear-cut" : "tight"} encounter. ${
          homeRank > 0 && awayRank > 0
            ? `They sit #${homeRank} vs #${awayRank} in the table.`
            : "League table data is currently unavailable."
        }`
      : null,
    formGuide: hasContextInputs
      ? {
          homeForm: `${homeName} ${homeRank > 0 ? `sit ${homeRank}${homeRank === 1 ? "st" : homeRank === 2 ? "nd" : homeRank === 3 ? "rd" : "th"}` : "are mid-table"} with ${homePoints || 0} points and a goal difference of ${homeGoalDiff >= 0 ? "+" : ""}${homeGoalDiff || 0}.`,
          awayForm: `${awayName} ${awayRank > 0 ? `sit ${awayRank}${awayRank === 1 ? "st" : awayRank === 2 ? "nd" : awayRank === 3 ? "rd" : "th"}` : "are mid-table"} with ${awayPoints || 0} points and a goal difference of ${awayGoalDiff >= 0 ? "+" : ""}${awayGoalDiff || 0}.`,
        }
      : null,
    tacticalEdge: hasXgInputs
      ? {
          homeStrengths: [
            homeSot > awaySot
              ? `Clinical finishing (${homeSot} shots on target)`
              : null,
            homePoss > awayPoss + 5
              ? `Possession dominance (${homePoss}%)`
              : null,
            homeCorners > awayCorners + 2
              ? `Set-piece pressure (${homeCorners} corners)`
              : null,
          ]
            .filter(Boolean)
            .slice(0, 3),
          homeWeaknesses: [
            awaySot > homeSot
              ? `Vulnerable to shots on target (${awaySot} conceded)`
              : null,
            awayPoss > homePoss + 5 ? `Losing the possession battle` : null,
            homeRed > 0
              ? `Man disadvantage from ${homeRed} red card${homeRed > 1 ? "s" : ""}`
              : null,
          ]
            .filter(Boolean)
            .slice(0, 3),
          awayStrengths: [
            awaySot > homeSot
              ? `Clinical finishing (${awaySot} shots on target)`
              : null,
            awayPoss > homePoss + 5
              ? `Possession dominance (${awayPoss}%)`
              : null,
            awayCorners > homeCorners + 2
              ? `Set-piece pressure (${awayCorners} corners)`
              : null,
          ]
            .filter(Boolean)
            .slice(0, 3),
          awayWeaknesses: [
            homeSot > awaySot
              ? `Vulnerable to shots on target (${homeSot} conceded)`
              : null,
            homePoss > awayPoss + 5 ? `Losing the possession battle` : null,
            awayRed > 0
              ? `Man disadvantage from ${awayRed} red card${awayRed > 1 ? "s" : ""}`
              : null,
          ]
            .filter(Boolean)
            .slice(0, 3),
    ...
    attackingStrength: hasXgInputs
      ? {
          home: clamp(
            Math.round(40 + homeShots * 1.5 + homeSot * 3 + homeCorners * 0.8),
            10,
            95,
          ),
          away: clamp(
            Math.round(40 + awayShots * 1.5 + awaySot * 3 + awayCorners * 0.8),
            10,
            95,
          ),
        }
    ...
    defensiveStrength: hasXgInputs
      ? {
          home: clamp(
            Math.round(60 - awaySot * 3 + homeBlocked * 1.5 + homeGkSaves * 2),
            10,
            95,
          ),
          away: clamp(
            Math.round(60 - homeSot * 3 + awayBlocked * 1.5 + awayGkSaves * 2),
            10,
            95,
          ),
        }
    ...
    matchPattern: hasXgInputs
      ? homePoss > awayPoss + 8
        ? `${homeName} dominate possession, ${awayName} look to counter-attack.`
        : awayPoss > homePoss + 8
          ? `${awayName} hold the ball, ${homeName} play on the transition.`
          : `Open and balanced contest with chances at both ends.`
      : null,
    predictionExplanation: hasMatchSignal
      ? `${prediction === "Draw" ? "Analysis points to an evenly-matched contest" : prediction === "Home Win" ? `${homeName} hold a statistical advantage` : `${awayName} hold a statistical advantage`} based on ${hasXgInputs ? "shot quality and possession" : "score and match flow"}. ${
          homeRed > 0 || awayRed > 0 ? "Red cards play a crucial role. " : ""
        }Confidence is ${confidence >= 65 ? "relatively high" : "moderate"} given the available data.`
      : null,
    formation: null,
    pressureIndex: hasXgInputs
      ? clamp(
          Math.round(homePoss * 0.6 + homeCorners * 2 + homeShots * 1.2),
          0,
          100,
        )
      : null,
    ...
    unavailableReason: !hasMatchSignal
      ? "Minimal match signals available; prediction uses contextual baseline"
      : !hasXgInputs
        ? "xG limited: prediction uses score, events and league context"
        : null,
  };
}
...
function findTeamRowByName(rows, teamName) {
  ...
  for (const row of list) {
    ...
    if (
      rowKey === teamKey ||
      rowKey.includes(teamKey) ||
      teamKey.includes(rowKey)
    ) {
      return row;
    }
  }
...
app.get("/health", (req, res) => {
  ...
  res.json({
    ...
    tz: TZ,
    aiReady,
    integrationsReady: { zilliz: _zillizReady },
  });
});
...
app.get("/api/img", async (req, res) => {
  ...
  // Some client flows can accidentally hand us an already-proxied local image URL
  // like http://localhost:8080/api/img?url=https://api.sofascore.app/...
  // Unwrap it before hostname allow-listing to avoid recursive 403 loops.
  for (let i = 0; i < 3; i += 1) {
    try {
      const nested = new URL(url);
      const isLocalProxy =
        (nested.hostname === "localhost" || nested.hostname === "127.0.0.1") &&
        nested.pathname === "/api/img";
      const nextUrl = String(nested.searchParams.get("url") || "").trim();
      if (!isLocalProxy || !/^https?:\/\//i.test(nextUrl)) break;
      url = nextUrl;
    } catch {
      break;
    }
  ...
  try {
    ...
    // Expo Web runs on a different origin (e.g. localhost:18081) than the API
    // server (localhost:8080). Allow these proxied player photos to be embedded
    // cross-origin in <img>/expo-image tags.
...
function filterMatchesByLeague(matches, league) {
  ...
  const selected = String(league || "Alle")
    .trim()
    .toLowerCase();
  ...
  return matches.filter((m) => {
    ...
    return (
      leagueName === selected ||
      leagueName.includes(selected) ||
      selected.includes(leagueName)
    );
  });
}
  ...
  for (let i = 0; i <= ESPN_LOOKAHEAD_DAYS; i += 1) {
    try {
      ...
      const filtered = mapped.filter(
        (m) => major.has(normalizeLeagueName(m.league)) || m.league,
      );
      const liveRaw = filtered.filter((m) => m.status === "live");
      const upcomingRaw = filtered
        .filter((m) => m.status === "upcoming")
        .sort(sortMatchByKickoff);
      const finishedRaw = filtered
...
async function enhanceSportsFallbackWithAI(scoredRows) {
  ...
  try {
    const sys = {
      role: "system",
      content:
        "Je bent een voetbalanalist. Geef enkel geldige JSON zonder markdown of extra tekst.",
    };
    const user = {
      role: "user",
      content:
    ...
    if (process.env.GEMINI_API_KEY)
      providers.push({
        name: "gemini",
        run: () => geminiChat([sys, user], { temperature: 0.15 }),
      });
    if (process.env.DEEPSEEK_API_KEY)
      providers.push({
        name: "deepseek",
        run: () => deepseekChat([sys, user], { temperature: 0.15 }),
      });
    if (process.env.OPENROUTER_API_KEY)
      providers.push({
        name: "openrouter",
        run: () => openrouterChat([sys, user], { temperature: 0.15 }),
      });
    if (process.env.GROQ_API_KEY)
      providers.push({
        name: "groq",
        run: () => groqChat([sys, user], { temperature: 0.15 }),
      });
    if (process.env.OPENAI_API_KEY)
      providers.push({
        name: "openai",
        run: () =>
          openaiChat([sys, user], { temperature: 0.15, model: "gpt-4o-mini" }),
      });
    if (process.env.OLLAMA_MODEL)
      providers.push({
        name: "ollama",
        run: () => ollamaChat([sys, user], { temperature: 0.15 }),
      });
    ...
    for (const provider of providers) {
      try {
        ...
        const mapped = Array.isArray(parsed?.predictions)
          ? parsed.predictions
    ...
    const byId = new Map(
      predictions.map((item) => [String(item?.matchId || ""), item]),
    );
    const merged = scoredRows.map((row) => {
      ...
      const homePct =
        sum > 0 ? Math.round((homePctRaw / sum) * 100) : row.homePct;
      const drawPct =
        sum > 0 ? Math.round((drawPctRaw / sum) * 100) : row.drawPct;
      const awayPct = sum > 0 ? 100 - homePct - drawPct : row.awayPct;

      const prediction = ["Home Win", "Away Win", "Draw"].includes(
        String(p.prediction || ""),
      )
        ? String(p.prediction)
        : row.prediction;
      ...
      return {
        ...
        prediction,
        confidence:
          clamp(Math.round(toNum(p.confidence)), 0, 100) || row.confidence,
        homePct,
        drawPct,
        awayPct,
        over25Pct:
          clamp(Math.round(toNum(p.over25Pct)), 0, 100) || row.over25Pct,
        bothTeamsToScorePct:
          clamp(Math.round(toNum(p.bothTeamsToScorePct)), 0, 100) ||
          row.bothTeamsToScorePct,
        doubleChanceHomePct: clamp(homePct + drawPct, 0, 100),
        doubleChanceAwayPct: clamp(awayPct + drawPct, 0, 100),
      };
    });
    ...
    console.warn(
      "[menu-tools] AI fallback enrichment failed:",
      err?.message || err,
    );
    return { rows: scoredRows, usedAI: false, aiProvider: null };
  }
...
async function buildSportsMenuToolsPayload(payload) {
  ...
  const {
    ...
    usedAI,
    aiProvider,
  } = await enhanceSportsFallbackWithAI(scored);
  ...
  return {
    ...
    source: usedAI
      ? `backend-model-v1+${String(aiProvider || "ai")}`
      : "backend-model-v1",
    footballPredictions,
    dailyAccaPicks,
  };
...
function mapEspnSummaryStats(summary, homeTeamId, awayTeamId) {
  ...
  const fillStats = (target, row) => {
    ...
    for (const stat of stats) {
      const key = normalizeEspnStatKey(
        stat?.name || stat?.abbreviation || stat?.displayName,
      );
      if (!key) continue;
  ...
  for (const row of boxTeams) {
    ...
    const target =
      rowTeamId && rowTeamId === String(homeTeamId || "")
        ? home
  ...
  if (Object.keys(home).length === 0 && boxTeams[0])
    fillStats(home, boxTeams[0]);
  if (Object.keys(away).length === 0 && boxTeams[1])
    fillStats(away, boxTeams[1]);
...
function mapEspnSummaryDetails(summary) {
    ...
    for (const ev of keyEvents) {
      ...
      const assist = String(
        ev?.athletesInvolved?.[1]?.displayName ||
          ev?.participants?.[1]?.athlete?.displayName ||
          "",
      );
      mapped.push({
        time,
        ...
        team:
          ev?.team?.displayName ||
          ev?.participants?.[0]?.athlete?.team?.displayName ||
          "",
  ...
  if (scoringPlays.length > 0 && !mapped.some((m) => /goal/i.test(m.type))) {
    for (const sp of scoringPlays) {
      ...
      const player = String(
        sp?.athletesInvolved?.[0]?.displayName ||
          sp?.scoringAthlete?.displayName ||
          "",
      );
      mapped.push({
        time,
        ...
        player,
        assist: String(
          sp?.athletesInvolved?.[1]?.displayName ||
            sp?.assistAthlete?.displayName ||
            "",
        ),
      });
    }
...
function normalizeTeamSide(teamName, homeTeam, awayTeam) {
  ...
  if (
    eventTeam === homeKey ||
    eventTeam.includes(homeKey) ||
    homeKey.includes(eventTeam)
  )
    return "home";
  if (
    eventTeam === awayKey ||
    eventTeam.includes(awayKey) ||
    awayKey.includes(eventTeam)
  )
    return "away";
  return "center";
}
...
function classifyTimelineEvent(ev) {
  const joined =
    `${ev?.type || ""} ${ev?.detail || ""} ${ev?.text || ""}`.toLowerCase();
  if (
    joined.includes("kick off") ||
    joined.includes("kickoff") ||
    joined.includes("match start")
  ) {
  ...
  if (
    ...
    joined.includes("match ended") ||
    joined.includes("fulltime")
  ) {
    return {
      kind: "fulltime",
      title: "Full Time",
  ...
  if (
    joined.includes("own goal") ||
    joined.includes("own_goal") ||
    joined.includes("eigen goal")
  ) {
    return {
      kind: "own_goal",
      title: "Own Goal",
  ...
  if (
    joined.includes("missed penalty") ||
    (joined.includes("penalty") && joined.includes("miss"))
  ) {
    return {
      kind: "missed_penalty",
  ...
  if (
    joined.includes("penalty") &&
    (joined.includes("goal") || joined.includes("scored"))
  ) {
    return {
      kind: "penalty_goal",
      title: "Penalty Goal",
  ...
  if (
    ...
    joined.includes("scores") ||
    joined.includes("scored")
  ) {
  ...
  if (
    joined.includes("substitut") ||
    joined.includes("sub ") ||
    joined.includes("substitution")
  ) {
    return {
      kind: "substitution",
      title: "Substitution",
  ...
  if (
    joined.includes("chance") ||
    joined.includes("shot") ||
    joined.includes("attempt") ||
    joined.includes("woodwork")
  ) {
    return {
      kind: "chance",
      title: "Big Chance",
...
function buildTimelineDescription(ev, meta) {
  ...
  if (meta.kind === "substitution") {
    if (player && assist)
      return { description: `${player} in`, secondary: `${assist} out` };
    if (player) return { description: player, secondary: detail || null };
  }

  if (
    meta.kind === "goal" ||
    meta.kind === "penalty_goal" ||
    meta.kind === "own_goal"
  ) {
    return {
  ...
  if (
    meta.kind === "yellow_card" ||
    meta.kind === "red_card" ||
    meta.kind === "second_yellow"
  ) {
  ...
  if (meta.kind === "var" || meta.kind === "chance") {
    return {
      description: detail || player || meta.title,
      secondary: assist || null,
    };
  }
...
function buildNormalizedTimeline(
  ...
  if (
    (statusLower === "finished" ||
      (statusLower === "live" && minuteNow >= 45)) &&
    !normalized.some((e) => e.kind === "halftime")
  ) {
    normalized.push({
      id: "halftime_synth",
      kind: "halftime",
  ...
  if (
    statusLower === "finished" &&
    !normalized.some((event) => event.kind === "fulltime")
  ) {
    normalized.push({
      id: "fulltime_synth",
      kind: "fulltime",
  ...
  if (
    !hasGoalEvents &&
    totalGoals > 0 &&
    (statusLower === "finished" || statusLower === "live")
  ) {
...
function countTimelineEvents(timeline, side, kinds) {
  ...
  return (Array.isArray(timeline) ? timeline : []).filter(
    (event) => event.side === side && wanted.has(event.kind),
  ).length;
}
...
function toRoundedStat(key, value) {
  ...
  if (["expected_goals", "goals_prevented"].includes(key))
...
function buildAdvancedStats(homeStatsRaw, awayStatsRaw, timeline) {
  const makeTeam = (rawStats, side) => {
    ...
    const touchesInBox =
      read("touches_in_box") ??
      (read("total_shots") != null
        ? Math.round(read("total_shots") * 3.2)
        : null);
    const passesFinalThird =
      read("passes_final_third") ??
      (read("total_passes") != null
        ? Math.round(read("total_passes") * 0.28)
        : null);
    const keyPasses =
      read("key_passes") ??
      countTimelineEvents(timeline, side, ["goal", "penalty_goal", "chance"]);
    const bigChances =
      read("big_chances") ??
      countTimelineEvents(timeline, side, [
        "goal",
        "penalty_goal",
        "chance",
        "missed_penalty",
    ...
    const readRaw = (key) => {
      const v = firstStatValue(rawStats, MATCH_STAT_ALIASES[key] || []);
      return v != null ? toNum(v) : null;
    };
    const rawPassAcc = readRaw("pass_accuracy");
    const passAccuracy =
      rawPassAcc != null && rawPassAcc > 0 && rawPassAcc <= 1
        ? Math.round(rawPassAcc * 100)
        : rawPassAcc != null
          ? Math.round(rawPassAcc)
          : null;
    const rawPossession = readRaw("possession");
    const possession =
      rawPossession != null && rawPossession > 0 && rawPossession <= 1
        ? Math.round(rawPossession * 100)
        : rawPossession != null
          ? Math.round(rawPossession)
          : null;
    ...
    return Object.fromEntries(
      Object.entries({
        ...
        yellow_cards:
          read("yellow_cards") ??
          countTimelineEvents(timeline, side, ["yellow_card", "second_yellow"]),
        red_cards:
          read("red_cards") ??
          countTimelineEvents(timeline, side, ["red_card"]),
        saves: read("saves"),
...
function buildMatchHighlights(timeline, match) {
  ...
  const highlightable = events.filter(
    (event) =>
      !["kickoff", "halftime", "fulltime", "info"].includes(event.kind),
  );
  const topMoments = [...highlightable]
    .sort(
      (a, b) => b.importance - a.importance || a.minuteValue - b.minuteValue,
    )
    .slice(0, 8)
    .sort((a, b) => a.minuteValue - b.minuteValue);

  const recap = [];
  const goals = highlightable.filter((event) =>
    ["goal", "penalty_goal", "own_goal"].includes(event.kind),
  );
  const cards = highlightable.filter((event) =>
    ["yellow_card", "red_card", "second_yellow"].includes(event.kind),
  );
  ...
  if (goals.length > 0) {
    recap.push(
      `${goals.length} beslissende doelmoment${goals.length === 1 ? "" : "en"} bepaalden het scoreverloop.`,
    );
  }
  if (cards.length > 0) {
    const reds = cards.filter((c) =>
      ["red_card", "second_yellow"].includes(c.kind),
    );
    if (reds.length > 0) {
      recap.push(
        `${reds.length} rode kaart${reds.length === 1 ? "" : "en"} beïnvloedde${reds.length === 1 ? "" : "n"} het wedstrijdverloop.`,
      );
    }
    recap.push(
      `Discipline speelde mee met ${cards.length} kaartmoment${cards.length === 1 ? "" : "en"}.`,
    );
  }
  if (vars.length > 0) {
    recap.push(
      `VAR greep ${vars.length} keer in tijdens cruciale fases van de match.`,
    );
  }
  ...
  if (hPoss > 0 && aPoss > 0 && Math.abs(hPoss - aPoss) >= 10) {
    const dom =
      hPoss > aPoss ? match?.homeTeam || "Thuis" : match?.awayTeam || "Uit";
    recap.push(
      `${dom} domineerde het balbezit met ${Math.max(hPoss, aPoss)}%.`,
    );
  }
  if (hShots > 0 && aShots > 0 && Math.abs(hShots - aShots) >= 4) {
    const dom =
      hShots > aShots ? match?.homeTeam || "Thuis" : match?.awayTeam || "Uit";
    recap.push(
      `${dom} was gevaarlijker met ${Math.max(hShots, aShots)} schoten (${Math.max(hSot || 0, aSot || 0)} op doel).`,
    );
  }
  if (subs.length > 0) {
    recap.push(
      `${subs.length} wissel${subs.length === 1 ? "" : "s"} brachten vers bloed in de wedstrijd.`,
    );
  }

  if (highlightable.length === 0 && recap.length === 0) {
    recap.push(
      "Nog geen grote incidenten; de wedstrijd wordt voorlopig vooral tactisch uitgevochten.",
    );
  }
...
function finalizeMatchPayload(match) {
  ...
  const highlights = buildMatchHighlights(timeline, {
    ...base,
      ...
      const highlights = items
        ...
        .map((item) => {
          ...
          const firstVideoEmbed =
            videos.length > 0 ? videos[0]?.embed || "" : "";
...
app.get("/api/sports/prefetch-home", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[prefetch] Warmed ${fulfilled}/${results.length} caches in ${Date.now() - started}ms`,
    );
    res.json({
      ok: true,
      warmed: fulfilled,
...
// Reverse mapping: ESPN slug → display name (built from ESPN_LEAGUE_SLUGS, first name wins)
const ESPN_SLUG_TO_NAME = {};
for (const [name, slug] of Object.entries(ESPN_LEAGUE_SLUGS)) {
  if (!ESPN_SLUG_TO_NAME[slug]) ESPN_SLUG_TO_NAME[slug] = name;
}

// ESPN slug prefix → country
const ESPN_SLUG_COUNTRY = {
  eng: "England",
  esp: "Spain",
  ger: "Germany",
  ita: "Italy",
...
function leagueSlugToDisplayName(slug) {
  return ESPN_SLUG_TO_NAME[slug] || normalizeLeagueName(slug) || slug;
}

function leagueSlugToCountry(slug) {
  const prefix = String(slug || "")
    .split(".")[0]
    .toLowerCase();
  return ESPN_SLUG_COUNTRY[prefix] || "";
}
...
function resolveCanonicalTeamName(teamName) {
  const raw = String(teamName || "").trim();
  if (!raw) return "";
  const normalized = normalizeTeamKey(raw);
  if (!normalized) return raw;

  const aliasHit = TEAM_LOGO_ALIASES[normalized];
  if (aliasHit) return aliasHit;

  for (const [alias, canonical] of Object.entries(TEAM_LOGO_ALIASES)) {
    if (normalizeTeamKey(canonical) === normalized) return canonical;
    if (normalizeTeamLookupKey(alias) === normalizeTeamLookupKey(normalized)) {
      return canonical;
    }
  }
...
function resolveDomesticLeagueSlug(teamName) {
  const canonical = resolveCanonicalTeamName(teamName);
  const normalizedTarget = normalizeTeamKey(canonical || teamName);
  const looseTarget = normalizeTeamLookupKey(canonical || teamName);
  if (!normalizedTarget) return "";

  let bestFolder = "";
  let bestScore = 0;
  for (const [mapTeam, mapValue] of Object.entries(TEAM_FILENAME_TO_FOLDER)) {
    const mapNorm = normalizeTeamKey(mapTeam);
    const mapLoose = normalizeTeamLookupKey(mapTeam);
    let score = 0;

    if (mapNorm === normalizedTarget) score = 1.2;
    else if (looseTarget && mapLoose === looseTarget) score = 1.12;
    else {
      const strictSimilarity = similarityScore(normalizedTarget, mapNorm);
      const looseSimilarity =
        looseTarget && mapLoose ? similarityScore(looseTarget, mapLoose) : 0;

      if (strictSimilarity >= 0.86 || looseSimilarity >= 0.9) {
        score = Math.max(strictSimilarity, looseSimilarity);
      } else if (
        mapNorm.includes(normalizedTarget) ||
        normalizedTarget.includes(mapNorm)
      ) {
        score = 0.8;
      } else if (
        looseTarget &&
        mapLoose &&
        Math.min(looseTarget.length, mapLoose.length) >= 5 &&
        (mapLoose.includes(looseTarget) || looseTarget.includes(mapLoose))
      ) {
        score = 0.76;
      }
    }
  ...
  if (bestScore < 0.72 || !bestFolder) return "";
  const leaguePart = String(bestFolder)
    .split(" - ")
    .slice(1)
    .join(" - ")
    .trim();
  return (
    ESPN_LEAGUE_SLUGS[leaguePart] ||
    ESPN_LEAGUE_SLUGS[normalizeLeagueName(leaguePart)] ||
    ""
  );
}
...
function extractStandingsGroupsFromEspn(data) {
  const groups = [];
  const pushGroup = (name, index, entries) => {
  ...
  children.forEach((group, index) => {
    const entries = Array.isArray(group?.standings?.entries)
      ...
      : Array.isArray(group?.entries)
        ? group.entries
        : [];
    pushGroup(group?.name || group?.abbreviation || "", index, entries);
  });
  ...
  if (
    ...
    data.standings.entries.length
  ) {
    pushGroup(
      data?.name || data?.abbreviation || "Regular Season",
      0,
  ...
  if (Array.isArray(data?.standings)) {
    ...
    if (
      rows.length &&
      (rows[0]?.team || rows[0]?.stats || rows[0]?.rank != null)
    ) {
      pushGroup(data?.name || "Regular Season", 0, rows);
    }
  }
  ...
  if (Array.isArray(data?.entries) && data.entries.length) {
    pushGroup(data?.name || "Regular Season", 0, data.entries);
  }

  return groups;
}

function scoreStandingsGroup(group) {
  const name = String(group?.name || "").toLowerCase();
  let score = Math.min((group?.entries?.length || 0) * 2, 80);
  if (/(play\s*-?\s*off|championship|title|kampioen|top\s*\d+)/i.test(name)) {
    score += 120;
  }
  if (/(regular\s*season)/i.test(name)) {
    score += 24;
  }
  if (/(relegation|play\s*-?\s*out|bottom|degrad)/i.test(name)) {
    score -= 40;
  }
  return score;
}

function pickPreferredStandingsGroup(data) {
  const groups = extractStandingsGroupsFromEspn(data);
  if (!groups.length) return { preferredGroup: null, groups: [] };
  const preferredGroup = [...groups].sort(
    (a, b) => scoreStandingsGroup(b) - scoreStandingsGroup(a),
  )[0];
  return { preferredGroup, groups };
}

async function espnStandings(leagueName) {
  const slug =
    ESPN_LEAGUE_SLUGS[leagueName] ||
    ESPN_LEAGUE_SLUGS[normalizeLeagueName(leagueName)] ||
    leagueName;
  ...
  for (const st of [1, 2]) {
    ...
    const data = await resp.json();
    // Check if this response has actual standings entries (supports multi-shape payloads)
    const hasGroups = extractStandingsGroupsFromEspn(data).length > 0;
    if (hasGroups) return data;
    // seasontype=1 empty → try seasontype=2 (playoffs)
    console.log(
      `[standings] ${leagueName}: seasontype=${st} returned no entries, trying next`,
    );
  }
...
async function espnTopScorers(leagueName) {
  const slug =
    ...
    leagueName;
  const url = withEspnApiKey(
    `https://site.web.api.espn.com/apis/v2/sports/soccer/${slug}/leaders`,
  );
  const resp = await fetchWithTimeout(
    fetch(url, { headers: espnRequestHeaders() }),
    12000,
...
async function fetchJsonWithTimeout(url, timeoutMs = 12000) {
  ...
  if (!resp.ok)
    throw new Error(`Fetch ${resp.status}: ${redactSensitiveUrl(finalUrl)}`);
  return resp.json();
}

async function espnSeasonLeaders(
  leagueName,
  ...
  for (const type of [1, 2]) {
    try {
      ...
      console.warn(
        `[leaders] ${leagueName}: season=${season} type=${type} failed: ${e.message}`,
      );
    }
  }
  return null;
...
async function mapEspnCoreLeaders(data, kind) {
  try {
    ...
    const leaders = Array.isArray(category?.leaders)
      ? category.leaders.slice(0, 30)
      : [];
    if (!leaders.length) return [];

    const mapped = await Promise.all(
      leaders.map(async (entry, idx) => {
        ...
        const [athlete, team] = await Promise.all([
          entry?.athlete?.$ref
            ? fetchJsonWithTimeout(entry.athlete.$ref, 8000).catch(() => null)
            : Promise.resolve(null),
          entry?.team?.$ref
            ? fetchJsonWithTimeout(entry.team.$ref, 8000).catch(() => null)
            : Promise.resolve(null),
        ]);
        const displayName = athlete?.displayName || athlete?.fullName || "";
        const teamName = team?.displayName || team?.name || "";
        const directPhoto = athleteId
          ? `https://a.espncdn.com/i/headshots/soccer/players/full/${encodeURIComponent(String(athleteId))}.png`
          : null;
        const directLogo = teamId
          ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(teamId))}.png`
          : null;
        return {
          rank: idx + 1,
          id: String(athleteId || ""),
...
function mapEspnStandings(data) {
  try {
    const { preferredGroup, groups } = pickPreferredStandingsGroup(data);
    if (!groups.length) {
    ...
    const phases = groups
      .map((group, gIdx) => {
        const phaseName =
          group?.name || (gIdx === 0 ? "Regular Season" : `Group ${gIdx + 1}`);
        const standings = (group?.entries || [])
          .map((entry, idx) =>
            mapStandingsEntry(entry, idx, phaseName, group.index),
          )
          .sort((a, b) => (a.rank || 99) - (b.rank || 99));
        return { phase: phaseName, standings };
      })
      .filter((p) => p.standings.length > 0);

    const selectedPhase = phases.find(
      (phase) => phase.phase === (preferredGroup?.name || ""),
    ) ||
      phases[0] || { standings: [] };

    return {
      entries: selectedPhase.standings,
      phases,
      selectedPhase: preferredGroup?.name || selectedPhase.phase || null,
...
function mapStandingsEntry(entry, idx, groupPhase = null, groupIndex = null) {
  const team = entry?.team || {};
  const stats = {};
  for (const s of entry?.stats || []) {
    stats[s.name] = s.value;
  }
  const explicitRank = Number(entry?.rank ?? entry?.position);
  const rank =
    Number.isFinite(explicitRank) && explicitRank > 0 ? explicitRank : idx + 1;
  return {
    rank,
    ...
    logo: normalizeTeamLogo(
      ...
      team.logos?.[0]?.href || team.logo || null,
      team.id
        ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(team.id))}.png`
        : null,
    ),
      ...
      return {
        ...
        photo: ensureValidPlayerPhoto(
          normalizePlayerPhoto(
            ...
            ath.headshot?.href,
            athleteId
              ? `https://a.espncdn.com/i/headshots/soccer/players/full/${encodeURIComponent(athleteId)}.png`
              : null,
          ),
          displayName,
          athleteId,
        ...
        teamLogo: normalizeTeamLogo(
          ...
          ath.team?.id
            ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(ath.team.id))}.png`
            : null,
        ),
        goals: entry?.value ?? 0,
...
function mapEspnTopAssists(data) {
  try {
    const assistLeaders = (data?.leaders || []).find(
      (l) =>
        l.name === "assists" ||
        l.displayName?.toLowerCase().includes("assist") ||
        l.abbreviation === "A",
    );
...
async function espnTopScorersFromHtml(leagueName) {
  const code =
    ESPN_STATS_LEAGUE_CODES[leagueName] ||
  ...
  while ((tr = trRe.exec(html)) !== null) {
    ...
    const playerMatch = row.match(
      /\/soccer\/player\/_\/id\/(\d+)\/[^\"]*\">([^<]+)<\/a>/i,
    );
    const teamMatch = row.match(
      /\/soccer\/club\/_\/id\/(\d+)\/[^\"]*\">([^<]+)<\/a>/i,
    );
    const goalsMatches = Array.from(
      row.matchAll(
        /<td class=\"tar Table__TD\"><span class=\"tar\">(\d+)<\/span><\/td>/gi,
      ),
    );
    ...
    const payload = await getOrFetch(key, 15 * 60_000, async () => {
      ...
      try {
        ...
        if (scorers.length > 0) {
          ...
          // Enrich player photos: Sofascore team-cache → TM → TheSportsDB → ESPN CDN validation
          scorers = await enrichScorersPhotos(scorers, leagueName);
          console.log(
            `[topscorers] ${leagueName}: ESPN core → ${scorers.length} scorers`,
          );
          scorers = ensurePlayersHaveValidPhotos(scorers);
          return withScorersCompat({
            league: leagueName,
            season,
            seasonLabel,
            scorers,
        ...
        console.warn(
          `[topscorers] ESPN core failed for ${leagueName}: ${e.message}`,
        );
      }
      ...
      try {
        ...
        if (htmlScorers.length > 0) {
          ...
          console.log(
            `[topscorers] ${leagueName}: ESPN HTML → ${htmlScorers.length} scorers`,
          );
        ...
        console.warn(
          `[topscorers] ESPN HTML failed for ${leagueName}: ${e.message}`,
        );
      }
      ...
      return withScorersCompat({
        ...
        scorers: [],
        source: "espn",
        error: "Topscorers tijdelijk niet beschikbaar via ESPN",
      });
    });
    if (!res.headersSent) res.json(withScorersCompat(payload));
  } catch (e) {
    ...
    const payload = await getOrFetch(key, 15 * 60_000, async () => {
      ...
      try {
        ...
        if (assists.length > 0) {
          ...
          console.log(
            `[topassists] ${leagueName}: ESPN core → ${assists.length} assisters`,
          );
          assists = ensurePlayersHaveValidPhotos(assists);
          return withAssistsCompat({
            league: leagueName,
            season,
            seasonLabel,
            assists,
        ...
        console.warn(
          `[topassists] ESPN core failed for ${leagueName}: ${e.message}`,
        );
      }
      if (!leagueId) {
        return withAssistsCompat({
      ...
      return withAssistsCompat({
        ...
        assists: [],
        source: "espn",
        error: "Top assists tijdelijk niet beschikbaar",
      });
    });
    if (!res.headersSent) res.json(withAssistsCompat(payload));
  } catch (e) {
...
app.get("/api/sports/competition-stats/:league", async (req, res) => {
  ...
  try {
    const payload = await getOrFetch(key, 20 * 60_000, async () => {
      ...
      const totalMatches =
        standings.reduce((s, t) => s + (t.played || 0), 0) / 2;
      const avgGoalsPerMatch =
        totalMatches > 0 ? (totalGoals / totalMatches).toFixed(2) : null;
      ...
      const bestDefense = [...standings]
        ...
        .slice(0, 5);
      const homeTable = [...standings]
        .sort((a, b) => (b.points || 0) - (a.points || 0))
        .slice(0, 5);
      const mostWins = [...standings]
        .sort((a, b) => (b.wins || 0) - (a.wins || 0))
        .slice(0, 5);
      const mostDraws = [...standings]
      ...
      return {
        ...
        avgGoalsPerMatch,
        bestAttack: bestAttack.map((t) => ({
          team: t.team,
          logo: t.logo,
          teamId: t.teamId,
          goalsFor: t.goalsFor,
        })),
        bestDefense: bestDefense.map((t) => ({
          team: t.team,
          logo: t.logo,
          teamId: t.teamId,
          goalsAgainst: t.goalsAgainst,
        })),
        mostWins: mostWins.map((t) => ({
          team: t.team,
          logo: t.logo,
          teamId: t.teamId,
          wins: t.wins,
        })),
        mostDraws: mostDraws.map((t) => ({
          team: t.team,
          logo: t.logo,
          teamId: t.teamId,
          draws: t.draws,
        })),
        leaderTable: homeTable.map((t) => ({
          team: t.team,
          logo: t.logo,
          teamId: t.teamId,
          points: t.points,
    ...
    res
      .status(200)
      .json({ league: leagueName, error: String(e?.message || e) });
  }
});
...
async function espnLeagueMatches(leagueName, wideRange = false) {
  ...
  const dateStrs = [""];
  const offsets = wideRange
    ? [-84, -70, -56, -42, -28, -21, -14, -7, 7, 14, 21, 28]
    : [-21, -14, -7, 7, 14];
  ...
  await Promise.allSettled(
    dateStrs.map(async (dateStr) => {
      const url = dateStr
        ? `${baseUrl}?dates=${dateStr}&limit=20`
        : `${baseUrl}?limit=50`;
      const controller = new AbortController();
  ...
  try {
    const payload = await getOrFetch(key, 10 * 60_000, async () => {
      const resp = await fetchWithTimeout(
        fetch(
          withEspnApiKey(
            `https://site.api.espn.com/apis/site/v2/sports/soccer/${encodeURIComponent(espnSlug)}/teams`,
          ),
          {
            headers: espnRequestHeaders(),
          },
      ...
      const rawTeams =
        json?.sports?.[0]?.leagues?.[0]?.teams || json?.teams || [];
    ...
    res
      .status(200)
      .json({ league: leagueName, teams: [], error: String(e?.message || e) });
  }
});
...
app.get("/api/sports/team/:teamId", async (req, res) => {
  ...
  const countryCodeFromQuery = String(req.query?.countryCode || "")
    .trim()
    .toUpperCase();

  // Auto-detect league from teamCountryMap when no explicit league is provided
  let espnLeague = String(req.query?.league || "").trim();
  // Normalize display names (e.g. "La Liga") to ESPN slugs (e.g. "esp.1")
  const normalizedName = normalizeLeagueName(espnLeague);
  if (normalizedName && ESPN_LEAGUE_SLUGS[normalizedName]) {
    espnLeague = ESPN_LEAGUE_SLUGS[normalizedName];
  }
  if (!espnLeague || espnLeague === "eng.1") {
    const lookupName = teamId.startsWith("name:")
      ? decodeURIComponent(teamId.replace(/^name:/, ""))
      : teamNameFromQuery;
    const autoLeagueSlug = resolveDomesticLeagueSlug(lookupName);
    if (autoLeagueSlug) espnLeague = autoLeagueSlug;
    if (!espnLeague) espnLeague = "eng.1";
  }
  ...
  // Hard deadline: the entire team endpoint must respond within 20s.
  // If getOrFetch (enrichment + standings + stats + schedule) takes longer,
  // return a minimal response from params so the client isn't left hanging.
  const TEAM_ENDPOINT_DEADLINE = 20_000;

  try {
    const payload = await Promise.race([
      getOrFetch(key, 1_800_000, async () => {
        ...
        // Treat non-numeric teamIds (e.g. "Barcelona") as name lookups, same as "name:Barcelona".
        // This happens when team-detail is navigated to with the team display name instead of
        // a numeric ESPN team ID.
        if (
          !resolvedTeamId.startsWith("name:") &&
          !/^\d+$/.test(resolvedTeamId)
        ) {
          resolvedTeamId = `name:${resolvedTeamId}`;
        }

        if (resolvedTeamId.startsWith("name:")) {
          const rawName = decodeURIComponent(
            resolvedTeamId.replace(/^name:/, "").replace(/\+/g, " "),
          ).toLowerCase();
          const nationalId =
            NATIONAL_TEAM_IDS[rawName] ||
            (countryCodeFromQuery
              ? NATIONAL_TEAM_IDS_BY_CODE[countryCodeFromQuery]
              : undefined);
          if (nationalId && espnLeague.includes("fifa")) {
            ...
            const teamsResp = await fetch(
              withEspnApiKey(
                `https://site.api.espn.com/apis/site/v2/sports/soccer/${encodeURIComponent(espnLeague)}/teams`,
              ),
              {
                headers: espnRequestHeaders(),
                signal: AbortSignal.timeout(8000),
            ...
            const found = teams
              ...
              .find((t) => {
                ...
                return (
                  n === rawName ||
                  (rawName.length >= 4 &&
                    (n.includes(rawName) || rawName.includes(n)))
                );
              });
        ...
        if (resolvedTeamId.startsWith("name:") && espnLeague.includes("fifa")) {
          ...
          try {
            ...
            const fallbackJson = fallbackResp.ok
              ? await fallbackResp.json()
              : {};
            const fallbackTeams =
              fallbackJson?.sports?.[0]?.leagues?.[0]?.teams ||
              fallbackJson?.teams ||
              [];
            const match = fallbackTeams
              .map((t) => t?.team || t)
              .find((t) => {
                const n = String(t?.displayName || t?.name || "").toLowerCase();
                return (
        ...
        const leagueVariants = espnLeague.includes("fifa")
          ? [espnLeague, "fifa.friendly", "uefa.nations", "uefa.euro"]
          : [espnLeague];
        ...
        for (const leagueSlug of leagueVariants) {
          const [teamResp, rosterResp] = await Promise.all([
            fetch(
              withEspnApiKey(
                `https://site.api.espn.com/apis/site/v2/sports/soccer/${encodeURIComponent(leagueSlug)}/teams/${encodeURIComponent(resolvedTeamId)}`,
              ),
              {
                headers: espnRequestHeaders(),
                signal: AbortSignal.timeout(8000),
            ...
            fetch(
              withEspnApiKey(
                `https://site.api.espn.com/apis/site/v2/sports/soccer/${encodeURIComponent(leagueSlug)}/teams/${encodeURIComponent(resolvedTeamId)}/roster`,
              ),
              {
                headers: espnRequestHeaders(),
                signal: AbortSignal.timeout(8000),
          ...
          if (Array.isArray(input)) {
            return input.flatMap((item) => {
              ...
              if (item?.items && Array.isArray(item.items))
                return flattenAthletes(item.items);
              if (item?.athletes && Array.isArray(item.athletes))
                return flattenAthletes(item.athletes);
              if (item?.displayName || item?.fullName || item?.id)
                return [item];
              return [];
            });
          }
        ...
        const athletes = flattenAthletes(
          rosterJson?.athletes || rosterJson?.groups || rosterJson?.items || [],
        );
        const dedup = new Map();
        ...
        if (basePlayers.length === 0 && teamDisplayName) {
          ...
          for (const player of Array.isArray(tmFallbackPlayers)
            ? tmFallbackPlayers
            : []) {
            const key = normalizePersonName(player?.name || "");
            if (!key) continue;
            fallbackMap.set(key, {
              ...
              marketValue: player?.marketValueEur
                ? formatEURShort(player.marketValueEur)
                : undefined,
              isRealValue: Boolean(player?.marketValueEur),
              photo: keepIncomingPlayerPhoto(player?.photo),
            });
          }
          for (const player of Array.isArray(dbFallbackPlayers)
            ? dbFallbackPlayers
            : []) {
            const key = normalizePersonName(player?.name || "");
            if (!key) continue;
        ...
        // ── Resolve domestic league for display (don't let European competitions
        //    or the eng.1 default override the team's actual domestic league) ──
        const domesticLeagueSlug = resolveDomesticLeagueSlug(teamDisplayName);
        // Use domestic league for display; keep espnLeague for data fetching
        const displayLeagueSlug = domesticLeagueSlug || espnLeague;

        const isNationalTeam =
          espnLeague.includes("fifa") ||
          /teamlogos\/countries/i.test(String(team?.logos?.[0]?.href || ""));
        const espnLogo = team?.id
          ? `https://a.espncdn.com/i/teamlogos/soccer/500/${encodeURIComponent(String(team.id))}.png`
          : null;
        const rawTeamLogo = team?.logos?.[0]?.href || team?.logo || null;
        const footballLogosUrl = resolveFootballLogosUrl(
          teamDisplayName,
          normalizeLeagueName(espnLeague) || "",
        );

        // Run enrichment + logo resolution with a short deadline so screens render fast.
        // Standings, stats, and schedule run IN PARALLEL with enrichment since they
        // depend only on team.id/resolvedTeamId/espnLeague, which are already resolved.
        const ENRICH_DEADLINE = 6_000;
        const normalizedLeagueName =
          ESPN_SLUG_TO_NAME[espnLeague] ||
          normalizeLeagueName(espnLeague) ||
          normalizeLeagueName(rosterJson?.team?.links?.[0]?.text || "") ||
          "";
        const resolvedId = team?.id || resolvedTeamId || "";
        ...
        const [
          ...
          // ── Enrichment (capped at ENRICH_DEADLINE) ──
          Promise.race([
            (async () => {
              // Start market values + logo fetch in parallel with photo enrichment
              const marketValuePromise = Promise.race([
                enrichRosterMarketValues(
                  basePlayers,
                  teamDisplayName,
                  espnLeague,
                ),
                new Promise((resolve) =>
                  setTimeout(() => resolve(basePlayers), 4000),
                ),
              ]);
              const sportsDbLogoPromise = Promise.race([
                fetchTheSportsDBTeamLogo(teamDisplayName),
                new Promise((resolve) => setTimeout(() => resolve(null), 5000)),
              ]);

              // Check if a previous background enrichment cached full photo results
              const photoCacheKey = `team_photos_full_${normalizePersonName(teamDisplayName)}`;
              const cachedPhotos = cacheGet(photoCacheKey);

              let photoPlayers;
              if (Array.isArray(cachedPhotos) && cachedPhotos.length > 0) {
                // Use cached full enrichment from a previous background run
                photoPlayers = cachedPhotos;
                console.log(
                  `[photos] ${teamDisplayName}: using cached full enrichment (${cachedPhotos.filter((p) => p?.photo).length}/${cachedPhotos.length} photos)`,
                );
              } else {
                // Phase 1: fast bulk photo enrichment (Sofascore ~1-2s, TM+TSDB ~2-4s)
                const bulkPlayers = await Promise.race([
                  enrichRosterPhotosBulk(
                    basePlayers,
                    teamDisplayName,
                    espnLeague,
                  ),
                ...
                photoPlayers = bulkPlayers;

                // Fire-and-forget: run individual enrichment in background and cache result
                enrichRosterPhotosIndividual(
                  bulkPlayers,
                  teamDisplayName,
                  espnLeague,
                )
                  .then((fullPlayers) => {
                    const photoCount = (fullPlayers || []).filter(
                      (p) => p?.photo,
                    ).length;
                    const bulkPhotoCount = (bulkPlayers || []).filter(
                      (p) => p?.photo,
                    ).length;
                    if (photoCount > bulkPhotoCount) {
                      cacheSet(photoCacheKey, fullPlayers, 3_600_000); // 1h cache
                      console.log(
                        `[photos][bg] ${teamDisplayName}: background enrichment cached ${photoCount}/${fullPlayers.length} photos`,
                      );

                      // ── Patch the main team endpoint cache so refetches
                      //    within the 30-min window already include the better photos ──
                      try {
                        const teamCacheEntry = __cache.get(key);
                        if (
                          teamCacheEntry?.value &&
                          Array.isArray(teamCacheEntry.value.players)
                        ) {
                          const photoMap = new Map();
                          for (const p of fullPlayers) {
                            const k = String(p?.id || p?.name || "");
                            if (k && p?.photo) photoMap.set(k, p);
                          }
                          const patched = teamCacheEntry.value.players.map(
                            (player) => {
                              if (player?.photo) return player; // already has photo
                              const k = String(
                                player?.id || player?.name || "",
                              );
                              const enriched = photoMap.get(k);
                              if (!enriched?.photo) return player;
                              return {
                                ...player,
                                photo: ensureValidPlayerPhoto(
                                  enriched.photo,
                          ...
                          const remaining = Math.max(
                            60_000,
                            teamCacheEntry.expiresAt - Date.now(),
                          );
                          cacheSet(
                            key,
                            { ...teamCacheEntry.value, players: patched },
                            remaining,
                          );
                          const patchedCount = patched.filter(
                            (p) => p?.photo,
                          ).length;
                          console.log(
                            `[photos][bg] ${teamDisplayName}: patched team cache ${bulkPhotoCount}→${patchedCount} photos (ttl=${Math.round(remaining / 1000)}s)`,
                          );
                        }
                      } catch (e) {
                        console.warn(
                          `[photos][bg] Failed to patch team cache:`,
                          e?.message || e,
                        );
                      }
                    }
              ...
              // Await market values + logo (already started in parallel)
              const [valuedPlayers, sportsDbLogo] = await Promise.all([
                marketValuePromise,
                sportsDbLogoPromise,
              ]);
              ...
              const enrichedPlayers = (photoPlayers || basePlayers).map(
                (player) => {
                  ...
                  if (valued) {
                    return {
                      ...valued,
                      ...player,
                      // Fill gaps from valued (market value enrichment) data
                      nationality:
                        player.nationality || valued.nationality || undefined,
                      position: player.position || valued.position || "",
                      height: player.height || valued.height || undefined,
                      weight: player.weight || valued.weight || undefined,
                      age: player.age || valued.age || undefined,
                      photo: ensureValidPlayerPhoto(
              ...
              const tmClubData = cacheGet(
                `transfermarkt_club_${normalizePersonName(teamDisplayName)}`,
              );
              ...
              return {
                enrichedPlayers,
                squadMarketValue: clubMarketValue
                  ? formatEURShort(clubMarketValue)
                  : null,
                resolvedLogo,
              };
            })(),
            new Promise((resolve) =>
              setTimeout(() => resolve(null), ENRICH_DEADLINE),
            ),
          ]),
          // ── Standings (capped at 8s) ──
          Promise.race([
            (async () => {
              if (!normalizedLeagueName) return null;
              try {
                const standingsData = await espnStandings(normalizedLeagueName);
                const mappedStandings =
                  mapEspnStandings(standingsData)?.entries || [];
                const byTeamId = mappedStandings.find(
                  (row) =>
                    String(row?.teamId || "") ===
                    String(team?.id || resolvedTeamId || ""),
                );
                const byTeamName = mappedStandings.find(
                  (row) =>
                    normalizePersonName(row?.team || "") ===
                    normalizePersonName(teamDisplayName || ""),
                );
                const byQueryName = teamNameFromQuery
                  ? mappedStandings.find(
                      (row) =>
                        normalizePersonName(row?.team || "") ===
                        normalizePersonName(teamNameFromQuery),
                    )
                  : null;
                return { byTeamId, byTeamName, byQueryName };
              } catch {
                return null;
              }
            })(),
          ...
          // ── Team season stats (5s timeout) ──
          Promise.race([
            (async () => {
              const teamSeasonStats = {};
              if (resolvedId && !resolvedId.startsWith("name:")) {
                try {
                  const season = seasonForDate(new Date());
                  const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${encodeURIComponent(espnLeague)}/seasons/${season}/types/1/teams/${encodeURIComponent(resolvedId)}/statistics`;
                  const statsJson = await fetchJsonWithTimeout(
                    statsUrl,
                    5000,
                  ).catch(() => null);
                  const splits =
                  ...
                  for (const cat of splits) {
                    for (const stat of cat?.stats || []) {
                      if (stat?.name && stat?.value != null) {
                        teamSeasonStats[stat.name] = stat.value;
                      }
                    }
                  }
                } catch {
          ...
          (async () => {
            ...
            try {
              const scheduleUrl = withEspnApiKey(
                `https://site.api.espn.com/apis/site/v2/sports/soccer/${encodeURIComponent(espnLeague)}/teams/${encodeURIComponent(String(team?.id || resolvedTeamId))}/schedule`,
              );
              const scheduleJson = await fetchJsonWithTimeout(
                scheduleUrl,
                9000,
              ).catch(() => null);
              const events = Array.isArray(scheduleJson?.events)
                ? scheduleJson.events
                : [];
              const scheduleEvents = events
                .map((event) => {
                  ...
                  const statusType = String(
                    comp?.status?.type?.state || comp?.status?.type?.name || "",
                  ).toLowerCase();
                  ...
                  return {
                    ...
                    opponent: String(
                      home?.team?.id === team?.id ||
                        home?.team?.displayName === teamDisplayName
                    ...
                    status: statusType,
                    homeScore: Number(
                      typeof home?.score === "object"
                        ? (home?.score?.value ?? home?.score?.displayValue ?? 0)
                        : (home?.score ?? 0),
                    ),
                    awayScore: Number(
                      typeof away?.score === "object"
                        ? (away?.score?.value ?? away?.score?.displayValue ?? 0)
                        : (away?.score ?? 0),
                    ),
                  };
                })
              ...
              const recentResults = scheduleEvents
                .filter(
                  (row) => row.status === "post" || row.status === "final",
                )
                .slice(0, 5);

              const upcomingMatches = scheduleEvents
                .filter(
                  (row) => !(row.status === "post" || row.status === "final"),
                )
                .slice(0, 3);

              return { recentResults, upcomingMatches, scheduleEvents };
            } catch {
              return {
                recentResults: [],
                upcomingMatches: [],
          ...
          // ── Per-player season stats from ESPN athlete stats (6s timeout) ──
          Promise.race([
            (async () => {
              // Only fetch if we have numeric player IDs from ESPN
              const numericPlayers = basePlayers.filter(
                (p) => p && /^\d+$/.test(String(p.id || "")),
              );
              if (
                numericPlayers.length === 0 ||
                !resolvedId ||
                resolvedId.startsWith("name:")
              )
                return new Map();
              try {
                const season = seasonForDate(new Date());
                const baseUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${encodeURIComponent(espnLeague)}/seasons/${season}/types/1/teams/${encodeURIComponent(resolvedId)}`;
                // Fetch team leaders for goals, assists, appearances in parallel
                const [goalsJson, assistsJson] = await Promise.all([
                  fetchJsonWithTimeout(
                    `${baseUrl}/leaders?limit=50`,
                    4000,
                  ).catch(() => null),
                  // Appearances come from the same leaders endpoint under different category
                  Promise.resolve(null),
                ]);
                const statsMap = new Map();
                const processLeaders = (data, statName) => {
                  for (const cat of data?.categories || data?.leaders || []) {
                    const catName = String(
                      cat?.name || cat?.displayName || "",
                    ).toLowerCase();
                    for (const leader of cat?.leaders || []) {
                      const athleteRef =
                        leader?.athlete?.$ref || leader?.athlete?.id || "";
                      const idMatch =
                        String(athleteRef).match(/athletes\/(\d+)/);
                      const athleteId = idMatch
                        ? idMatch[1]
                        : String(leader?.athlete?.id || "");
                      if (!athleteId) continue;
                      const existing = statsMap.get(athleteId) || {};
                      const val = Number(
                        leader?.value ?? leader?.displayValue ?? 0,
                      );
                      if (
                        catName.includes("goal") &&
                        !catName.includes("against")
                      )
                        existing.goals = val;
                      else if (catName.includes("assist"))
                        existing.assists = val;
                      else if (
                        catName.includes("appearance") ||
                        catName.includes("games")
                      )
                        existing.appearances = val;
                      else if (catName.includes("start")) existing.starts = val;
                      else if (catName === "foulscommitted")
                        existing.foulsCommitted = val;
                      else if (catName.includes("yellow"))
                        existing.yellowCards = val;
                      else if (catName.includes("shot")) existing.shots = val;
                      statsMap.set(athleteId, existing);
                    }
                  }
                };
                if (goalsJson) processLeaders(goalsJson, "goals");
                if (assistsJson) processLeaders(assistsJson, "assists");
                return statsMap;
              } catch {
                return new Map();
              }
        ...
        const resolvedLogo =
          enrichResult?.resolvedLogo ??
          (rawTeamLogo || espnLogo || footballLogosUrl || null);

        // Apply standings
        let standingsRow = null;
        if (standingsResult) {
          standingsRow =
            standingsResult.byTeamId ||
            standingsResult.byTeamName ||
            standingsResult.byQueryName ||
            null;
          if (
            standingsRow &&
            !standingsResult.byTeamId &&
            ...
            teamDisplayName =
              standingsRow.team || teamNameFromQuery || teamDisplayName;
            if (standingsRow.teamId)
              resolvedTeamId = String(standingsRow.teamId);
          }
        }

        const teamSeasonStats = statsResult;
        const { recentResults, upcomingMatches, scheduleEvents } =
          scheduleResult;

        // Merge per-player season stats from ESPN leaders endpoint
        const playerStats =
          playerStatsResult instanceof Map ? playerStatsResult : new Map();

        // Sofascore squad photos are now fetched inside enrichRosterPhotosBulk via
        // findSofaTeamIdByName → fetchSofaTeamPlayerPhotos (fast path, <2s).
        // The old collectTeamLineupPhotoIndex (10+s serial) is no longer needed here.
        const finalPlayers = ensurePlayersHaveValidPhotos(baseFinalPlayers).map(
          (player) => {
            if (!player) return player;
            const pStats = playerStats.get(String(player.id || ""));
            if (!pStats) return player;
            return {
              ...player,
              seasonStats: {
                goals: pStats.goals ?? null,
                assists: pStats.assists ?? null,
                appearances: pStats.appearances ?? null,
                starts: pStats.starts ?? null,
                yellowCards: pStats.yellowCards ?? null,
              },
            };
          },
        );
        ...
        const founded =
          Number(team?.founded || team?.foundedYear || team?.foundedOn || 0) ||
          null;
        const clubColors = [team?.color, team?.alternateColor]
          .filter(Boolean)
          .map((value) => {
            const raw = String(value).replace("#", "").slice(0, 6);
            return /^[0-9a-fA-F]{3,6}$/.test(raw)
              ? `#${
                  raw.length === 3
                    ? raw
                        .split("")
        ...
        return {
          ...
          leagueName:
            leagueSlugToDisplayName(displayLeagueSlug) ||
            rosterJson?.team?.links?.[0]?.text ||
            normalizedLeagueName ||
            espnLeague,
          leagueRank: standingsRow?.rank ?? null,
          leaguePoints: standingsRow?.points ?? null,
          leaguePlayed: standingsRow?.played ?? null,
          leagueWon: standingsRow?.wins ?? null,
          leagueDrawn: standingsRow?.draws ?? null,
          leagueLost: standingsRow?.losses ?? null,
          venue: team?.venue?.fullName || team?.venue?.name || "",
          stadiumCapacity: Number(team?.venue?.capacity || 0) || null,
          country:
            leagueSlugToCountry(displayLeagueSlug) || team?.location || "",
          founded,
          clubColors,
          ...
          shotsPerGame:
            teamSeasonStats.totalShots != null &&
            (standingsRow?.played || 0) > 0
              ? parseFloat(
                  (
                    teamSeasonStats.totalShots / (standingsRow?.played || 1)
                  ).toFixed(1),
                )
              : (teamSeasonStats.shotsPerGame ??
                teamSeasonStats.avgShots ??
                null),
          possession:
            teamSeasonStats.possessionPct ?? teamSeasonStats.possession ?? null,
          recentResults,
          upcomingMatches,
        ...
        // Debug: log team response summary
        const photosWithUrl = finalPlayers.filter((p) => p?.photo).length;
        const playersWithStats = finalPlayers.filter(
          (p) =>
            p?.seasonStats?.goals != null ||
            p?.seasonStats?.appearances != null,
        ).length;
        console.log(
          `[team-detail] ${teamDisplayName}: ${finalPlayers.length} players, ${photosWithUrl} with photos, ${playersWithStats} with stats, league=${espnLeague}, id=${resolvedId}`,
        );

        return result;
      }),
      new Promise((resolve) =>
        setTimeout(() => resolve(null), TEAM_ENDPOINT_DEADLINE),
      ),
    ]);

    if (!payload) {
      // Deadline exceeded — return minimal response from request params
      console.warn(
        `[team-detail] Deadline exceeded (${TEAM_ENDPOINT_DEADLINE}ms) for team ${teamId}`,
      );
      return res.json({
        id: teamId,
        name: teamNameFromQuery || "Team",
        logo: null,
        color: "#1a3a6b",
        leagueName: espnLeague,
        players: [],
        error: "Data loading timed out — try again for cached results",
      });
    }
...
app.get("/api/sports/player/:playerId", async (req, res) => {
  ...
  let espnLeague = String(req.query?.league || "").trim();
  if (!espnLeague && teamName) {
    const autoLeagueSlug = resolveDomesticLeagueSlug(teamName);
    if (autoLeagueSlug) espnLeague = autoLeagueSlug;
    if (!espnLeague) espnLeague = "eng.1";
  }
  if (!espnLeague) espnLeague = "eng.1";
  ...
  try {
    const payload = await Promise.race([
      getOrFetch(cacheKey, 600_000, async () => {
        // ── All external fetches run in parallel ────────────────────────────
        const [espnResult, apifyFallback, githubFallback, tmDirect] =
          await Promise.all([
            // ESPN athlete profile
            (async () => {
              if (!playerId) return { athlete: null, team: null };
              try {
                const athleteResp = await fetch(
                  `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${encodeURIComponent(espnLeague)}/athletes/${encodeURIComponent(playerId)}`,
                  {
                    headers: {
                      "user-agent": "Mozilla/5.0 (Nexora/1.0)",
                      accept: "application/json",
                ...
                if (!athleteResp.ok) return { athlete: null, team: null };
                const athleteJson = await athleteResp.json();
                return {
                  athlete: athleteJson?.athlete || athleteJson || null,
                  team: athleteJson?.team || athleteJson?.athlete?.team || null,
                };
              } catch {
                return { athlete: null, team: null };
              }
            })(),
            // Apify/Transfermarkt profile fallback
            Promise.race([
              fetchApifyPlayerFallback(playerId, playerName, teamName),
              new Promise((resolve) => setTimeout(() => resolve(null), 6000)),
            ]),
            // GitHub FIFA stats fallback
            Promise.race([
              fetchGithubPlayerFallback(playerName, teamName),
              new Promise((resolve) => setTimeout(() => resolve(null), 4000)),
            ]),
            // Transfermarkt direct: needs extra time for sequential search + transfers calls
            Promise.race([
              fetchTransfermarktPlayerDirect(playerName, teamName),
              new Promise((resolve) => setTimeout(() => resolve(null), 12000)),
            ]),
          ]);

        let espnAthlete = espnResult.athlete;
        let espnTeam = espnResult.team;
        ...
        if (playerName && espnAthlete) {
          ...
          const espnResolved = normalizePersonName(
            espnAthlete?.displayName || espnAthlete?.fullName || "",
          );
          if (
            requested &&
            espnResolved &&
            similarityScore(requested, espnResolved) < 0.55
          ) {
            espnAthlete = null;
            espnTeam = null;
          }
        ...
        const normalizedPlayer = {
          ...
          age:
            Number(
              profile?.age || espnAthlete?.age || apifyFallback?.age || 0,
            ) || undefined,
          nationality:
            profile?.nationality ||
            espnAthlete?.citizenship ||
            espnAthlete?.birthCountry ||
            apifyFallback?.nationality ||
            undefined,
          position:
        ...
        const valuedFromModel =
          (
            await Promise.race([
              ...
              new Promise((resolve) =>
                setTimeout(() => resolve([normalizedPlayer]), 3500),
              ),
            ])
          )[0] || normalizedPlayer;
        ...
        if (
          tmDirect?.marketValueEur &&
          (!valued?.isRealValue || valued?.valueMethod === "estimated")
        ) {
          valued = {
            ...valued,
        ...
        const fallbackInsights = inferStrengthsWeaknesses(
          valued?.position,
          valued?.age,
        );
        ...
        const transferPool = tmTransfers.length
          ? tmTransfers
          : apiSportsTransfers.length
            ? apiSportsTransfers
            : apifyTransfers;
        const formerClubs = normalizeFormerClubEvents(transferPool || [], {
          limit: 20,
          descending: true,
        });
        ...
        const sourceTag =
          apifyFallback?.source ||
          githubFallback?.source ||
          apiSports?.source ||
          "espn";
        ...
        const appearancesRaw = toPositiveOrNull(
          profileStats?.games?.appearences ||
            profileStats?.games?.appearances ||
            apifyFallback?.appearances ||
            0,
        );
        const startsRaw = toPositiveOrNull(
          profileStats?.games?.lineups ||
            profileStats?.games?.starts ||
            apifyFallback?.starts ||
            0,
        );
        const minutesRaw = toPositiveOrNull(
          profileStats?.games?.minutes || apifyFallback?.minutes || 0,
        );
        const goalsRaw = toPositiveOrNull(
          profileStats?.goals?.total ||
            profileStats?.goals ||
            apifyFallback?.goals ||
            0,
        );
        const assistsRaw = toPositiveOrNull(
          profileStats?.goals?.assists ||
            profileStats?.passes?.assists ||
            apifyFallback?.assists ||
            0,
        );
        const ratingRaw = toPositiveOrNull(
          profileStats?.games?.rating ||
            profileStats?.rating ||
            apifyFallback?.rating ||
            0,
        );
        ...
        const derivedStarts =
          startsRaw ||
          (appearancesRaw
            ? Math.max(1, Math.round(appearancesRaw * 0.7))
            : null);
        ...
        const derivedRating =
          ratingRaw ||
          (appearancesRaw && appearancesRaw >= 3
            ? Number(
                (
                  6.2 +
                  Math.min(1.8, contributions / Math.max(appearancesRaw, 1))
                ).toFixed(2),
              )
            : null);
        ...
        const seasonStats = {
          ...
          yellowCards: toPositiveOrNull(
            profileStats?.cards?.yellow || apifyFallback?.yellowCards || 0,
          ),
          redCards: toPositiveOrNull(
            profileStats?.cards?.red || apifyFallback?.redCards || 0,
          ),
        };
        ...
        const recentForm = {
          ...
          contributionLabel:
            seasonStats.goals || seasonStats.assists
              ...
              : null,
          strengths: aiInsights?.strengths?.length
            ? aiInsights.strengths.slice(0, 3)
            : fallbackInsights.strengths.slice(0, 3),
        };
          ...
          if (Array.isArray(tmPlayers)) {
            ...
            for (const p of tmPlayers) {
              if (
                p.photo &&
                (p.name === normName ||
                  similarityScore(normName, p.name) >= 0.5)
              ) {
        ...
        if (!resolvedPhoto && clubName) {
          ...
          for (const dbp of dbPlayers) {
            if (
              dbp.name === normName ||
              similarityScore(normName, dbp.name) >= 0.5
            ) {
        ...
        // Sofascore squad cache scan — search any cached team squad for this player by name.
        // This covers youth/fringe players who lack Wikipedia articles but are in the Sofascore squad.
        // Much safer than Wikipedia which produces false positives (historical figures, wrong persons).
        if (!resolvedPhoto && name && name !== "Onbekend") {
          const normName = normalizePersonName(name);
          for (const [cacheKey, entry] of __cache.entries()) {
            if (
              !cacheKey.startsWith("sofascore_team_players_") ||
              !entry?.value
            )
              continue;
            const squad = entry.value;
            if (!Array.isArray(squad)) continue;
            for (const p of squad) {
              const pName = normalizePersonName(p.name || "");
              if (
                pName === normName ||
                similarityScore(normName, pName) >= 0.9
              ) {
                resolvedPhoto = proxyPhotoUrl(
                  `https://api.sofascore.app/api/v1/player/${encodeURIComponent(p.id)}/image`,
                );
                break;
              }
            }
            if (resolvedPhoto) break;
          }
        }
        ...
        if (!resolvedPhoto && name && name !== "Onbekend") {
          try {
            ...
            if (tsdbPhoto === null) {
              ...
              if (parts.length >= 2)
                searchQueries.push(parts[parts.length - 1]);
              for (const q of searchQueries) {
                if (tsdbPhoto) break;
                const tsdbResp = await fetch(
                ...
                const results = Array.isArray(tsdbData?.player)
                  ? tsdbData.player
                  : [];
                for (const r of results) {
                  // Only accept soccer players — reject unknown sport too
                  ...
                  const rName = normalizePersonName(r?.strPlayer || "");
                  const photo =
                    r?.strCutout || r?.strThumb || r?.strRender || null;
                  if (!photo || !/^https?:\/\//i.test(photo)) continue;
                  const nameScore = similarityScore(normName, rName);
                  if (nameScore < 0.45) continue;
                  // Also check team when available
                  const rTeam = normalizePersonName(r?.strTeam || "");
                  const normTeamName = normalizePersonName(teamName || "");
                  const teamMatch =
                    normTeamName &&
                    rTeam &&
                    similarityScore(normTeamName, rTeam) >= 0.4;
                  // For surname-only searches, require team match to avoid false positives
                  const isSurnameSearch = q !== name;
                  if (isSurnameSearch && !teamMatch) continue;
                  if (nameScore >= 0.45 || teamMatch) {
                    tsdbPhoto = photo;
                    break;
                  }
                }
        ...
        // No further fallbacks — return null so the client renders initials instead of a wrong image.
        const baseClubLogo = normalizeTeamLogo(
          clubName,
          profileStats?.team?.logo ||
            espnTeam?.logo ||
            espnTeam?.logos?.[0]?.href ||
            apifyFallback?.teamLogo ||
            null,
        );
        ...
        return {
          ...
          birthDate:
            profile?.dateOfBirth ||
            espnAthlete?.dateOfBirth ||
            apifyFallback?.birthDate ||
            null,
          nationality: valued?.nationality,
          position: valued?.position,
          height: toMetersStringFromAny(
            profile?.height ||
              espnAthlete?.displayHeight ||
              espnAthlete?.height ||
              apifyFallback?.height,
          ),
          weight: toKgStringFromAny(
            profile?.weight ||
              espnAthlete?.displayWeight ||
              espnAthlete?.weight ||
              apifyFallback?.weight,
          ),
          ...
          weaknesses: aiInsights?.weaknesses?.length
            ? aiInsights.weaknesses
            : fallbackInsights.weaknesses,
          analysis:
            aiInsights?.summary ||
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("player profile timeout")), 25_000),
      ),
    ]);

    res.json(payload);
  } catch (e) {
    // On timeout, return partial data from params so client isn't empty
    const fallbackName = playerName || "Onbekend";
    res.status(200).json({
      id: playerId || "",
      name: fallbackName,
      photo: null,
...
app.get("/api/sports/player-analysis/:playerId", async (req, res) => {
  ...
  const language =
    String(req.query?.language || "nl").toLowerCase() === "en" ? "en" : "nl";
  ...
  if (!refresh) {
    ...
    if (stored?.summary) {
      return res.json({
        ...
        provider: "cached",
        updatedAt: new Date(
          Number(stored.updatedAt || Date.now()),
        ).toISOString(),
      });
    }
  }
  ...
  try {
    ...
    const detailResp = await fetch(
      `${base}/api/sports/player/${encodeURIComponent(playerId)}?${query.toString()}`,
      {
        headers: {
          "user-agent": "Mozilla/5.0 (Nexora/1.0)",
          accept: "application/json",
  ...
  const safeProfile = {
    ...
    formerClubs: Array.isArray(playerProfile?.formerClubs)
      ? playerProfile.formerClubs
      : [],
  };

  const ai = await aiAnalyzePlayerProfile(safeProfile, {
    league,
    source: playerProfile?.source || "sports-service",
  });
  const fallback = inferStrengthsWeaknesses(
    safeProfile.position,
    safeProfile.age,
  );
  ...
  const payload = {
    ...
    strengths:
      Array.isArray(ai?.strengths) && ai.strengths.length
        ? ai.strengths.slice(0, 5)
        : fallback.strengths,
    weaknesses:
      Array.isArray(ai?.weaknesses) && ai.weaknesses.length
        ? ai.weaknesses.slice(0, 5)
        : fallback.weaknesses,
    tactical: null,
    physical: null,
    mental: null,
    transferPotential: safeProfile.marketValue
      ? String(safeProfile.marketValue)
      : null,
    language,
...
app.get("/api/sports/player-analysis-stream/:playerId", async (req, res) => {
  ...
  try {
    ...
    const detailResp = await fetch(
      `${base}/api/sports/player-analysis/${encodeURIComponent(playerId)}?${q.toString()}`,
      {
        headers: { accept: "application/json" },
...
app.get("/api/sports/team/:teamId/player-quality", async (req, res) => {
  ...
  try {
    ...
    if (req.query?.countryCode)
      query.set("countryCode", String(req.query.countryCode));

    const origin = `${req.protocol}://${req.get("host")}`;
    const detailResp = await fetch(
      `${origin}/api/sports/team/${encodeURIComponent(teamId)}?${query.toString()}`,
      {
        headers: {
          "user-agent": "Mozilla/5.0 (Nexora/1.0)",
          accept: "application/json",
    ...
    const rows = players.map((p, idx) => {
      ...
      const hasPosition = Boolean(
        String(p?.position || p?.positionName || "").trim(),
      );
      const hasAge =
        Number.isFinite(Number(p?.age || 0)) && Number(p?.age || 0) > 0;
    ...
    const summary = {
      total: rows.length,
      photoCoveragePct: rows.length
        ? Math.round((rows.filter((r) => r.photo).length / rows.length) * 100)
        : 0,
      valueCoveragePct: rows.length
        ? Math.round(
            (rows.filter((r) => r.marketValue).length / rows.length) * 100,
          )
        : 0,
      realValuePct: rows.length
        ? Math.round(
            (rows.filter((r) => r.isRealValue).length / rows.length) * 100,
          )
        : 0,
      highConfidencePct: rows.length
        ? Math.round(
...
app.get("/api/sports/multisport/scoreboard", async (req, res) => {
  try {
    ...
    if (!sport || !knownSports.includes(sport)) {
      return res
        .status(400)
        .json({ error: "Invalid sport", allowed: knownSports });
    }

    const leagueMap = ESPN_MULTISPORT_LEAGUES[sport];
    const targetLeagues = leagueParam
      ? leagueMap[leagueParam]
        ? { [leagueParam]: leagueMap[leagueParam] }
        : null
      : leagueMap;

    if (!targetLeagues) {
      return res.status(400).json({
        error: `Unknown league '${leagueParam}' for sport '${sport}'`,
        allowed: Object.keys(leagueMap),
      });
    }
    ...
    for (const r of results) {
      if (r.status === "fulfilled") {
        ...
        const mapped = events.map((ev) =>
          mapEspnMultiSportEvent(ev, sport, leagueSlug),
        );
        allEvents.push(...mapped);
        if (mapped.length)
          byLeague.push({ league: leagueName, leagueSlug, events: mapped });
      }
    }

    const live = allEvents
...
app.get("/api/sports/news", async (req, res) => {
  try {
    ...
    if (!resp.ok)
      return res
        .status(200)
        .json({ items: [], error: `ESPN news ${resp.status}` });
    const data = await resp.json();
    ...
    const items = (data?.feed || []).map((item) => ({
      ...
      categories: (item?.categories || []).map((c) =>
        String(c?.description || c?.displayName || ""),
      ),
    }));
...
app.get("/api/sports/match/:matchId/odds", async (req, res) => {
  try {
    ...
    if (!resp.ok)
      return res.json({ matchId, odds: [], error: `ESPN odds ${resp.status}` });
    const data = await resp.json();
...
app.get("/api/sports/multisport/teams", async (req, res) => {
  try {
    ...
    if (!resp.ok)
      return res
        .status(200)
        .json({ teams: [], error: `ESPN teams ${resp.status}` });
    const data = await resp.json();
    ...
    const teams = (
      ...
      return {
        ...
        logo:
          ...
          team?.logo ||
          (team?.id
            ? `https://a.espncdn.com/i/teamlogos/${logoPath}/500/${team.id}.png`
            : null),
        sport,
        league,
      };
...
app.get("/api/sports/multisport/standings", async (req, res) => {
  try {
    ...
    if (!resp.ok)
      return res
        .status(200)
        .json({ standings: [], error: `ESPN standings ${resp.status}` });
    const data = await resp.json();

    // Flatten standings groups (conferences/divisions/phases) with robust shape handling.
    const groups = extractStandingsGroupsFromEspn(data);
    const allEntries = [];
    for (const group of groups) {
      ...
      for (const entry of entries) {
        ...
        for (const stat of entry?.stats || []) {
          stats[String(stat?.abbreviation || stat?.name || "")] =
    ...
    return res
      .status(200)
      .json({ standings: [], error: String(e?.message || e) });
  }
});
...
app.get("/api/sports/multisport/game/:gameId", async (req, res) => {
  try {
    ...
    if (!gameId || !sport || !league) {
      return res
        .status(400)
        .json({ error: "gameId, sport, and league are required" });
    }
    ...
    if (!resp.ok)
      return res
        .status(200)
        .json({ gameId, sport, league, error: `ESPN summary ${resp.status}` });
    const data = await resp.json();
    ...
    return res.json({
      ...
      header: {
        ...
        odds: data?.odds?.[0]
          ? {
              details: String(data.odds[0].details || ""),
              overUnder: data.odds[0]?.overUnder ?? null,
            }
          : null,
        homeTeam: {
          ...
          linescores: (home?.linescores || []).map(
            (ls) => ls?.displayValue ?? ls?.value ?? null,
          ),
        ...
        awayTeam: {
          ...
          logo:
            ...
            (away?.team?.id
              ? `https://a.espncdn.com/i/teamlogos/${logoPath}/500/${away.team.id}.png`
              : null),
          linescores: (away?.linescores || []).map(
            (ls) => ls?.displayValue ?? ls?.value ?? null,
          ),
      ...
      videos: (data?.videos || []).slice(0, 5).map((v) => ({
        ...
        links:
          v?.links?.source?.HD?.href || v?.links?.source?.full?.href || null,
      })),
...
app.get("/api/sports/multisport/teams/:teamId", async (req, res) => {
  try {
    ...
    if (!teamId || !sport || !league) {
      return res
        .status(400)
        .json({ error: "teamId, sport, and league are required" });
    }
    ...
    if (!resp.ok)
      return res.status(200).json({
        ...
        sport,
        league,
        error: `ESPN team detail ${resp.status}`,
      });
    const data = await resp.json();
...
app.get("/api/sports/multisport/rankings", async (req, res) => {
  try {
    const sport = String(req.query.sport || "football")
      .trim()
      .toLowerCase();
    const league = String(req.query.league || "college-football")
      .trim()
      .toLowerCase();
    ...
    if (!resp.ok)
      return res.status(200).json({
        ...
        league,
        rankings: [],
        error: `ESPN rankings ${resp.status}`,
      });
    const data = await resp.json();
...
app.get("/api/sports/stream/:matchId", async (req, res) => {
  try {
    ...
    // 1. Try streamed.pk first (reliable, no Cloudflare block)
    if (homeTeam && awayTeam) {
      try {
        const spk = await resolveStreamedPkUrl(homeTeam, awayTeam);
        if (spk?.url) {
          return res.json({
            matchId,
            url: spk.url,
            embedUrl: spk.embedUrl || null,
            directStreamUrl: spk.directStreamUrl || null,
            candidates: [spk.directStreamUrl, spk.url, spk.embedUrl].filter(
              Boolean,
            ),
            officialOptions: [],
            source: spk.source,
            streamSource: spk.streamSource || null,
        ...
        serverLog.warn("streamed.pk lookup failed", {
          error: String(spkErr?.message || spkErr),
        });
      }
    }

    // 2. Last resort: streamed.pk football category page
    const fallbackUrl = "https://streamed.pk/category/football";
    return res.json({
      matchId,
      url: fallbackUrl,
    ...
    return res.status(200).json({
      matchId: String(req.params.matchId || ""),
      url: "https://streamed.pk/category/football",
      candidates: ["https://streamed.pk/category/football"],
      source: "streamed.pk",
      error: String(e?.message || e),
    });
  }
...
function classifyCategory(group, url, name) {
  ...
  if (
    /\bseries?\b|\bserie\b|seizoen|season|tv.?show|episode|tvshow|sitcom|docu.?series|mini.?series/i.test(
      g,
    ) ||
    g.includes("tv shows") ||
    g.includes("show")
...
function parseM3U(content) {
  ...
  for (const line of lines) {
    ...
    if (l.startsWith("#EXTINF")) {
      ...
      current = {
        id:
          `${attr["tvg-id"] || ""}${name || attr["tvg-name"] || ""}`.trim() ||
...
function extractTitleForSearch(name) {
  ...
  t = t.replace(/\[.*?\]|\(.*?\)/g, "");
  t = t.replace(
    /\b(4K|UHD|FHD|HD|SD|HEVC|H265|H264|AAC|AC3|x265|x264|720p|1080p|2160p|HDR|WEB-DL|BluRay|BRRip|DVDRip)\b/gi,
    "",
  );
...
async function enrichWithTmdb(parsed) {
  ...
  const needsEnrich = items.filter(
    (it) =>
      !it.poster || !it.poster.startsWith("http") || it.poster.includes("tvg"),
  );
      ...
      batch.map(async (entry) => {
        try {
          ...
          const data = await tmdb(
            `${endpoint}?query=${encodeURIComponent(entry.title)}&page=1`,
          );
          const first = (data?.results || [])[0];
          if (!first) return null;

          const poster = first.poster_path
            ? `${TMDB_IMG_500}${first.poster_path}`
            : null;
          const backdrop = first.backdrop_path
            ? `${TMDB_IMG_780}${first.backdrop_path}`
            : null;
          const year = (first.release_date || first.first_air_date || "").slice(
            0,
            4,
          );
          const rating = first.vote_average
            ? Number(Number(first.vote_average).toFixed(1))
            : 0;
...
function channelNameFromUrl(rawUrl) {
  try {
    ...
    const tail = decodeURIComponent(
      u.pathname.split("/").filter(Boolean).pop() || "Live Stream",
    );
    return (
      tail
...
app.get("/api/user/followed-teams", (req, res) => {
  try {
    ...
    if (!deviceId)
      return res
        .status(400)
        .json({ error: "Missing or invalid X-Device-Id header" });
...
app.get("/api/movies/:id/providers", tmdbLimiter, async (req, res) => {
  try {
    ...
    if (cached) return res.json(cached);
    const data = await tmdb(
      `/movie/${encodeURIComponent(id)}/watch/providers`,
      { cacheTtlMs: 0 },
    );
...
app.get("/api/series/:id/providers", tmdbLimiter, async (req, res) => {
  try {
    ...
    const cached = await cacheGet(cacheKey);
    if (cached) return res.json(cached);
    const data = await tmdb(`/tv/${encodeURIComponent(id)}/watch/providers`, {
      cacheTtlMs: 0,
    });
    const payload = { results: data?.results ?? {} };
    await cacheSet(cacheKey, payload, TMDB_PROVIDERS_CACHE_TTL_MS);
  ...
  try {
    ...
    const shows = Array.isArray(data)
      ? data.slice(0, 10).map((item) => ({
          ...
          network:
            item.show?.network?.name || item.show?.webChannel?.name || null,
        }))
      : [];
...
app.get("/api/radio/top", async (req, res) => {
  try {
    ...
    if (cached) return res.json(cached);
    const r = await fetch(
      `${RADIO_BROWSER_BASE}/stations/topvote/${limit}?hidebroken=true`,
      {
        signal: AbortSignal.timeout(8000),
        headers: { "User-Agent": "Nexora/1.0", accept: "application/json" },
      },
...
app.get("/api/weather", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "lat and lon are required (valid coordinates)" });
    }
    ...
    const r = await fetch(`${OPEN_METEO_BASE}/forecast?${params}`, {
      signal: AbortSignal.timeout(8000),
    });
...
function logStartupConfigCheck() {
  ...
  if (!process.env.TMDB_API_KEY) {
    ...
    console.warn(
      "    All /api/movies/* and /api/series/* endpoints will return empty arrays.",
    );
    console.warn(
  ...
  if (
    ...
    optional.push(
      "AI keys (GEMINI_API_KEY / OPENROUTER_API_KEY / XAI_API_KEY / etc.)  → AI analysis disabled",
    );
  }
  if (!process.env.APIFY_TOKEN) {
    optional.push(
      "APIFY_TOKEN  → player/team enrichment via Sofascore disabled",
    );
  }
  ...
  if (optional.length > 0) {
    console.info(
      "[nexora-server] Optional keys not set (non-critical):\n  " +
        optional.join("\n  "),
    );
  }
}
...
app.listen(PORT, () => {
  serverLog.info(`Nexora server running on :${PORT}`, {
    sportsSource: footballSource(),
    port: PORT,
  });
  logStartupConfigCheck();

  // Flush stale TheSportsDB caches that may contain cross-sport (baseball/basketball) results.
  // These were cached before the strSport === "Soccer" filter was added.
  let flushed = 0;
  for (const key of __cache.keys()) {
    if (
      key.startsWith("tsdb_player_") ||
      key.startsWith("thesportsdb_team_") ||
      key.startsWith("thesportsdb_players_")
    ) {
      __cache.delete(key);
      flushed++;
    }
  ...
  if (flushed > 0)
    console.log(`[startup] Flushed ${flushed} stale TheSportsDB cache entries`);
  ...
  // Startup sports cache warmup — fire immediately so ESPN data is ready
  // before the first user request. Disk cache (sports.js) will serve
  // previous-run data instantly; this refreshes it in the background.
  setImmediate(async () => {
    try {
      const today = new Date().toISOString().slice(0, 10);
      await Promise.allSettled([
        fetch(`http://127.0.0.1:${PORT}/api/sports/by-date?date=${today}`, {
          signal: AbortSignal.timeout(30_000),
        }),
        fetch(`http://127.0.0.1:${PORT}/api/sports/live?date=${today}`, {
          signal: AbortSignal.timeout(30_000),
        }),
      ]);
      serverLog.info("Sports cache warmed on startup", { date: today });
    } catch (e) {
      serverLog.warn("Startup sports warmup failed", { message: e.message });
    }
  });
  ...
  async function runEnrichmentCycle() {
    try {
      ...
      for (const name of teamNames) {
        const league =
          matches.find((m) => m.homeTeam === name || m.awayTeam === name)
            ?.league || "";
      ...
      console.log(
        `[enrichment] Warmed caches for ${teamNames.length} teams, ${uniqueTeams.length} rosters`,
      );
    } catch (e) {
