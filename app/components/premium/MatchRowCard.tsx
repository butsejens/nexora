...
const ActionBtn = memo(function ActionBtn({
  ...
  return (
    ...
    >
      ...
      />
      <Text style={[s.actionBtnText, primary && { color: "#fff" }]}>
        {label}
      </Text>
    </TouchableOpacity>
  );
...
function MatchRowCardInner({
  ...
  const stateLabel = live
    ? `${(match as any).clock && String((match as any).clock).includes("+") ? (match as any).clock : (match.minute ?? 0)}'`
    : finished
      ? tFn("common.ft")
          ...
          <View style={[s.metaRow, compact && s.metaRowCompact]}>
            ...
            {onNotificationToggle ? (
              <TouchableOpacity
                style={[
                  s.notifyQuickBtn,
                  compact && s.notifyQuickBtnCompact,
                  isNotificationOn ? s.notifyQuickBtnActive : null,
                ]}
              ...
              >
                <Ionicons
                  name={
                    isNotificationOn ? "notifications" : "notifications-outline"
                  }
                  size={14}
          ...
          <View style={[s.row, compact && s.rowCompact]}>
            <View style={[s.teamCol, compact && s.teamColCompact]}>
              <Text
                style={[
                  s.teamName,
                  compact && s.teamNameCompact,
                  homeWin && s.teamNameBold,
                ]}
              ...
              {!upcoming && (match.redCards?.home ?? 0) > 0 ? (
                <Text style={s.cards}>
                  {"🟥".repeat(Math.min(match.redCards?.home ?? 0, 3))}
                </Text>
              ) : null}
            </View>
            ...
            <View style={[s.center, compact && s.centerCompact]}>
              ...
              <Text
                style={[
                  s.centerCompetitionText,
                  compact && s.centerCompetitionTextCompact,
                ]}
                numberOfLines={compact ? 1 : 2}
              >
              ...
              {!upcoming ? (
                <>
                  ...
                  <View style={[s.statusRow, compact && s.statusRowCompact]}>
                    ...
                    <Text
                      style={[
                        s.statusBelow,
                        compact && s.statusBelowCompact,
                        live ? s.statusBelowLive : null,
                        finished ? s.statusBelowFinished : null,
                      ]}
                    >
                      {stateLabel}
                    </Text>
                ...
                <>
                  <Text
                    style={[s.kickoffLabel, compact && s.kickoffLabelCompact]}
                  >
                    {tFn("common.upcoming")}
                  </Text>
                  <Text style={[s.kickoff, compact && s.kickoffCompact]}>
                    {stateLabel}
                  </Text>
                </>
              )}
            ...
            <View style={[s.teamCol, compact && s.teamColCompact]}>
              ...
              {!upcoming && (match.redCards?.away ?? 0) > 0 ? (
                <Text style={s.cards}>
                  {"🟥".repeat(Math.min(match.redCards?.away ?? 0, 3))}
                </Text>
              ) : null}
            </View>
          </View>
