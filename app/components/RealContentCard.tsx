...
export const RealContentCard = React.memo(function RealContentCard({
  item,
  onPress,
  onFavorite,
  isFavorite,
  ...
  const imdbScore = Number(item.imdbRating || 0);
  const hasImdb = Number.isFinite(imdbScore) && imdbScore > 0;
  const tmdbScore = Number(item.imdb || item.rating || 0);
        ...
        >
          {item.poster && !imageError ? (
            <>
              {!imageLoaded && (
                <Animated.View
                  style={[StyleSheet.absoluteFill, { opacity: shimmerAnim }]}
                >
                  <LinearGradient
                    colors={[
            ...
            <>
              <LinearGradient
                colors={[
                  ...
                  `${item.color || COLORS.cardElevated}BB`,
                  COLORS.background,
                ]}
                style={StyleSheet.absoluteFill}
                start={{ x: 0.2, y: 0 }}
                ...
                {!!item.quality && (
                  <View
                    style={[
                      ...
                      {
                        borderColor:
                          QUALITY_COLORS[item.quality] || COLORS.accent,
                      },
                    ]}
                  >
                    <Text
                      style={[
                        styles.qualityText,
                        {
                          color: QUALITY_COLORS[item.quality] || COLORS.accent,
                        },
                      ]}
                    >
                      {item.quality}
              ...
              <View style={styles.progressTrack}>
                <View
                  style={[
                    ...
                    {
                      width:
                        `${Math.round((item.progress || 0) * 100)}%` as any,
                    },
                  ]}
                />
              </View>
...
export const RealHeroBanner = React.memo(function RealHeroBanner({
  item,
  onPlay,
  onInfo,
  trailerKey,
        ...
        <View style={[styles.heroBanner, { height: heroHeight }]}>
          {backdropUri && !imageError ? (
            ...
            <LinearGradient
              colors={[
                item.color || COLORS.card,
                `${item.color || COLORS.cardElevated}CC`,
                COLORS.background,
              ]}
              style={StyleSheet.absoluteFill}
              start={{ x: 0.3, y: 0 }}
          ...
          {showTrailer && trailerKey && (
            ...
            >
              <ExpoImage
                source={{
                  uri: `https://img.youtube.com/vi/${trailerKey}/maxresdefault.jpg`,
                }}
                style={StyleSheet.absoluteFill}
                contentFit="cover"
                cachePolicy="memory-disk"
          ...
          <View style={styles.heroContent}>
            ...
            <View style={styles.metaRow}>
              ...
              {item.imdbRating ? (
                <View style={styles.ratingRow}>
                  <Ionicons name="film" size={11} color="#FFB81C" />
              ...
              {item.duration ? (
                <Text style={styles.heroMeta}>{item.duration}</Text>
              ) : null}
              {item.seasons ? (
                <Text style={styles.heroMeta}>{item.seasons}S</Text>
              ) : null}
            </View>
...
const styles = StyleSheet.create({
  ...
  tomato: { fontSize: 12, marginRight: 1 },
