...
import {
  resolveMatchCompetitionLabel,
  resolveMatchEspnLeagueCode,
} from "@/lib/sports-competition";
import {
  resolveCompetitionBrand,
  resolveTeamLogoUri,
} from "@/lib/logo-manager";
import { resolveEspnLeagueCodeFromConfig } from "@/lib/competition-league-config";
import {
  detectLocaleSignals,
  searchCompetitions,
  searchTeams,
...
interface SearchResult {
  ...
  title: string;
  subtitle?: string;
  type: "team" | "player" | "match" | "series" | "movie" | "competition";
  image?: string;
  sport?: string;
  poster?: string;
  rating?: number;
...
export function SearchTab({ onSelectResult }: SearchTabProps) {
      ...
      try {
        ...
        const matchArrays = await Promise.all(
          dateOffsets.map(async (offset) => {
            ...
            try {
              const response = await apiRequestJson<any>(
                `/api/sports/by-date?date=${encodeURIComponent(dateStr)}`,
              );
              return [
        ...
        const competitionsById = new Map<
          string,
          { id: string; name: string; espnLeague: string; sportKey: string }
        >();
        ...
        const playerResults = await Promise.allSettled(
          PLAYER_LEAGUES.map((league) =>
            apiRequestJson<any>(
              `/api/sports/topscorers/${encodeURIComponent(league)}`,
            ),
          ),
        );
        const players: {
        ...
        playerResults.forEach((result, i) => {
          ...
          const scorers: any[] =
            result.value?.scorers || result.value?.players || [];
          const espnLeague = PLAYER_LEAGUES[i];
          scorers.forEach((s: any) => {
            const name = String(
              s?.name || s?.displayName || s?.shortName || "",
            ).trim();
            ...
            players.push({
              ...
              espnLeague,
              photo:
                String(s?.photo || s?.headshot || s?.image || "").trim() ||
                undefined,
            });
          });
        });
  ...
  const curatedSportsItems = useMemo(() => {
    ...
    const competitions = searchCompetitions(
      debouncedQuery,
      ["football"],
      localeSignals,
      40,
    ...
    const teamItems: SearchResult[] = teams.map((team) => ({
      ...
      espnLeague: String(
        (team as any)?.espnLeague ||
          resolveEspnLeagueCodeFromConfig({
            name: String(team.competition || ""),
          }) ||
          "",
      ),
    }));

    const competitionItems: SearchResult[] = competitions.map(
      (competition) => ({
  ...
  const performSearch = useCallback(
    ...
    [sportsItems, curatedSportsItems, mediaItems],
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: TOP_NAV_H + 10 }]}>
