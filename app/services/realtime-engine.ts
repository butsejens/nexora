...
import {
  logRealtimeEvent,
  measureRealtimeTask,
} from "@/services/realtime-telemetry";
...
export const realtimeCacheKeys = {
  ...
  matchDetail: (matchId: string, league: string) =>
    `sports:match-detail:${matchId}:${league}`,
};

export const realtimePolicies = {
  sportsLive: {
    ttlMs: CacheTTL.LIVE_MATCH,
    staleTime: 8_000,
    refetchInterval: 8_000,
  },
  sportsSchedule: {
    ttlMs: CacheTTL.TODAY_SPORTS,
    staleTime: 45_000,
    refetchInterval: 45_000,
  },
  sportsHighlights: {
    ttlMs: CacheTTL.MATCH_DETAIL,
    staleTime: 60_000,
...
function prefetchImages(urls: string[]) {
  ...
  void Promise.allSettled(uniqueUrls.map((url) => Image.prefetch(url))).then(
    (results) => {
...
async function fetchWithPersistentCache<T>(
  config: RealtimeQueryConfig<T>,
): Promise<T> {
  ...
  try {
    const data = await measureRealtimeTask(
      "fetch",
      config.label,
      config.fetcher,
      {
...
export function seedRealtimeQueryFromCache<T>(
...
export async function prefetchRealtimeQuery<T>(
  queryClient: QueryClient,
  config: RealtimeQueryConfig<T>,
) {
...
export function buildHomeSportsPayload(
  payloads: Array<SportsPayload | null | undefined>,
): SportsPayload {
  const live = uniqueItemsByKey(
    ...
    (item, index) =>
      String(
        item?.id ||
          `${item?.homeTeam || "home"}_${item?.awayTeam || "away"}_${item?.startDate || index}`,
      ),
  );
...
export async function fetchHomeSportsSnapshot(
  date: string,
): Promise<SportsPayload> {
...
export async function fetchHighlightsSnapshot(): Promise<any[]> {
  const payload = await apiRequestJson<HighlightsPayload>(
    "/api/sports/highlights",
  );
  return Array.isArray(payload?.highlights) ? payload.highlights : [];
}

export function deriveCuratedHomeMedia(
  vodHome: VodHomePayload | null | undefined,
) {
  const trendingMovies = Array.isArray(vodHome?.trendingMovies)
    ? vodHome.trendingMovies
    : [];
  const trendingSeries = Array.isArray(vodHome?.trendingSeries)
    ? vodHome.trendingSeries
    : [];
  const recentMovies = Array.isArray(vodHome?.recentMovies)
    ? vodHome.recentMovies
    : [];
  const recentSeries = Array.isArray(vodHome?.recentSeries)
...
export function buildContinueWatchingRows(
  ...
  const baseHistory =
    Array.isArray(syncedContinueWatching) && syncedContinueWatching.length > 0
      ? syncedContinueWatching
      : history;
  const movieRows = createContinueWatching(baseHistory as any, "movie", limit);
  const seriesRows = createContinueWatching(
    baseHistory as any,
    "series",
    limit,
  );
  return [...movieRows, ...seriesRows].slice(0, limit).map((item: any) =>
    enrichVodModuleItem({
      ...item,
      type: item.season ? "series" : item.type || "movie",
    }),
  );
}
...
export async function primeBootstrapRealtimeData(
  queryClient: QueryClient,
  today: string,
  options?: { networkPrefetch?: boolean },
) {
  const networkPrefetch = options?.networkPrefetch === true;
  const configs = [
    {
  ...
  // Startup should stay lightweight: only fetch from network when explicitly enabled.
  if (!networkPrefetch) return;

  await Promise.allSettled(
    configs.map((config) =>
      prefetchRealtimeQuery(queryClient, config as RealtimeQueryConfig<any>),
    ),
  );
}
...
export function buildHomeSportsQuery(date: string, enabled: boolean) {
  return buildRealtimeQueryOptions<SportsPayload>({
    ...
    ttlMs: realtimePolicies.sportsSchedule.ttlMs,
    staleTime: realtimePolicies.sportsLive.staleTime,
    refetchInterval: (data: SportsPayload | undefined) => {
      const liveCount = data?.live?.length ?? 0;
      return liveCount > 0 ? 8_000 : 45_000;
    },
    enabled,
...
export function buildVodHomeQuery(enabled: boolean) {
  return buildRealtimeQueryOptions<VodHomePayload>({
    ...
    fetcher: getVodHomePayload,
    shouldPersist: (data) =>
      Array.isArray(data?.allItems) && data.allItems.length > 0,
    collectImageUrls: collectVodArtwork,
  });
}

export function buildVodCatalogRootQuery(enabled: boolean) {
  return buildRealtimeQueryOptions<
    Awaited<ReturnType<typeof getVodCatalogChunk>>
  >({
...
export function buildVodCollectionsQuery(enabled: boolean) {
  return buildRealtimeQueryOptions<
    Awaited<ReturnType<typeof getVodCollections>>
  >({
    queryKey: mediaKeys.vodCollections(),
    cacheKey: realtimeCacheKeys.vodCollections(),
    ...
    collectImageUrls: (collections) =>
      collections
        .flatMap((entry) => [entry.poster || null, entry.backdrop || null])
        .filter(Boolean) as string[],
  });
}
...
export async function fetchFollowedMatchSnapshot(
  matchId: string,
  espnLeague: string,
) {
