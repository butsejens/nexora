...
import { getVodStudioById } from "@/lib/services/media-service";
...
function toMediaId(item: Partial<VodModuleItem>): string {
  return String(item.tmdbId || item.id || "").trim();
}

export default function StudioPage() {
  const insets = useSafeAreaInsets();
  const { id, name } = useLocalSearchParams<{ id?: string; name?: string }>();
  const studioId = String(id || "").trim();

  const studioQuery = useQuery({
    queryKey: ["media", "studio-page", studioId],
    queryFn: () => getVodStudioById(studioId),
    enabled: Boolean(studioId),
    staleTime: 10 * 60 * 1000,
  });

  const items = useMemo(() => {
    const list = Array.isArray(studioQuery.data?.items)
      ? (studioQuery.data.items as Partial<VodModuleItem>[])
      : [];
  ...
  const headerTitle =
    String(name || "").trim() ||
    String(studioQuery.data?.studio?.name || "Studio");
