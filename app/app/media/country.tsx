...
export default function CountryPage() {
  ...
  const { countryCode, countryName, languageCode, languageName, type } = useLocalSearchParams<{
    countryCode: string;
    countryName: string;
    languageCode?: string;
    languageName?: string;
  ...
  const initialFilter: Filter =
    type === "movie" ? "movie" : type === "series" ? "series" : "all";
  const [filter, setFilter] = useState<Filter>(initialFilter);
  const country = String(languageCode || countryCode || "").trim().toLowerCase();

  const { data: movies = [], isFetching: loadingMovies } = useCountryMovies(
    country || null,
  );
  const { data: series = [], isFetching: loadingSeries } = useCountrySeries(
    country || null,
  );
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>
          {languageName || countryName ? `${languageName || countryName}` : country || "Taal"}
        </Text>
