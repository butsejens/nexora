...
async function fetchPublicVodSeries() {
  const sources = [
  ...
  const settled = await Promise.allSettled(
    sources.map(async (url) => {
      try {
        ...
        console.warn(`[Series] VOD fetch failed for ${url}:`, e);
        return { series: [] };
      }
    })
  );
  ...
  for (const entry of settled) {
    if (entry.status === "fulfilled") {
      series.push(...(entry.value?.series || []));
    }
  }
...
export default function SeriesScreen() {
  ...
  const publicVodQuery = useQuery({
    queryKey: ["series", "public-vod"],
    queryFn: fetchPublicVodSeries,
  ...
  const filteredIptv = useMemo(() => {
    ...
    return list.map(c => ({
      ...
      year: c.year, imdb: c.rating, genre: [], quality: "HD", seasons: (c as any).seasons || 1,
      isIptv: true, streamUrl: c.url, tmdbId: (c as any).tmdbId,
      color: COLORS.card,
    }));
  ...
  const publicVodMapped = useMemo(() => {
    ...
    return items.map((c: any, idx: number) => ({
      id: `public-series-${idx}-${c.id || c.name || "item"}`,
...
const styles = StyleSheet.create({
  ...
  brandTile: {
    ...
    minHeight: 60,
    borderRadius: 12,
    backgroundColor: "rgba(10, 24, 57, 0.85)",
    borderWidth: 1,
    borderColor: "rgba(160,194,241,0.25)",
    alignItems: "center",
