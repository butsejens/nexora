...
export function getApiBaseCandidates(): string[] {
  if (/:8082(?:\/|$)/.test(lastWorkingApiBase)) {
    // Expo runs on 8082 — never treat it as the API server
    lastWorkingApiBase = "";
  }
  // In dev, don't cache a cloud/production URL as last-working: a brief local
  // server restart can pin the app to production, hiding all local data.
  if (
    __DEV__ &&
    (isRenderUrl(lastWorkingApiBase) ||
      isCloudflareSportsUrl(lastWorkingApiBase))
  ) {
    lastWorkingApiBase = "";
  }
  const explicit = normalizeBase(process.env.EXPO_PUBLIC_API_BASE || "");
  const explicitList = parseEnvBaseList(
    process.env.EXPO_PUBLIC_API_BASES || "",
  );
  const inferredHost = getInferredNativeHost();
  const inferredNative = shouldUseInferredNativeHost(inferredHost)
    ? `http://${inferredHost}:8080`
    : "";
  ...
  if (Platform.OS !== "web") {
    ...
    if (!__DEV__) {
      const safeExplicitList = explicitList.filter(
        (candidate) => !isLoopbackHost(candidate),
      );
      const safeExplicit =
        explicit && !isLoopbackHost(explicit) ? explicit : "";
      return unique([
        lastWorkingApiBase,
        safeExplicit,
        ...safeExplicitList,
    ...
    // Physical devices should prefer inferred LAN host first in dev when
    // available, then non-loopback explicit overrides, then cloud fallback.
    if (isPhysicalDeviceSession) {
      const nonLoopbackExplicit = explicitList.filter(
        (candidate) => !isLoopbackHost(candidate),
      );
      const safeExplicit =
        explicit && !isLoopbackHost(explicit) ? explicit : "";
    ...
    // In dev, prefer local candidates (simulator/emulator) before production.
    // This ensures that after a local server restart the app switches back to
    // local instead of staying pinned to the production fallback.
    return unique([
      lastWorkingApiBase,
      ...explicitList,
      inferredNative,
  ...
  if (Platform.OS === "web") {
    ...
    if (typeof window !== "undefined" && window.location?.origin) {
      // DEV QUALITY-OF-LIFE:
      // When running Expo Web on any local dev port while the Node API runs
      // on :8080, same-origin points to the web bundler, not the API.
      // Detect ANY local port that isn't :8080 and inject :8080 as a
      // candidate so the local API server is always reachable.
      ...
      try {
        ...
        const isNotApiPort = port !== "8080" && port !== "";
        if (isLocal && isNotApiPort) {
    ...
    return unique([lastWorkingApiBase, ...candidates, DEFAULT_RENDER_API_BASE]);
  }
...
export function getSportsApiBaseCandidates(): string[] {
  ...
  if (
    ...
    (isRenderUrl(lastWorkingSportsApiBase) ||
      isCloudflareSportsUrl(lastWorkingSportsApiBase))
  ) {
    lastWorkingSportsApiBase = "";
  }
...
function getBaseCandidatesForRoute(route: string): string[] {
  return isSportsRoute(route)
    ? getSportsApiBaseCandidates()
    : getApiBaseCandidates();
}
...
// Local URLs (http) — connection refused is instant but some endpoints (e.g.
// /api/movies/:id/full) make multiple sequential external API calls that can
...
function timeoutForUrl(url: string, isSports: boolean = false): number {
  if (__DEV__) {
    if (isSports) {
      ...
      if (isRenderUrl(url)) return 9000;
      return 8000;
    }
    if (isRenderUrl(url)) return 12000;
    return 10000;
  }
...
export async function apiRequest(
  ...
  const isSports = isSportsRoute(route);
  const totalBudgetMs = requestBudgetForRoute(route, isSports);
  let attempt = 0;

  for (const baseUrl of baseUrls) {
    if (Date.now() - startedAt >= totalBudgetMs) {
      lastError = new Error(
        `[nexora] request budget exceeded (${totalBudgetMs}ms) for ${route}`,
      );
      break;
    }
    attempt += 1;
...
export async function apiRequestJson<T>(
  ...
  const inflight = inflightJsonRequests.get(requestKey) as
    | Promise<T>
    | undefined;
...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey }) => {
    ...
    const startedAt = Date.now();
    const totalBudgetMs = requestBudgetForRoute(path, isSports);
    if (baseUrls.length === 0) {
      throw new Error(
        "API base URL is not configured. Set EXPO_PUBLIC_API_BASE or run the app in an environment where the host can be inferred (e.g. Expo simulator).",
      );
...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      ...
      // Mark data as stale after 5 minutes to allow background refetches
      // while keeping a responsive feel. Per-query overrides still work.
      staleTime: 5 * 60 * 1000,
      // Keep inactive queries alive in memory for 10 minutes.
      // Balances RAM pressure with navigation-remount cost on mobile.
      gcTime: 10 * 60 * 1000,
      retry: 1,
