...
const FRANCHISE_FALLBACKS = [
  {
    ...
    terms: ["star wars", "mandalorian", "book of boba fett", "ahsoka", "andor"],
  },
  {
    key: "harry-potter",
    label: "Harry Potter Collection",
    terms: ["harry potter", "fantastic beasts", "hogwarts"],
  },
  {
    key: "fast-furious",
    label: "Fast & Furious Collection",
    terms: ["fast & furious", "fast and furious"],
  },
  {
    key: "lord-rings",
    label: "The Lord of the Rings Collection",
    terms: ["lord of the rings", "the hobbit"],
  },
  {
    key: "mission-impossible",
    label: "Mission: Impossible Collection",
    terms: ["mission: impossible", "mission impossible"],
  },
  ...
  {
    key: "jurassic",
    label: "Jurassic Collection",
    terms: ["jurassic park", "jurassic world"],
  },
  {
    key: "transformers",
    label: "Transformers Collection",
    terms: ["transformers", "bumblebee"],
  },
  {
    key: "pirates",
    label: "Pirates Of The Caribbean Collection",
    terms: ["pirates of the caribbean", "jack sparrow"],
  },
  {
    key: "james-bond",
    label: "James Bond Collection",
    terms: ["james bond", "007"],
  },
];
  ...
  return Boolean(
    ...
    (String(item.originalLanguage || "").toLowerCase() === "ja" ||
      normalizedKeywords.some(
        (keyword) => keyword.includes("anime") || keyword.includes("manga"),
      )),
  );
}

export function enrichVodModuleItem(
  baseItem: any,
  detail?: any,
): VodModuleItem {
  const mergedGenre =
    detail?.genre && Array.isArray(detail.genre) && detail.genre.length
      ? detail.genre
      : Array.isArray(baseItem.genre)
        ? baseItem.genre
        : [];

  const rawId = baseItem.id ?? detail?.id ?? "";
  const idObj = rawId && typeof rawId === "object" ? rawId : null;
  const resolvedId = idObj
    ? String(idObj.tmdbId || idObj.id || "")
    : String(rawId || "");
  const resolvedTmdbId =
    Number(
      (idObj ? idObj.tmdbId : null) ||
        baseItem.tmdbId ||
        detail?.tmdbId ||
        detail?.id ||
        0,
  ...
  return {
    ...
    genre: mergedGenre,
    genreIds:
      Array.isArray(detail?.genreIds) && detail.genreIds.length
        ? detail.genreIds
        : baseItem.genreIds || [],
    keywords: Array.isArray(detail?.keywords) ? detail.keywords : [],
    originalLanguage:
      detail?.originalLanguage ?? baseItem.originalLanguage ?? null,
    collection: detail?.collection ?? baseItem.collection ?? null,
    productionCompanies: Array.isArray(detail?.productionCompanies)
      ? detail.productionCompanies
      : baseItem.productionCompanies || [],
    studios: Array.isArray(detail?.studios)
      ? detail.studios
      : baseItem.studios || [],
    ...
    imdbRating: detail?.imdbRating ?? baseItem.imdbRating ?? null,
    imdbVotes: detail?.imdbVotes ?? baseItem.imdbVotes ?? null,
    rottenTomatoesRating:
      detail?.rottenTomatoesRating ?? baseItem.rottenTomatoesRating ?? null,
    metacriticScore:
      detail?.metacriticScore ?? baseItem.metacriticScore ?? null,
  };
}
...
export function categorizeVodItem(item: VodModuleItem): string[] {
  ...
  const imdbScore = Number(item.imdbRating || item.imdb || 0);
  const legacyScore = Number(item.rating || 0);
  const primaryScore =
    rtScore > 0 ? rtScore / 10 : imdbScore > 0 ? imdbScore : legacyScore;
  if (primaryScore >= 7.8) categories.add("Top Rated");
  if (primaryScore >= 7.0 && primaryScore < 7.8 && !item.isTrending)
  ...
  if (!categories.size)
    categories.add(item.type === "series" ? "Series" : "Movies");
...
function resolveFallbackCollection(
  item: VodModuleItem,
  ...
  const matchesTerm = (text: string, term: string): boolean => {
    ...
    // Phrase/word-boundary style match to avoid substring false positives,
    // e.g. "thor" should not match inside "thornberrys".
    return ` ${text} `.includes(` ${normalized} `);
  };
  for (const franchise of FRANCHISE_FALLBACKS) {
    if (franchise.terms.some((term) => matchesTerm(haystack, term))) {
      return { key: franchise.key, name: franchise.label };
    }
  }

  // Pull in more relevant Marvel titles (especially series) even when the
  // title itself doesn't contain a franchise keyword.
  ...
  if (
    companyHaystack.includes("marvel studios") ||
    companyHaystack.includes("marvel television") ||
    companyHaystack.includes("marvel")
  ) {
    return { key: "marvel", name: "Marvel Collection" };
  }

  return null;
}

function shouldKeepCollectionItem(
  groupName: string,
  item: VodModuleItem,
  source: "tmdb" | "fallback",
): boolean {
  // Keep TMDB collection parts untouched; this only cleans up fuzzy fallback groups.
  if (source !== "fallback") return true;

  const normalizedGroup = normalizeText(groupName);
  const year = toYear(item.year);
  const genreHaystack = normalizeText((item.genre || []).join(" "));
  const keywordHaystack = normalizeText((item.keywords || []).join(" "));
  const isAnimated =
    genreHaystack.includes("animation") ||
    keywordHaystack.includes("animated") ||
    keywordHaystack.includes("cartoon");

  // Remove legacy animated entries from Marvel fallback collections
  // (e.g. very old cartoons) while keeping modern/live-action series.
  if (
    normalizedGroup.includes("marvel") &&
    item.type === "series" &&
    isAnimated &&
    (year || 0) < 1995
  ) {
...
export function buildCollectionGroups(
  ...
  uniqueItems(items).forEach((item) => {
    ...
    if (!shouldKeepCollectionItem(collection.name, item, collection.source))
      return;
    ...
    const year = toYear(item.year);
    existing.fromYear =
      existing.fromYear == null
        ? year
        : Math.min(existing.fromYear, year || existing.fromYear);
    existing.toYear =
      existing.toYear == null
        ? year
        : Math.max(existing.toYear, year || existing.toYear);
  });
  ...
  return Array.from(groups.values())
    .map((group) => ({
      ...
      items: [...group.items].sort(
        (left, right) => parseReleaseDate(left) - parseReleaseDate(right),
      ),
    ...
    .sort((left, right) => {
      ...
      return (
        (parseReleaseDate(left.items[0]) || 0) -
        (parseReleaseDate(right.items[0]) || 0)
      );
    });
}

function studioPriority(name: string): number {
  const directIndex = PRIORITY_STUDIOS.findIndex(
    (value) => normalizeText(value) === normalizeText(name),
  );
...
export function buildStudioGroups(items: VodModuleItem[]): VodStudioGroup[] {
  ...
  uniqueItems(items).forEach((item) => {
    const companies =
      Array.isArray(item.productionCompanies) && item.productionCompanies.length
  ...
  return Array.from(groups.values())
    ...
    .sort((left, right) => {
      const priorityDelta =
        studioPriority(left.name) - studioPriority(right.name);
      if (priorityDelta !== 0) return priorityDelta;
      return right.itemCount - left.itemCount;
    });
}

export function filterBySearchFilter(
  items: VodModuleItem[],
  filter: VodSearchFilter,
): VodModuleItem[] {
  switch (filter) {
    case "movie":
...
export function pickFeaturedItem(items: VodModuleItem[]): VodModuleItem | null {
  ...
  return (
    [...items].sort((left, right) => {
      const leftScore =
        Number(left.rating || left.imdb || 0) + (left.isTrending ? 2 : 0);
      const rightScore =
        Number(right.rating || right.imdb || 0) + (right.isTrending ? 2 : 0);
