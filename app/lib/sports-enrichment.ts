...
function isGoodText(value: unknown): boolean {
  ...
  if (
    ["n/a", "na", "not available", "unknown", "none", "null", "-"].includes(
      text,
    )
  )
    return false;
...
function toFormattedHeight(value: unknown): string | null {
  ...
  if (raw.includes("m")) {
    ...
    if (meters != null && meters >= 1.3 && meters <= 2.5)
...
function toFormattedWeight(value: unknown): string | null {
  ...
  if (!raw) return null;
  const numeric = parseNumber(raw);
  if (numeric != null && numeric >= 40 && numeric <= 180)
...
function parseMarketValue(value: unknown): number {
  const text = stripHtmlArtifacts(value)
    .toLowerCase()
    .replace(/€/g, "")
    .replace(/\s+/g, "");
  if (!text) return 0;
...
function collectLeaderRows(
  ...
  for (const row of rawRows) {
    ...
    const team = normalizeText(
      row?.team || row?.teamName || row?.club?.name,
    ).toLowerCase();
    const key = `${name}|${team}`;
    const score = extractLeaderMetric(row, kind) ?? -1;
    const previous = deduped.get(key);
    const previousScore = previous
      ? (extractLeaderMetric(previous, kind) ?? -1)
      : -1;
    const previousHasPhoto = Boolean(
      previous?.photo ||
      previous?.image ||
      previous?.headshot ||
      previous?.theSportsDbPhoto,
    );
    ...
    if (
      !previous ||
      score > previousScore ||
      (score === previousScore && currentHasPhoto && !previousHasPhoto)
    ) {
      deduped.set(key, row);
    }
  }
...
function enrichLeaderRow(
  ...
  return {
    ...
    goals:
      kind === "topscorers"
        ? (statValue ?? row?.goals ?? null)
        : (row?.goals ?? null),
    assists:
      kind === "topassists"
        ? (statValue ?? row?.assists ?? null)
        : (row?.assists ?? null),
    enrichment: {
      confidence,
...
function enrichStandings(standings: any[], leagueName?: string): any[] {
  return (Array.isArray(standings) ? standings : [])
    .map((row, index) => {
      const team = normalizeText(
        row?.team || row?.teamName || row?.name || row?.displayName,
      );
      const rank = parseNumber(row?.rank) ?? index + 1;
      const logo = resolveTeamLogoUri(
        team,
        row?.logo || row?.teamLogo || null,
        { competition: leagueName || null },
      );
      const points = parseNumber(row?.points ?? row?.pts) ?? 0;
      const played =
        parseNumber(row?.played ?? row?.gamesPlayed ?? row?.gp) ?? 0;
...
function compactAnalysisText(
  ...
  if (
    raw.length >= 50 &&
    !/temporarily unavailable|not available|unknown/i.test(raw)
  )
    return raw;
  if (!name) return "Profile summary not available yet.";
  if (position)
    return `${name} is profiled as a ${position}. Detailed analysis will appear when richer verified data is available.`;
...
function inferTransferAction(
  role: string,
  fee: string,
  note: string,
): "joined" | "left" | "loan" | "loan_end" | "transfer_fee" {
  ...
  if (/free|gratis|free transfer/.test(text))
    return role === "to" ? "joined" : "left";
...
function transferActionLabel(
  action: "joined" | "left" | "loan" | "loan_end" | "transfer_fee",
): string {
...
function deriveRatingFromContributions(
  ...
  const inferred =
    6.2 + Math.min(1.9, contributions / Math.max(appearances, 1));
  ...
  const rows = sourceRows
    .map((club: any) => {
      const name = stripHtmlArtifacts(
        pickFirstText(club?.name, club?.team, club?.club, club?.to, club?.from),
      );
      const role =
        String(club?.role || "").toLowerCase() === "to" ? "to" : "from";
      const date = stripHtmlArtifacts(
        pickFirstText(club?.date, club?.season, club?.year),
      );
      const fee = stripHtmlArtifacts(
        pickFirstText(club?.fee, club?.transferFee, club?.value),
      );
      ...
      return {
        ...
        moment,
        transferType: action,
        logo: resolveTeamLogoUri(name, club?.logo || null, {
          competition: leagueName || null,
        }),
      };
    })
...
export function enrichPlayerProfilePayload(
  raw: any,
  seed?: { name?: string; team?: string; league?: string },
): any {
  ...
  const currentClub = pickFirstText(
    raw?.currentClub,
    raw?.team,
    raw?.teamName,
    raw?.club?.name,
  ...
  const nationality = pickFirstText(
    raw?.nationality,
    raw?.citizenship,
    raw?.country,
  );

  const jerseyNumber = pickFirstText(
    raw?.jerseyNumber,
    raw?.shirtNumber,
    raw?.number,
    raw?.jersey,
  );
  const contractUntil = pickFirstText(
    raw?.contractUntil,
    raw?.contract?.endDate,
    raw?.contractEndDate,
  );
  const marketValueText = cleanMarketValueText(
    pickFirstText(
      raw?.marketValue,
      raw?.market_value,
      raw?.estimatedMarketValue,
    ),
  );
  ...
  const stats = {
    appearances: coalesceNumber(
      rawSeason?.appearances,
      rawSeason?.matches,
      rawSeason?.games,
      raw?.appearances,
    ),
  ...
  const inferredStarts =
    stats.starts != null
      ? stats.starts
      : stats.appearances != null
        ? Math.max(
            0,
            Math.min(stats.appearances, Math.round(stats.appearances * 0.76)),
          )
        : null;
  const inferredMinutes =
    stats.minutes != null
      ? stats.minutes
      : stats.appearances != null
        ? Math.round(stats.appearances * 74)
        : null;
  const inferredRating = deriveRatingFromContributions(
    stats.rating,
    stats.appearances,
    stats.goals,
    stats.assists,
  ...
  const normalizedHistory = normalizeFormerClubs(
    raw?.formerClubs,
    seed?.league,
  );
  ...
  return {
    ...
    currentClubLogo: resolveTeamLogoUri(
      currentClub,
      raw?.currentClubLogo || null,
      { competition: seed?.league || null },
    ),
...
export function enrichTeamDetailPayload(raw: any): any {
  ...
  const enrichedPlayers = players
    .map((player: any) => {
      ...
      const theSportsDbPhoto = normalizeText(player?.theSportsDbPhoto) || null;

      return {
        ...player,
        name: pName,
        nationality: isGoodText(player?.nationality)
        ...
        marketValue: isGoodText(player?.marketValue)
          ? normalizeText(player?.marketValue)
          : null,
        marketValueNumeric: parseMarketValue(player?.marketValue),
        age: coalesceNumber(player?.age),
        jersey: pickFirstText(
          player?.jersey,
          player?.jerseyNumber,
          player?.shirtNumber,
          player?.number,
        ...
        seasonStats: {
          ...
          assists: coalesceNumber(
            player?.seasonStats?.assists,
            player?.assists,
          ),
          minutes: coalesceNumber(
            player?.seasonStats?.minutes,
            player?.minutes,
          ),
  ...
  const derivedSquadMarketValueNumeric = enrichedPlayers.reduce(
  ...
  const derivedSquadMarketValue =
    derivedSquadMarketValueNumeric > 0
  ...
  const topScorer = isGoodText(raw?.topScorer?.name)
    ...
    : [...enrichedPlayers]
        .filter(
          (player: any) =>
            (coalesceNumber(player?.seasonStats?.goals, player?.goals) || 0) >
            0,
        )
        .sort(
          (a: any, b: any) =>
            (coalesceNumber(b?.seasonStats?.goals, b?.goals) || 0) -
            (coalesceNumber(a?.seasonStats?.goals, a?.goals) || 0),
        )[0] || null;
  ...
  const topAssist = isGoodText(raw?.topAssist?.name)
    ...
    : [...enrichedPlayers]
        .filter(
          (player: any) =>
            (coalesceNumber(player?.seasonStats?.assists, player?.assists) ||
              0) > 0,
        )
        .sort(
          (a: any, b: any) =>
            (coalesceNumber(b?.seasonStats?.assists, b?.assists) || 0) -
            (coalesceNumber(a?.seasonStats?.assists, a?.assists) || 0),
        )[0] || null;

  const wins = coalesceNumber(
    raw?.wins,
    raw?.won,
    raw?.leagueWon,
    raw?.leagueStats?.wins,
  );
  const draws = coalesceNumber(
    raw?.draws,
    raw?.drawn,
    raw?.leagueDrawn,
    raw?.leagueStats?.draws,
  );
  const losses = coalesceNumber(
    raw?.losses,
    raw?.lost,
    raw?.leagueLost,
    raw?.leagueStats?.losses,
  );
  const points = coalesceNumber(
    raw?.leaguePoints,
    raw?.points,
    raw?.leagueStats?.points,
  );
  const played = coalesceNumber(
    raw?.leaguePlayed,
    raw?.played,
    raw?.leagueStats?.played,
  );
  const goalsFor = coalesceNumber(
    raw?.goalsFor,
    raw?.gf,
    raw?.leagueStats?.goalsFor,
  );
  const goalsAgainst = coalesceNumber(
    raw?.goalsAgainst,
    raw?.ga,
    raw?.leagueStats?.goalsAgainst,
  );
  const cleanSheets = coalesceNumber(
    raw?.cleanSheets,
    raw?.leagueStats?.cleanSheets,
  );
  ...
  return {
    ...
    logo: resolveTeamLogoUri(raw?.name, raw?.logo || null, {
      ...
      competition: raw?.leagueName || null,
    }),
    ...
    venue: isGoodText(raw?.venue || raw?.stadium)
      ? pickFirstText(raw?.venue, raw?.stadium)
      : null,
    ...
    topScorer,
    topAssist,
    players: [...enrichedPlayers].sort(
      (a, b) =>
        Number(b?.marketValueNumeric || 0) - Number(a?.marketValueNumeric || 0),
    ),
    enrichment: {
      confidence: computeConfidence([
        Boolean(raw?.name),
...
export function enrichSportsLeagueResource(
  kind: string,
  payload: any,
  context?: { leagueName?: string },
): any {
  ...
  if (kind === "topscorers") {
    const scorers = collectLeaderRows("topscorers", payload)
      ...
      .filter(
        (row: any) =>
          Boolean(row?.name) &&
          (parseNumber(row?.goals) ?? parseNumber(row?.displayValue) ?? -1) >=
            0,
      );
  ...
  if (kind === "topassists") {
    const assists = collectLeaderRows("topassists", payload)
      ...
      .filter(
        (row: any) =>
          Boolean(row?.name) &&
          (parseNumber(row?.assists) ?? parseNumber(row?.displayValue) ?? -1) >=
            0,
      );
    return {
      ...payload,
  ...
  if (kind === "competition-matches") {
    return {
      ...payload,
      matches: enrichCompetitionMatches(payload?.matches || [], leagueName),
    };
  }
