 ...
 * favorites/playlists/IPTV state which already lives there.
 *
 * Usage:
...
import {
  loadFollowedTeams, saveFollowedTeams, followTeam, unfollowTeam, isTeamFollowed,
  loadFollowedMatches, followMatch, unfollowMatch,
  getContinueWatching, trackWatchProgress, clearWatchProgress,
...
interface UserStateContextValue {
  ...
  followMatchAction: (match: Omit<FollowedMatch, "followedAt">) => Promise<void>;
...
export function UserStateProvider({ children }: { children: ReactNode }) {
  ...
  const followTeamAction = useCallback(async (team: Omit<FollowedTeam, "followedAt">) => {
  ...
  const value = useMemo<UserStateContextValue>(() => ({
    ...
    followedTeams, isFollowingTeam, followTeamAction, unfollowTeamAction,
    followedMatches, isFollowingMatch, followMatchAction, unfollowMatchAction,
    continueWatching, trackProgress, clearProgress,
    moodPreferences, refreshMoodPreferences,
    isReady,
  ]);
...
export function useFollowState() {
  const { followedTeams, isFollowingTeam, followTeamAction, unfollowTeamAction,
          followedMatches, isFollowingMatch, followMatchAction, unfollowMatchAction } = useUserState();
  return {
    followedTeams, isFollowingTeam, followTeamAction, unfollowTeamAction,
    followedMatches, isFollowingMatch, followMatchAction, unfollowMatchAction,
  };
