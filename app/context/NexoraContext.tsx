...
interface NexoraContextValue {
  ...
  favorites: string[];
  toggleFavorite: (id: string, type?: "movie" | "series" | "channel" | "sport") => void;
  isFavorite: (id: string, type?: "movie" | "series" | "channel" | "sport") => boolean;
  ...
  preferredServerLabel: string;
  setPreferredServerLabel: (label: string) => void;
  uiLanguage: Language;
...
function normalizeDailyPredictionState(
  raw?: Partial<DailyPredictionUnlockState> | null,
): DailyPredictionUnlockState {
...
export function NexoraProvider({ children }: { children: ReactNode }) {
  ...
  const [preferredServerLabel, setPreferredServerLabelState] =
    useState("Server 1");
  ...
  const [premiumCategories, setPremiumCategoriesState] = useState<
    PremiumCategory[]
  >([]);
  ...
  const [dailyPredictionUnlockState, setDailyPredictionUnlockState] =
    useState<DailyPredictionUnlockState>(() => normalizeDailyPredictionState());
      ...
      try {
        ...
        if (!schema) {
          await AsyncStorage.multiRemove([
            "nexora_playlists",
            "nexora_iptv_channels", // Legacy cleanup — remove stale IPTV data from older versions
            "nexora_hidden_channels",
            "nexora_hidden_groups",
            "nexora_schema_v2",
          ]);
        ...
        const keys = [
          ...
          "nexora_downloads",
          "nexora_ui_lang",
          "nexora_preferred_server",
          "nexora_prediction_unlock_daily",
          "nexora_prediction_unlock_matches",
        ];
        const [
          favs,
          hist,
        ...
        if (preferredServer) setPreferredServerLabelState(preferredServer);
        if (uiLang && ["en", "nl", "fr", "de", "es", "pt"].includes(uiLang)) {
        ...
        if (dailyUnlock) {
          const normalized = normalizeDailyPredictionState(
            JSON.parse(dailyUnlock),
          );
        ...
        try {
          ...
          if (!cancelled) {
            setPremiumCategoriesState(
              hasPremiumEntitlement(customerInfo) ? ALL_CATS : [],
            );
          }
        } catch {
          if (!cancelled) {
  ...
  const addToHistory = async (item: WatchedItem) => {
    ...
    const next = [
      ...
      ...watchHistory.filter((h) => {
        ...
        if (
          h.contentId &&
          namespacedItem.contentId &&
          h.contentId === namespacedItem.contentId
        )
          return false;
        return true;
      }),
  ...
  const updateProgress = async (
    ...
    const idx = watchHistory.findIndex(
      (h) => h.id === namespacedId || h.contentId === namespacedId,
    );
    if (idx < 0) return;
    const updated = {
      ...watchHistory[idx],
  ...
  const setPreferredServerLabel = async (label: string) => {
    setPreferredServerLabelState(label);
    await AsyncStorage.setItem("nexora_preferred_server", label);
  };
  ...
  const activatePremiumCategories = async (cats: PremiumCategory[]) => {
    const merged = Array.from(
      new Set([...premiumCategories, ...cats]),
    ) as PremiumCategory[];
  ...
  const purchasePremiumSubscription = async (
    ...
    try {
      ...
      return {
        ok: false,
        reason: String((error as any)?.message || "Purchase failed."),
      };
    }
  };
    ...
    try {
      ...
      return restored
        ...
        : {
            ok: false,
            restored: false,
            reason: "No active premium purchase found to restore.",
          };
    } catch (error) {
      return {
        ok: false,
        restored: false,
        reason: String((error as any)?.message || "Restore failed."),
      };
    }
  };

  const normalizedUnlockState = normalizeDailyPredictionState(
    dailyPredictionUnlockState,
  );
  const dailyPredictionUnlocksRemaining = hasPremium("sport")
    ? 999
    : Math.max(0, 1 - normalizedUnlockState.used);
  ...
  const isPredictionUnlocked = (matchId: string) => {
    ...
    return (
      normalizedUnlockState.matchId === normalizedId ||
      unlockedPredictionIds.includes(normalizedId)
    );
  };

  const unlockPredictionWithRewardedAd = async (
    matchId: string,
  ): Promise<{ ok: boolean; reason?: string }> => {
    const normalizedId = String(matchId || "").trim();
    ...
    if (current.used >= 1 && current.matchId !== normalizedId) {
      return {
        ok: false,
        reason:
          "Free daily unlock already used. Upgrade to premium for unlimited access.",
      };
    }
  ...
  const addDownload = async (item: DownloadedItem) => {
    const next = [
      item,
      ...downloads.filter((d) => d.contentId !== item.contentId),
    ];
    setDownloads(next);
  ...
  const isDownloaded = (contentId: string) =>
    downloads.some((d) => d.contentId === contentId);
  const getDownload = (contentId: string) =>
    downloads.find((d) => d.contentId === contentId);
  ...
  const resetAll = async () => {
    ...
    setPreferredServerLabelState("Server 2");
